...
#define LOCTEXT_NAMESPACE "ODVariableDragDropAction"
...
UBlueprint* FKismetVariableDragDropAction::GetSourceOdysseyBrush() const
{
                           ...
                           TArray<class UEdGraphPin*>& OutBroken)
{
    ...
    {
        if(const UEdGraphSchema_OdysseyBrush* Schema = Cast<const UEdGraphSchema_OdysseyBrush>(VarNodeUnderCursor->GetSchema()) )
        {
...
void FKismetVariableDragDropAction::HoverTargetChanged()
{
    ...
    if (TheHoveredGraph)
    {
        if (Cast<const UEdGraphSchema_OdysseyBrush>(TheHoveredGraph->GetSchema()) == nullptr)
        {
            bBadSchema = true;
        }
        ...
        const UBlueprint* DropOnOdysseyBrush = FBlueprintEditorUtils::FindBlueprintForGraph(TheHoveredGraph);
    ...
    else if (bBadGraph)
    {
        ...
        if (IsFromOdysseyBrush(FBlueprintEditorUtils::FindBlueprintForGraph(TheHoveredGraph)) && VariableProperty->GetOuter()->IsA(UFunction::StaticClass()))
        {
    ...
    else if (PinUnderCursor)
    {
        ...
        if (CanVariableBeDropped(VariableProperty, *PinUnderCursor->GetOwningNode()->GetGraph()))
        {
            ...
            {
                const UEdGraphSchema_OdysseyBrush* Schema = CastChecked<const UEdGraphSchema_OdysseyBrush>(PinUnderCursor->GetSchema());
                const bool bIsExecPin = Schema->IsExecPin(*PinUnderCursor);

                const bool bIsRead = (PinUnderCursor->Direction == EGPD_Input) && !bIsExecPin;
                const UBlueprint* OdysseyBrush = FBlueprintEditorUtils::FindBlueprintForNode(PinUnderCursor->GetOwningNode());
                const bool bWritableProperty = (FBlueprintEditorUtils::IsPropertyWritableInBlueprint(OdysseyBrush, VariableProperty) == FBlueprintEditorUtils::EPropertyWritableState::Writable);
    ...
    else if (VarNodeUnderCursor)
    {
        ...
        if (CanVariableBeDropped(VariableProperty, *VarNodeUnderCursor->GetGraph()))
        {
            ...
            const UBlueprint* OdysseyBrush = FBlueprintEditorUtils::FindBlueprintForNode(VarNodeUnderCursor);
            const bool bWritableProperty = (FBlueprintEditorUtils::IsPropertyWritableInBlueprint(OdysseyBrush, VariableProperty) == FBlueprintEditorUtils::EPropertyWritableState::Writable);
            const bool bCanWriteIfNeeded = bIsRead || bWritableProperty;
    ...
    {
        FMyOdysseyBrushItemDragDropAction::HoverTargetChanged();
    }
}
...
{
    PrimaryBrushOut = FOdysseyBrushEditor::GetVarIconAndColor(VariableSource.Get(), VariableName, IconColorOut, SecondaryBrushOut, SecondaryColorOut);
}
...
FReply FKismetVariableDragDropAction::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
    if (UEdGraphPin* TargetPin = GetHoveredPin())
    {
        ...
        {
            if (const UEdGraphSchema_OdysseyBrush* Schema = Cast<const UEdGraphSchema_OdysseyBrush>(TargetPin->GetSchema()))
            {
                ...
                if (CanVariableBeDropped(VariableProperty, *TargetPin->GetOwningNode()->GetGraph()))
                {
                    ...
                    const UBlueprint* OdysseyBrush = FBlueprintEditorUtils::FindBlueprintForNode(TargetPin->GetOwningNode());
                    const bool bWritableProperty = (FBlueprintEditorUtils::IsPropertyWritableInBlueprint(OdysseyBrush, VariableProperty) == FBlueprintEditorUtils::EPropertyWritableState::Writable);
                    const bool bCanWriteIfNeeded = bIsRead || bWritableProperty;
                    ...
                    if (bTypeMatch && bCanWriteIfNeeded)
                    {
                        ...
                        UBlueprint* DropOnOdysseyBrush = FBlueprintEditorUtils::FindBlueprintForGraph(TargetPin->GetOwningNode()->GetGraph());
                        UEdGraphSchema_OdysseyBrush::ConfigureVarNode(VarNode, VariableName, VariableSource.Get(), DropOnOdysseyBrush);
...
FReply FKismetVariableDragDropAction::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
    ...
    if (TargetNode && (VariableName != TargetNode->GetVarName()))
    {
        ...
        if(CanVariableBeDropped(VariableProperty, *TargetNode->GetGraph()))
        {
            ...
            const UEdGraphSchema_OdysseyBrush* Schema = Cast<const UEdGraphSchema_OdysseyBrush>(TargetNode->GetSchema());
            ...
            UBlueprint* DropOnOdysseyBrush = FBlueprintEditorUtils::FindBlueprintForGraph(TargetNode->GetGraph());
            UEdGraphPin* Pin = TargetNode->FindPin(OldVarName);
            DropOnOdysseyBrush->Modify();
            TargetNode->Modify();
            ...
            UEdGraphSchema_OdysseyBrush::ConfigureVarNode(TargetNode, VariableName, VariableSource.Get(), DropOnOdysseyBrush);
            ...
            FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(DropOnOdysseyBrush);
...
void FKismetVariableDragDropAction::MakeGetter(FNodeConstructionParams InParams)
{
    ...
    const UEdGraphSchema_OdysseyBrush* K2_Schema = Cast<const UEdGraphSchema_OdysseyBrush>(InParams.Graph->GetSchema());
    if (K2_Schema)
    {
...
bool FKismetVariableDragDropAction::CanExecuteMakeSetter(FNodeConstructionParams InParams, UProperty* InVariableProperty)
{
    ...
    {
        const UBlueprint* OdysseyBrush = FBlueprintEditorUtils::FindBlueprintForGraph(InParams.Graph);
        const bool bWritableProperty = (FBlueprintEditorUtils::IsPropertyWritableInBlueprint(OdysseyBrush, InVariableProperty) == FBlueprintEditorUtils::EPropertyWritableState::Writable);
...
{
    if (Graph.GetSchema()->IsA<UEdGraphSchema_OdysseyBrush>())
    {
        UProperty* VariableProperty = GetVariableProperty();
...
bool FKismetVariableDragDropAction::CanVariableBeDropped(const UProperty* InVariableProperty, const UEdGraph& InGraph) const
{
    ...
    if (InVariableProperty)
    {
        ...
        bCanVariableBeDropped = IsFromOdysseyBrush(FBlueprintEditorUtils::FindBlueprintForGraph(&InGraph));
