...
#define LOCTEXT_NAMESPACE "OdysseySCSEditor"
...
FReply SSCSEditor::TryHandleAssetDragDropOperation(const FDragDropEvent& DragDropEvent)
{
    ...
    if (Operation.IsValid() && (Operation->IsOfType<FExternalDragOperation>() || Operation->IsOfType<FAssetDragDropOp>()))
    {
        ...
        if (NumAssets > 0)
        {
            ...
            if (bMarkBlueprintAsModified && EditorMode == EComponentEditorMode::OdysseyBrushSCS)
            {
                UBlueprint* OdysseyBrush = GetBlueprint();
                check(OdysseyBrush != nullptr && OdysseyBrush->SimpleConstructionScript != nullptr);

                OdysseyBrush->Modify();
                SaveSCSCurrentState(OdysseyBrush->SimpleConstructionScript);
...
UActorComponent* FSCSEditorTreeNode::GetEditableComponentTemplate(UBlueprint* ActualEditedOdysseyBrush)
{
    return nullptr;
}
...
FString FSCSEditorTreeNodeComponentBase::GetDisplayString() const
{
    ...
    UBlueprint* OdysseyBrush = GetBlueprint();
    UClass* VariableOwner = (OdysseyBrush != nullptr) ? OdysseyBrush->SkeletonGeneratedClass : nullptr;
...
UActorComponent* FSCSEditorTreeNodeInstancedInheritedComponent::GetEditableComponentTemplate(UBlueprint* ActualEditedOdysseyBrush)
{
    if (CanEditDefaults())
    {
...
UActorComponent* FSCSEditorTreeNodeInstanceAddedComponent::GetEditableComponentTemplate(UBlueprint* ActualEditedOdysseyBrush)
{
    return GetComponentTemplate();
}
...
UActorComponent* FSCSEditorTreeNodeComponent::GetEditableComponentTemplate(UBlueprint* ActualEditedOdysseyBrush)
{
    if (CanEditDefaults())
    {
        if (!IsNative() && IsInheritedSCS())
        {
            if (ActualEditedOdysseyBrush != nullptr)
            {
                return INTERNAL_GetOverridenComponentTemplate(ActualEditedOdysseyBrush, true);
            }
            else
            {
                return nullptr;
    ...
    if (InActor != NULL)
    {
        if (SCS_Node != NULL)
        {
            ...
            if (VariableName != NAME_None)
            {
                ...
                {
                    // If this is the preview actor, return the cached component instance that's being used for the preview actor prior to recompiling the OdysseyBrush
...
UActorComponent* FSCSEditorTreeNodeComponent::INTERNAL_GetOverridenComponentTemplate(UBlueprint* OdysseyBrush, bool bCreateIfNecessary) const
{
    UActorComponent* OverriddenComponent = NULL;

    FComponentKey Key(GetSCSNode());

    const bool OdysseyBrushCanOverrideComponentFromKey = Key.IsValid()
        && OdysseyBrush
        && OdysseyBrush->ParentClass
        && OdysseyBrush->ParentClass->IsChildOf(Key.GetComponentOwner());

    if (OdysseyBrushCanOverrideComponentFromKey)
    {
        UInheritableComponentHandler* InheritableComponentHandler = OdysseyBrush->GetInheritableComponentHandler(bCreateIfNecessary);
...
FText SSCS_RowWidget::GetComponentAddSourceToolTipText() const
{
    ...
    if (FSCSEditorTreeNode* TreeNode = TreeNodePtr.Get())
    {
        if (TreeNode->IsInherited())
        {
            ...
            {
                NodeType = LOCTEXT("InheritedOdysseyBrushComponent", "Inherited (OdysseyBrush)");
            }
        }
        else
        {
            ...
            {
                NodeType = LOCTEXT("ThisOdysseyBrushAddedComponent", "This OdysseyBrush");
            }
        }
    }
        ...
        if (TreeNode->IsInherited())
        {
            if (UActorComponent* ComponentTemplate = TreeNode->GetComponentTemplate())
            {
                ...
                if (BestClass == nullptr)
                {
                    ...
                    {
                        IntroducedInTooltip = LOCTEXT("IntroducedInUnknownError", "Unknown OdysseyBrush Class (via an Add Component call)");
                    }
                    else
                    {
...
{
    const FLinearColor InheritedOdysseyBrushComponentColor(0.08f, 0.35f, 0.6f);
    const FLinearColor InstancedInheritedOdysseyBrushComponentColor(0.08f, 0.35f, 0.6f);
    ...
    if (InNode->IsInherited())
    {
        ...
        {
            return InstancedInheritedOdysseyBrushComponentColor;
        }
        else
        {
            return InheritedOdysseyBrushComponentColor;
        }
    }
    else
    {
...
TSharedPtr<SWidget> SSCS_RowWidget::BuildSceneRootDropActionMenu(FSCSEditorTreeNodePtrType DroppedNodePtr)
{
    ...
    MenuBuilder.BeginSection("SceneRootNodeDropActions", LOCTEXT("SceneRootNodeDropActionContextMenu", "Drop Actions"));
    {
        ...
        bool bDroppedInSameOdysseyBrush = true;
        if (SCSEditor.Pin()->GetEditorMode() == EComponentEditorMode::OdysseyBrushSCS)
        {
            bDroppedInSameOdysseyBrush = DroppedNodePtr->GetBlueprint() == GetBlueprint();
        }

        MenuBuilder.AddMenuEntry(
            LOCTEXT("DropActionLabel_AttachToRootNode", "Attach"),
            bDroppedInSameOdysseyBrush
        ...
        MenuBuilder.AddMenuEntry(
            ...
            bDroppedInSameOdysseyBrush ? NewRootNodeText : NewRootNodeFromCopyText,
            FSlateIcon(),
            FUIAction(
...
FReply SSCS_RowWidget::HandleOnDragDetected( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
        ...
        && SCSEditorPtr->IsEditingAllowed()) //can only drag when editing
    {
        ...
        {
            // Do not use the OdysseyBrush from FirstNode, it may still be referencing the parent.
            UBlueprint* OdysseyBrush = GetBlueprint();
            const FName VariableName = FirstNode->GetVariableName();
            UStruct* VariableScope = (OdysseyBrush != nullptr) ? OdysseyBrush->SkeletonGeneratedClass : nullptr;
            ...
            for (auto SourceNodeIter = DragRowOp->SourceNodes.CreateConstIterator(); SourceNodeIter && (Message.IsEmpty() || DragRowOp->PendingDropAction != FSCSRowDragDropOp::DropAction_None); ++SourceNodeIter)
            {
                ...
                else if (NodePtr == SceneRootNodePtr)
                {
                    ...
                    if (!NodePtr->CanReparent() && (!NodePtr->IsDefaultSceneRoot() || NodePtr->IsInherited()))
                    {
                        ...
                        Message = LOCTEXT("DropActionToolTip_Error_CannotReparentRootNode", "The root component in this OdysseyBrush is inherited and cannot be replaced.");
                    }
                    ...
                    else if (SCSEditor.Pin()->GetEditorMode() == EComponentEditorMode::OdysseyBrushSCS && DraggedNodePtr->GetBlueprint() != GetBlueprint())
                    {
                        if (bCanMakeNewRoot)
                        {
                            if (NodePtr->IsDefaultSceneRoot())
                            {
                                // Only available action is to copy the dragged node to the other OdysseyBrush and make it the new root
                        ...
                        {
                            // Only available action is to copy the dragged node(s) to the other OdysseyBrush and attach it to the root
                            if (DragRowOp->SourceNodes.Num() > 1)
                            {
...
void SSCS_RowWidget::OnAttachToDropAction(const TArray<FSCSEditorTreeNodePtrType>& DroppedNodePtrs)
{
    ...
    if (SCSEditorPtr->GetEditorMode() == EComponentEditorMode::OdysseyBrushSCS)
    {
        // Get the current OdysseyBrush context
        UBlueprint* OdysseyBrush = GetBlueprint();
        check(OdysseyBrush);
        ...
        for(const auto& DroppedNodePtr : DroppedNodePtrs)
        {
            ...
            if(DroppedNodePtr->GetBlueprint() != OdysseyBrush)
            {
                ...
                // Note: This will mark the OdysseyBrush as structurally modified
...
void SSCS_RowWidget::OnMakeNewRootDropAction(FSCSEditorTreeNodePtrType DroppedNodePtr)
{
    ...
    if (SCSEditorPtr->GetEditorMode() == EComponentEditorMode::OdysseyBrushSCS)
    {
        // Get the current OdysseyBrush context
        UBlueprint* OdysseyBrush = GetBlueprint();
        check(OdysseyBrush && OdysseyBrush->SimpleConstructionScript);
        ...
        if(DroppedNodePtr->GetParent().IsValid()
            && DroppedNodePtr->GetBlueprint() == OdysseyBrush)
        {
        ...
        OdysseyBrush->SimpleConstructionScript->RemoveNode(SceneRootNodePtr->GetSCSNode());
        ...
        OdysseyBrush->SimpleConstructionScript->AddNode(SceneRootNodePtr->GetSCSNode());
...
void SSCS_RowWidget::PostDragDropAction(bool bRegenerateTreeNodes)
{
    ...
    if(PinnedEditor.IsValid())
    {
        ...
        if (PinnedEditor->GetEditorMode() == EComponentEditorMode::OdysseyBrushSCS)
        {
            if(NodePtr.IsValid())
            {
                UBlueprint* OdysseyBrush = GetBlueprint();
                ...
                {
                    FBlueprintEditorUtils::PostEditChangeBlueprintActors(OdysseyBrush, true);
                }
            }
        }
        else
...
FString SSCS_RowWidget::GetDocumentationLink() const
{
    ...
    {
        return TEXT("Shared/Editors/OdysseyBrushEditor/ComponentsMode");
    }
...
bool SSCS_RowWidget::OnNameTextVerifyChanged(const FText& InNewText, FText& OutErrorMessage)
{
    ...
    if (!InNewText.IsEmpty())
    {
        ...
        if ((ExistingNameSearchScope == nullptr) && (OdysseyBrush != nullptr))
        {
            ExistingNameSearchScope = Cast<AActor>(OdysseyBrush->GeneratedClass->GetDefaultObject());
        }
...
FText SSCS_RowWidget_ActorRoot::GetActorDisplayText() const
{
    if (SCSEditor.IsValid())
    {
        ...
        if (SCSEditorPtr->ActorContext.IsSet())
        {
            ...
            {
                FString Name;
                UBlueprint* OdysseyBrush = UBlueprint::GetBlueprintFromClass(DefaultActor->GetClass());
                if(OdysseyBrush != nullptr && SCSEditorPtr->GetEditorMode() != EComponentEditorMode::ActorInstance)
                {
                    OdysseyBrush->GetName(Name);
                }
                else
                {
...
FText SSCS_RowWidget_ActorRoot::GetActorContextText() const
{
    if (SCSEditor.IsValid())
    {
        ...
        {
            if (UBlueprint* OdysseyBrush = UBlueprint::GetBlueprintFromClass(DefaultActor->GetClass()))
            {
...
void SSCSEditor::Construct( const FArguments& InArgs )
{
    ...
    SCSTreeWidget = SNew(SSCSTreeType)
        ...
        .ClearSelectionOnClick(InArgs._EditorMode == EComponentEditorMode::OdysseyBrushSCS ? true : false)
    ...
    FMenuBuilder EditOdysseyBrushMenuBuilder( true, NULL );

    EditOdysseyBrushMenuBuilder.BeginSection( NAME_None, LOCTEXT("EditOdysseyBrushMenu_ExistingOdysseyBrushHeader", "Existing OdysseyBrush" ) );

    EditOdysseyBrushMenuBuilder.AddMenuEntry
    (
        LOCTEXT("OpenOdysseyBrushEditor", "Open OdysseyBrush Editor"),
        LOCTEXT("OpenOdysseyBrushEditor_ToolTip", "Opens the blueprint editor for this asset"),
        FSlateIcon(),
        FUIAction(FExecuteAction::CreateSP(this, &SSCSEditor::OnOpenOdysseyBrushEditor, /*bForceCodeEditing=*/ false))
    );

    EditOdysseyBrushMenuBuilder.AddMenuEntry
    (
        LOCTEXT("OpenOdysseyBrushEditorScriptMode", "Add or Edit Script"),
        LOCTEXT("OpenOdysseyBrushEditorScriptMode_ToolTip", "Opens the blueprint editor for this asset, showing the event graph"),
        FSlateIcon(),
        FUIAction(FExecuteAction::CreateSP(this, &SSCSEditor::OnOpenOdysseyBrushEditor, /*bForceCodeEditing=*/ true))
    );

    EditOdysseyBrushMenuBuilder.BeginSection(NAME_None, LOCTEXT("EditOdysseyBrushMenu_InstanceHeader", "Instance modifications"));

    EditOdysseyBrushMenuBuilder.AddMenuEntry
    (
        LOCTEXT("PushChangesToOdysseyBrush", "Apply Instance Changes to OdysseyBrush"),
        TAttribute<FText>(this, &SSCSEditor::OnGetApplyChangesToOdysseyBrushTooltip),
        FSlateIcon(),
        FUIAction(FExecuteAction::CreateSP(this, &SSCSEditor::OnApplyChangesToOdysseyBrush))
    );

    EditOdysseyBrushMenuBuilder.AddMenuEntry
    (
        LOCTEXT("ResetToDefault", "Reset Instance Changes to OdysseyBrush Default"),
        TAttribute<FText>(this, &SSCSEditor::OnGetResetToOdysseyBrushDefaultsTooltip),
        FSlateIcon(),
        FUIAction(FExecuteAction::CreateSP(this, &SSCSEditor::OnResetToOdysseyBrushDefaults))
    );

    EditOdysseyBrushMenuBuilder.BeginSection( NAME_None, LOCTEXT("EditOdysseyBrushMenu_NewHeader", "Create New" ) );
    //EditOdysseyBrushMenuBuilder.AddMenuSeparator();

    EditOdysseyBrushMenuBuilder.AddMenuEntry
    (
        LOCTEXT("CreateChildOdysseyBrush", "Create Child OdysseyBrush Class"),
        LOCTEXT("CreateChildOdysseyBrushTooltip", "Creates a Child OdysseyBrush Class based on the current OdysseyBrush, allowing you to create variants easily.  This replaces the current actor instance with a new one based on the new Child OdysseyBrush Class." ),
        FSlateIcon(),
        FUIAction(FExecuteAction::CreateSP(this, &SSCSEditor::PromoteToOdysseyBrush))
    );
    ...
    const bool  bInlineSearchBarWithButtons = (EditorMode == EComponentEditorMode::OdysseyBrushSCS);
            ...
            [
                SAssignNew(HeaderBox, SVerticalBox)
                    ...
                    [
                        ...
                        [
                            SNew( SButton )
                            .AddMetaData<FTagMetaData>(FTagMetaData(TEXT("Actor.ConvertToOdysseyBrush")))
                            .Visibility( this, &SSCSEditor::GetPromoteToOdysseyBrushButtonVisibility )
                            .OnClicked( this, &SSCSEditor::OnPromoteToOdysseyBrushClicked )
                            .ButtonStyle(FEditorStyle::Get(), "FlatButton.Primary")
                            .ContentPadding(FMargin(10,0))
                            .ToolTip(IDocumentation::Get()->CreateToolTip(
                                LOCTEXT("PromoteToBluerprintTooltip","Converts this actor into a reusable OdysseyBrush Class that can have script behavior" ),
                                NULL,
                                TEXT("Shared/LevelEditor"),
                                TEXT("ConvertToOdysseyBrush")))
                            [
                                SNew(SHorizontalBox)
                                ...
                                [
                                    SNew(STextBlock)
                                    .TextStyle(FEditorStyle::Get(), "ContentBrowser.TopBar.Font")
                                    //.Text( LOCTEXT("PromoteToOdysseyBrush", "Add Script") )
                                    .Text(LOCTEXT("PromoteToOdysseyBrush", "Blueprint/Add Script"))
                                ]
                            ]
                        ]
                        + SHorizontalBox::Slot()
                        ...
                        [
                            SNew(SComboButton)
                            .AddMetaData<FTagMetaData>(FTagMetaData(TEXT("Actor.EditOdysseyBrush")))
                            .Visibility(this, &SSCSEditor::GetEditOdysseyBrushButtonVisibility)
                            .ContentPadding(FMargin(10, 0))
                            ...
                            [
                                ...
                                [
                                    SNew(STextBlock)
                                    .TextStyle(FEditorStyle::Get(), "ContentBrowser.TopBar.Font")
                                    .Text(LOCTEXT("EditOdysseyBrush", "Edit OdysseyBrush"))
                                ]
                            ]
                            .MenuContent()
                            [
                                EditOdysseyBrushMenuBuilder.MakeWidget()
                            ]
                        ]
                    ]
...
void SSCSEditor::GetSelectedItemsForContextMenu(TArray<FOdysseyComponentEventConstructionData>& OutSelectedItems) const
{
        ...
        if (SelectedItems.Num() > 0)
        {
            ...
            else
            {
                ...
                if (!bOnlyShowPasteOption)
                {
                    ...
                    if (EditorMode == EComponentEditorMode::OdysseyBrushSCS)
                    {
                        ...
                        if ( SelectionClasses.Num() )
                        {
                            ...
                            if( FBlueprintEditorUtils::CanClassGenerateEvents( SelectedClass ))
                            {
                                MenuBuilder.AddSubMenu(    LOCTEXT("AddEventSubMenu", "Add Event"),
                                    ...
                                    FOdysseyGetSelectedObjectsDelegate::CreateSP(this, &SSCSEditor::GetSelectedItemsForContextMenu)));
                            }
                        }
                    }
...
void SSCSEditor::BuildMenuEventsSection(FMenuBuilder& Menu, UBlueprint* OdysseyBrush, UClass* SelectedClass, FCanExecuteAction CanExecuteActionDelegate, FOdysseyGetSelectedObjectsDelegate GetSelectedObjectsDelegate)
{
    // Get Selected Nodes
    TArray<FOdysseyComponentEventConstructionData> SelectedNodes;
        ...
        if (!Property->HasAnyPropertyFlags(CPF_Parm) && Property->HasAllPropertyFlags(CPF_BlueprintAssignable))
        {
            ...
            for (auto NodeIter = SelectedNodes.CreateConstIterator(); NodeIter; ++NodeIter )
            {
                if( NodeIter->Component.IsValid() )
                {
                    ...
                    UObjectProperty* VariableProperty = FindField<UObjectProperty>( OdysseyBrush->SkeletonGeneratedClass, VariableName );

                    if( VariableProperty && FKismetEditorUtilities::FindBoundEventForComponent( OdysseyBrush, EventName, VariableProperty->GetFName() ))
                    {
                        FMenuEntry NewEntry;
                        ...
                        NewEntry.UIAction =    FUIAction(FExecuteAction::CreateStatic( &SSCSEditor::ViewEvent, OdysseyBrush, EventName, *NodeIter ), CanExecuteActionDelegate);
            ...
            // Create menu Add entry
                ...
                NewEntry.UIAction =    FUIAction(FExecuteAction::CreateStatic( &SSCSEditor::CreateEventsForSelection, OdysseyBrush, EventName, GetSelectedObjectsDelegate), CanExecuteActionDelegate);
...
void SSCSEditor::CreateEventsForSelection(UBlueprint* OdysseyBrush, FName EventName, FOdysseyGetSelectedObjectsDelegate GetSelectedObjectsDelegate)
{
    if (EventName != NAME_None)
    {
        ...
        {
            ConstructEvent( OdysseyBrush, EventName, *SelectionIter );
        }
    }
}

void SSCSEditor::ConstructEvent(UBlueprint* OdysseyBrush, const FName EventName, const FOdysseyComponentEventConstructionData EventData)
{
    // Find the corresponding variable property in the OdysseyBrush
    UObjectProperty* VariableProperty = FindField<UObjectProperty>(OdysseyBrush->SkeletonGeneratedClass, EventData.VariableName );

    if( VariableProperty )
    {
        if (!FKismetEditorUtilities::FindBoundEventForComponent(OdysseyBrush, EventName, VariableProperty->GetFName()))
        {
            FKismetEditorUtilities::CreateNewBoundEventForComponent(EventData.Component.Get(), EventName, OdysseyBrush, VariableProperty);
        }
    }
}

void SSCSEditor::ViewEvent(UBlueprint* OdysseyBrush, const FName EventName, const FOdysseyComponentEventConstructionData EventData)
{
    // Find the corresponding variable property in the OdysseyBrush
    UObjectProperty* VariableProperty = FindField<UObjectProperty>(OdysseyBrush->SkeletonGeneratedClass, EventData.VariableName );
    ...
    if( VariableProperty )
    {
        const UK2Node_ComponentBoundEvent* ExistingNode = FKismetEditorUtilities::FindBoundEventForComponent(OdysseyBrush, EventName, VariableProperty->GetFName());
        if (ExistingNode)
        {
...
void SSCSEditor::OnFindReferences()
{
    ...
    if (SelectedNodes.Num() == 1)
    {
        ...
        if (FoundAssetEditor.IsValid())
        {
            ...
            //@TODO: This logic is duplicated in SMyOdysseyBrush::OnFindReference(), keep in sync
            const FString SearchTerm = FString::Printf(TEXT("Nodes(VariableReference(MemberName=+\"%s\") || Name=\"(%s)\")"), *VariableName, *VariableName);

            TSharedRef<IOdysseyBrushEditor> OdysseyBrushEditor = StaticCastSharedRef<IOdysseyBrushEditor>(FoundAssetEditor.ToSharedRef());
            OdysseyBrushEditor->SummonSearchUI(true, SearchTerm);
        }
    }
}
    ...
    if(ActorComponent)
    {
        if (EditorMode == EComponentEditorMode::OdysseyBrushSCS)
        {
            ...
            if (!ActorComponent->IsTemplate())
            {
                ...
                // If the given component is one that's created during OdysseyBrush construction
...
void SSCSEditor::UpdateTree(bool bRegenerateTreeNodes)
{
    ...
    if(bRegenerateTreeNodes)
    {
        ...
        if (EditorMode == EComponentEditorMode::OdysseyBrushSCS)
        {
            ...
            if(AActor* Actor = GetActorContext())
            {
                ...
                if(ActorClass != nullptr)
                {
                    ...
                    // If it's a OdysseyBrush-generated class, also get the inheritance stack
            ...
            for(int32 StackIndex = ParentBPStack.Num() - 1; StackIndex >= 0; --StackIndex)
            {
                if(ParentBPStack[StackIndex]->SimpleConstructionScript != nullptr)
                {
                    ...
                    for(int32 NodeIndex = 0; NodeIndex < SCS_RootNodes.Num(); ++NodeIndex)
                    {
                        ...
                        {
                            // This call creates ICH override templates for the current OdysseyBrush. Without this, the parent node
        ...
        if(SelectedTreeNodes.Num() > 0)
        {
            ...
            if (GetEditorMode() != EComponentEditorMode::OdysseyBrushSCS)
            {
...
void SSCSEditor::DumpTree()
{
    ...
    */
    ...
    const UBlueprint* OdysseyBrushContext = nullptr;
    const AActor* ActorInstance = GetActorContext();
    if (ActorInstance)
    {
        OdysseyBrushContext = UBlueprint::GetBlueprintFromClass(ActorInstance->GetClass());
    }
        ...
        if (NodeList.Num() > 0)
        {
            ...
            if (Node.IsValid())
            {
                ...
                case FSCSEditorTreeNode::ENodeType::RootActorNode:
                    ...
                    case EComponentEditorMode::OdysseyBrushSCS:
                        NodeLabel = TEXT("[BLUEPRINT]");
                        break;
                    }

                    if (OdysseyBrushContext)
                    {
                        NodeLabel += FString::Printf(TEXT(" %s (self)"), *OdysseyBrushContext->GetName());
                    }
...
class FOdysseyBrushComponentClassParentFilter : public FComponentClassParentFilter
{
public:
    FOdysseyBrushComponentClassParentFilter(const TSubclassOf<UActorComponent>& InComponentClass) : FComponentClassParentFilter(InComponentClass) {}
...
UClass* SSCSEditor::CreateNewBPComponent(TSubclassOf<UActorComponent> ComponentClass)
{
    ...
    FGameProjectGenerationModule::Get().OpenAddBlueprintToProjectDialog(
        FAddToProjectConfig()
        .WindowTitle(LOCTEXT("AddNewOdysseyBrushComponent", "Add OdysseyBrush Component"))
        .ParentWindow(FSlateApplication::Get().FindWidgetWindow(SharedThis(this)))
        .Modal()
        .AllowableParents(MakeShareable( new FOdysseyBrushComponentClassParentFilter(ComponentClass) ))
        .FeatureComponentClasses()
...
UActorComponent* SSCSEditor::AddNewComponent( UClass* NewComponentClass, UObject* Asset, const bool bSkipMarkOdysseyBrushModified, const bool bSetFocusToNewItem )
{
    ...
    if (EditorMode == EComponentEditorMode::OdysseyBrushSCS)
    {
        ...
        // Defer OdysseyBrush class regeneration and tree updates if we need to copy object properties from a source template.
        const bool bMarkOdysseyBrushModified = !ComponentTemplate && !bSkipMarkOdysseyBrushModified;
        if(!bMarkOdysseyBrushModified)
        {
        ...
        NewComponent = AddNewNode(OdysseyBrush->SimpleConstructionScript->CreateNode(NewComponentClass, NewVariableName), Asset, bMarkOdysseyBrushModified, bSetFocusToNewItem);
        ...
        if (ComponentTemplate)
        {
            ...
            if (!bSkipMarkOdysseyBrushModified)
            {
                bAllowTreeUpdates = true;
                FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(OdysseyBrush);
            }
...
UActorComponent* SSCSEditor::AddNewNode(USCS_Node* NewNode, UObject* Asset, bool bMarkOdysseyBrushModified, bool bSetFocusToNewItem)
{
    ...
    if(VariableName != NAME_None)
    {
        FBlueprintEditorUtils::ValidateBlueprintChildVariables(OdysseyBrush, VariableName);
    }
    ...
    // Will call UpdateTree as part of OnOdysseyBrushChanged handling
    if(bMarkOdysseyBrushModified)
    {
        FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(OdysseyBrush);
    }
...
void SSCSEditor::PasteNodes()
{
    ...
    if (EditorMode == EComponentEditorMode::OdysseyBrushSCS)
    {
        ...
        for (const TPair<FName, UActorComponent*>& NewObjectPair : NewObjectMap)
        {
            ...
            NewActorComponent = AddNewNode(OdysseyBrush->SimpleConstructionScript->CreateNodeAndRenameComponent(NewActorComponent), nullptr, false, false);
        ...
        // Modify the OdysseyBrush generated class structure (this will also call UpdateTree() as a result)
        FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(OdysseyBrush);
    }
    else    // EComponentEditorMode::ActorInstance
    {
...
void SSCSEditor::OnDeleteNodes()
{
    ...
    if (EditorMode == EComponentEditorMode::OdysseyBrushSCS)
    {
        ...
        FThumbnailRenderingInfo* RenderInfo = GUnrealEd->GetThumbnailManager()->GetRenderingInfo( OdysseyBrush );
        ...
        for (int32 i = 0; i < SelectedNodes.Num(); ++i)
        {
            ...
            {
                USimpleConstructionScript* SCS = SCS_Node->GetSCS();
                check(SCS != nullptr && OdysseyBrush == SCS->GetBlueprint());
        ...
        FThumbnailRenderingInfo* NewRenderInfo = GUnrealEd->GetThumbnailManager()->GetRenderingInfo( OdysseyBrush );
        if ( RenderInfo && !NewRenderInfo )
        {
            // We removed the last visible primitive component, clear the thumbnail
            const FString BPFullName = FString::Printf(TEXT("%s %s"), *OdysseyBrush->GetClass()->GetName(), *OdysseyBrush->GetPathName());
            UPackage* BPPackage = OdysseyBrush->GetOutermost();
    ...
    // Do this AFTER marking the OdysseyBrush as modified
...
void SSCSEditor::RemoveComponentNode(FSCSEditorTreeNodePtrType InNodePtr)
{
    ...
    if (EditorMode == EComponentEditorMode::OdysseyBrushSCS)
    {
        ...
        if(SCS_Node != NULL)
        {
            ...
            UBlueprint* OdysseyBrush = SCS->GetBlueprint();
            if(OdysseyBrush != nullptr)
            {
                FBlueprintEditorUtils::RemoveVariableNodes(OdysseyBrush, InNodePtr->GetVariableName());
            }
            ...
            if (!InNodePtr->IsDefaultSceneRoot() && SCS_Node->ComponentTemplate != nullptr)
            {
                ...
                if (OdysseyBrush)
                {
                    ...
                    GetDerivedClasses(OdysseyBrush->GeneratedClass, ChildrenOfClass);
...
FSCSEditorTreeNodePtrType SSCSEditor::AddTreeNode(USCS_Node* InSCSNode, FSCSEditorTreeNodePtrType InParentNodePtr, const bool bIsInheritedSCS)
{
    ...
    // Determine whether or not the given node is inherited from a parent OdysseyBrush
...
EVisibility SSCSEditor::GetPromoteToOdysseyBrushButtonVisibility() const
{
...
EVisibility SSCSEditor::GetEditOdysseyBrushButtonVisibility() const
{
    EVisibility ButtonVisibility = EVisibility::Collapsed;
    if (EditorMode == EComponentEditorMode::ActorInstance)
    {
...
FText SSCSEditor::OnGetApplyChangesToOdysseyBrushTooltip() const
{
    int32 NumChangedProperties = 0;

    AActor* Actor = GetActorContext();
    UBlueprint* OdysseyBrush = (Actor != nullptr) ? Cast<UBlueprint>(Actor->GetClass()->ClassGeneratedBy) : nullptr;

    if(Actor != NULL && OdysseyBrush != NULL && Actor->GetClass()->ClassGeneratedBy == OdysseyBrush)
    {
        AActor* OdysseyBrushCDO = Actor->GetClass()->GetDefaultObject<AActor>();
        if(OdysseyBrushCDO != NULL)
        {
            ...
            NumChangedProperties += EditorUtilities::CopyActorProperties(Actor, OdysseyBrushCDO, CopyOptions);
        }
    ...
    {
        return LOCTEXT("DisabledPushToOdysseyBrushDefaults_ToolTip", "Replaces the OdysseyBrush's defaults with any altered property values.");
    }
    else if(NumChangedProperties > 1)
    {
        return FText::Format(LOCTEXT("PushToOdysseyBrushDefaults_ToolTip", "Click to apply {0} changed properties to the OdysseyBrush."), FText::AsNumber(NumChangedProperties));
    }
    else
    {
        return LOCTEXT("PushOneToOdysseyBrushDefaults_ToolTip", "Click to apply 1 changed property to the OdysseyBrush.");
    }
}

FText SSCSEditor::OnGetResetToOdysseyBrushDefaultsTooltip() const
{
    int32 NumChangedProperties = 0;
    ...
    if(Actor != NULL && OdysseyBrush != NULL && Actor->GetClass()->ClassGeneratedBy == OdysseyBrush)
    {
        ...
        {
            const EditorUtilities::ECopyOptions::Type CopyOptions = (EditorUtilities::ECopyOptions::Type)(EditorUtilities::ECopyOptions::PreviewOnly|EditorUtilities::ECopyOptions::OnlyCopyEditOrInterpProperties);
            NumChangedProperties += EditorUtilities::CopyActorProperties(OdysseyBrushCDO, Actor, CopyOptions);
        }
        NumChangedProperties += Actor->GetInstanceComponents().Num();
    }
    ...
    if(NumChangedProperties == 0)
    {
        return LOCTEXT("DisabledReSetBlueprintDefaults_ToolTip", "Resets altered properties back to their OdysseyBrush default values.");
    }
    else if(NumChangedProperties > 1)
    {
        return FText::Format(LOCTEXT("ResetToOdysseyBrushDefaults_ToolTip", "Click to reset {0} changed properties to their OdysseyBrush default values."), FText::AsNumber(NumChangedProperties));
    }
    else
    {
        return LOCTEXT("ResetOneToOdysseyBrushDefaults_ToolTip", "Click to reset 1 changed property to its OdysseyBrush default value.");
    }
}

void SSCSEditor::OnOpenOdysseyBrushEditor(bool bForceCodeEditing) const
{
    if (AActor* ActorInstance = GetActorContext())
    {
        if (UBlueprint* OdysseyBrush = Cast<UBlueprint>(ActorInstance->GetClass()->ClassGeneratedBy))
        {
            if (bForceCodeEditing && (OdysseyBrush->UbergraphPages.Num() > 0))
            {
                FKismetEditorUtilities::BringKismetToFocusAttentionOnObject(OdysseyBrush->GetLastEditedUberGraph());
            }
            else
            {
...
void SSCSEditor::OnApplyChangesToOdysseyBrush() const
{
    int32 NumChangedProperties = 0;
    ...
    if (Actor != NULL && OdysseyBrush != NULL && Actor->GetClass()->ClassGeneratedBy == OdysseyBrush)
    {
        ...
        {
            const FScopedTransaction Transaction(LOCTEXT("PushToOdysseyBrushDefaults_Transaction", "Apply Changes to OdysseyBrush"));
            ...
            // Perform the actual copy
            {
                ...
                if (OdysseyBrushCDO != NULL)
                {
                    ...
                    if (Actor->GetInstanceComponents().Num() > 0)
                    {
                        ...
                        FKismetEditorUtilities::AddComponentsToBlueprint(OdysseyBrush, Actor->GetInstanceComponents());
                        NumChangedProperties += Actor->GetInstanceComponents().Num();
        ...
        if (NumChangedProperties > 0)
        {
            ...
            {
                FFormatNamedArguments Args;
                Args.Add(TEXT("OdysseyBrushName"), FText::FromName(OdysseyBrush->GetFName()));
                Args.Add(TEXT("NumChangedProperties"), NumChangedProperties);
                Args.Add(TEXT("ActorName"), FText::FromString(ActorLabel));
                NotificationInfo.Text = FText::Format(LOCTEXT("PushToOdysseyBrushDefaults_ApplySuccess", "Updated OdysseyBrush {OdysseyBrushName} ({NumChangedProperties} property changes applied from actor {ActorName})."), Args);
            }
            else
            {
                FFormatNamedArguments Args;
                Args.Add(TEXT("OdysseyBrushName"), FText::FromName(OdysseyBrush->GetFName()));
                Args.Add(TEXT("ActorName"), FText::FromString(ActorLabel));
                NotificationInfo.Text = FText::Format(LOCTEXT("PushOneToOdysseyBrushDefaults_ApplySuccess", "Updated OdysseyBrush {OdysseyBrushName} (1 property change applied from actor {ActorName})."), Args);
            }
            CompletionState = SNotificationItem::CS_Success;
        }
        else
        {
            NotificationInfo.Text = LOCTEXT("PushToOdysseyBrushDefaults_ApplyFailed", "No properties were copied");
...
void SSCSEditor::OnResetToOdysseyBrushDefaults() const
{
    int32 NumChangedProperties = 0;
    ...
    if ((Actor != NULL) && (OdysseyBrush != NULL) && (Actor->GetClass()->ClassGeneratedBy == OdysseyBrush))
    {
        const FScopedTransaction Transaction(LOCTEXT("ResetToOdysseyBrushDefaults_Transaction", "Reset to Class Defaults"));
        ...
        if (NumChangedProperties > 0)
        {
            if (NumChangedProperties > 1)
            {
                ...
                NotificationInfo.Text = FText::Format(LOCTEXT("ResetToOdysseyBrushDefaults_ApplySuccess", "Reset {ActorName} ({NumChangedProperties} property changes applied from OdysseyBrush {OdysseyBrushName})."), Args);
            }
            else
            {
                FFormatNamedArguments Args;
                Args.Add(TEXT("OdysseyBrushName"), FText::FromName(OdysseyBrush->GetFName()));
                Args.Add(TEXT("ActorName"), FText::FromString(Actor->GetActorLabel()));
                NotificationInfo.Text = FText::Format(LOCTEXT("ResetOneToOdysseyBrushDefaults_ApplySuccess", "Reset {ActorName} (1 property change applied from OdysseyBrush {OdysseyBrushName})."), Args);
            }
            CompletionState = SNotificationItem::CS_Success;
        }
        else
        {
            NotificationInfo.Text = LOCTEXT("ResetToOdysseyBrushDefaults_Failed", "No properties were reset");
            CompletionState = SNotificationItem::CS_Fail;
        }
...
void SSCSEditor::PromoteToOdysseyBrush() const
{
    bool bHarvest = false;
    FCreateBlueprintFromActorDialog::OpenDialog(bHarvest, GetActorContext());
}

FReply SSCSEditor::OnPromoteToOdysseyBrushClicked()
{
    PromoteToOdysseyBrush();
    return FReply::Handled();
}
