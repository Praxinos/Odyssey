...
#define LOCTEXT_NAMESPACE "SOdysseyBrushRevisionMenu"
...
//------------------------------------------------------------------------------
SOdysseyBrushRevisionMenu::~SOdysseyBrushRevisionMenu()
{
...
//------------------------------------------------------------------------------
void SOdysseyBrushRevisionMenu::Construct(const FArguments& InArgs, UBlueprint const* OdysseyBrush)
{
    ...
    [
        ...
        [
            SNew(SBorder)
            .Visibility(this, &SOdysseyBrushRevisionMenu::GetInProgressVisibility)
            ...
            [
                ...
                [
                    SNew(SButton)
                    .Visibility(this, &SOdysseyBrushRevisionMenu::GetCancelButtonVisibility)
                    .OnClicked(this, &SOdysseyBrushRevisionMenu::OnCancelButtonClicked)
                    .VAlign(VAlign_Center)
    ...
    {
        bool const bIsLevelScriptBlueprint = FBlueprintEditorUtils::IsLevelScriptBlueprint(OdysseyBrush);
        Filename = SourceControlHelpers::PackageFilename(bIsLevelScriptBlueprint ? OdysseyBrush->GetOuter()->GetPathName() : OdysseyBrush->GetPathName());
        ...
        ISourceControlModule::Get().GetProvider().Execute(SourceControlQueryOp.ToSharedRef(), Filename, EConcurrency::Asynchronous, FSourceControlOperationComplete::CreateSP(this, &SOdysseyBrushRevisionMenu::OnSourceControlQueryComplete));
...
//------------------------------------------------------------------------------
EVisibility SOdysseyBrushRevisionMenu::GetInProgressVisibility() const
{
    return (SourceControlQueryState == ESourceControlQueryState::QueryInProgress) ? EVisibility::Visible : EVisibility::Collapsed;
}

//------------------------------------------------------------------------------
EVisibility SOdysseyBrushRevisionMenu::GetCancelButtonVisibility() const
{
    ISourceControlProvider& SourceControlProvider = ISourceControlModule::Get().GetProvider();
...
//------------------------------------------------------------------------------
FReply SOdysseyBrushRevisionMenu::OnCancelButtonClicked() const
{
...
//------------------------------------------------------------------------------
void SOdysseyBrushRevisionMenu::OnSourceControlQueryComplete(const FSourceControlOperationRef& InOperation, ECommandResult::Type InResult)
{
