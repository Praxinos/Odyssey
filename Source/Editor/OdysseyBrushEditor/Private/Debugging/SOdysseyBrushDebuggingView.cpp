...
#define LOCTEXT_NAMESPACE "OdysseyBrushDebuggingView"
...
{
    UBlueprint* ParentOdysseyBrush = NULL;

    if (ParentObject != NULL)
    {
        ParentOdysseyBrush = Cast<UBlueprint>(ParentObject);
        if (ParentOdysseyBrush == NULL)
        {
            ParentOdysseyBrush = Cast<UBlueprint>(ParentObject->GetClass()->ClassGeneratedBy);
        }
    }

    return ParentOdysseyBrush;
}
...
UBlueprintGeneratedClass* FDebugLineItem::GetClassForObject(UObject* ParentObject)
{
    if (ParentObject != NULL)
    {
        if (UBlueprint* OdysseyBrush = Cast<UBlueprint>(ParentObject))
        {
            return Cast<UBlueprintGeneratedClass>(*OdysseyBrush->GeneratedClass);
        }
...
FText FWatchLineItem::GetDisplayName() const
{
    ...
    {
        if (UBlueprint* OdysseyBrush = GetBlueprintForObject(ParentObjectRef.Get()))
        {
            if (UProperty* Property = FKismetDebugUtilities::FindClassPropertyForPin(OdysseyBrush, PinToWatch))
            {
...
FText FWatchLineItem::GetDescription() const
{
    if (UEdGraphPin* PinToWatch = ObjectRef.Get())
    {
        ...
        UBlueprint* ParentOdysseyBrush = GetBlueprintForObject(ParentObjectRef.Get());

        // Find a valid property mapping and display the current value
        UObject* ParentObject = ParentObjectRef.Get();
        if ((ParentOdysseyBrush != ParentObject) && (ParentOdysseyBrush != NULL))
        {
            FString WatchString;
            const FKismetDebugUtilities::EWatchTextResult WatchStatus = FKismetDebugUtilities::GetWatchText(WatchString, ParentOdysseyBrush, ParentObject, PinToWatch);
...
public:
    ...
    virtual void MakeMenu(class FMenuBuilder& MenuBuilder) override
    {
        ...
        if ((Breakpoint != NULL) && (ParentOdysseyBrush != NULL))
        {
            FUIAction ClearThisBreakpoint(
                FExecuteAction::CreateStatic( &FDebuggingActionCallbacks::ClearBreakpoint, Breakpoint, ParentOdysseyBrush ),
                AlwaysAllowExecute
                );
...
FText SKismetDebuggingView::GetTopText() const
{
    ...
    UBlueprint* OdysseyBrushObj = OdysseyBrushToWatchPtr.Get();
    UObject* DebuggedObject = (OdysseyBrushObj != nullptr) ? OdysseyBrushObj->GetObjectBeingDebugged() : nullptr;

    TSet<UObject*> NewRootSet;
    if (bIsDebugging && (OdysseyBrushObj != nullptr) && (DebuggedObject != nullptr))
    {
        return FText::Format(LOCTEXT("ShowDebugForObject", "Showing debug info for {0}"), FText::FromName(DebuggedObject->GetFName()));
    }
    else if (!bIsDebugging && (OdysseyBrushObj != nullptr))
    {
        return LOCTEXT("ShowDebugForOdysseyBrush", "Showing debug info for this blueprint");
    }
    else
    {
...
{
    OdysseyBrushToWatchPtr = InArgs._OdysseyBrushToWatch;
...
void SKismetDebuggingView::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
    ...
    else if (!bIsDebugging && (OdysseyBrushObj != nullptr))
    {
        ...
        NewRootSet.Add(OdysseyBrushObj);
    }
    else
    {
