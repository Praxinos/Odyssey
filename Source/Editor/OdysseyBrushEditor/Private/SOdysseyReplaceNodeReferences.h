...
public:
    ...
    void Construct(const FArguments& InArgs, TSharedPtr<class FOdysseyBrushEditor> InOdysseyBrushEditor);
    ~SReplaceNodeReferences();

    /** Forces a refresh on this widget when things in the OdysseyBrush Editor have changed */
    void Refresh();
...
protected:
    ...
    /** Recursively gathers all available OdysseyBrush Variable references to replace with */
    void GatherAllAvailableOdysseyBrushVariables(UClass* InTargetClass);
...
protected:
    ...
    TArray<FTreeViewItem> OdysseyBrushVariableList;

    /** Target SKEL_ class that is being referenced by this window */
    UClass* TargetClass;

    /** OdysseyBrush editor that owns this window */
