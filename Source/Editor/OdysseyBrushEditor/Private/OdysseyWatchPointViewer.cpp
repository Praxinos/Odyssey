...
#define LOCTEXT_NAMESPACE "OdysseyWatchPointViewer"
...
namespace
{
    struct FWatchRow
    {
        FWatchRow(
            ...
            , OdysseyBrushName(MoveTemp(InOdysseyBrushName))
            ...
            , Type(MoveTemp(InType))
        {
            ...
            OdysseyBrushPackageName = Package ? Package->GetFName() : FName();
        }
            ...
            , Type(MoveTemp(Info.Type))
        {
            ...
            {
                Children.Add(MakeShared<FWatchRow>(InBP, InNode, InPin, InObjectBeingDebugged, OdysseyBrushName, GraphName, NodeName, MoveTemp(ChildInfo)));
            }
        }
        ...
        FText GetTextForEntry() const
        {
            ...
            Args.Add(TEXT("OdysseyBrushName"), OdysseyBrushName);
            ...
            return FText::Format(LOCTEXT("WatchEntry", "{ObjectName}({OdysseyBrushName}) {GraphName} {NodeName} {DisplayName}({Type}): {Value}"), Args);
        }
    ...
    private:
        void SetObjectBeingDebuggedName()
        {
            ...
            {
                ObjectBeingDebuggedName = OdysseyBrushName;
            }
        }
    };
    ...
    void UpdateNonInstancedWatchDisplay()
    {
        ...
        for (TWeakObjectPtr<UBlueprint> OdysseyBrushObj : WatchedBlueprints)
        {
            if (!OdysseyBrushObj.IsValid())
            {
                continue;
            }
            FText OdysseyBrushName = FText::FromString(OdysseyBrushObj->GetName());

            for (const FEdGraphPinReference& PinRef : OdysseyBrushObj->WatchedPins)
            {
                if (UEdGraphPin* Pin = PinRef.Get())
                {
                    ...
                    DebugInfo.Type = UEdGraphSchema_OdysseyBrush::TypeToText(Pin->PinType);
    ...
    void UpdateWatchListFromOdysseyBrushImpl(TWeakObjectPtr<UBlueprint> OdysseyBrushObj, const bool bShouldWatch)
    {
        if (bShouldWatch)
        {
            // make sure the blueprint is in our list
            WatchedBlueprints.AddUnique(OdysseyBrushObj);
        }
        else
        {
            // if this blueprint shouldn't be watched and we aren't watching it already then there is nothing to do
            int32 FoundIdx = WatchedBlueprints.Find(OdysseyBrushObj);
            if (FoundIdx == INDEX_NONE)
            {
                // if we didn't find it, it could be because OdysseyBrushObj is no longer valid
    ...
    void UpdateAllOdysseyBrushWatches()
    {
        for (TWeakObjectPtr<UBlueprint> OdysseyBrush : WatchedBlueprints)
        {
            UpdateWatchListFromOdysseyBrushImpl(OdysseyBrush, true);
        }
    }
};
...
protected:
    ...
    {
        return WatchRow->OdysseyBrushName;
    }
...
{
    friend class FOdysseyBrushEditor;
public:
    ...
    void HandleWatchedPinsChanged(UBlueprint* OdysseyBrushObj);
...
void SWatchViewer::HandleWatchedPinsChanged(UBlueprint* OdysseyBrushObj)
{
    WatchViewer::UpdateWatchListFromOdysseyBrush(OdysseyBrushObj);
}
...
void WatchViewer::UpdateInstancedWatchDisplay()
{
#if DO_BLUEPRINT_GUARD
    {
        ...
        {
            UObject* OdysseyBrushInstance = ScriptFrame ? ScriptFrame->Object : nullptr;
            UClass* Class = OdysseyBrushInstance ? OdysseyBrushInstance->GetClass() : nullptr;
            UBlueprint* OdysseyBrushObj = (Class ? Cast<UBlueprint>(Class->ClassGeneratedBy) : nullptr);
            ...
            if (OdysseyBrushInstance->IsDefaultSubobject())
            {
                continue;
            }

            // Don't show info if this instance is pending kill
            if (OdysseyBrushInstance->IsPendingKill())
            {
                continue;
            }

            // Don't show info if this instance isn't in the current world
            UObject* ObjOuter = OdysseyBrushInstance;
            ...
            for (const FEdGraphPinReference& PinRef : OdysseyBrushObj->WatchedPins)
            {
                ...
                FDebugInfo DebugInfo;
                const FKismetDebugUtilities::EWatchTextResult WatchStatus = FKismetDebugUtilities::GetDebugInfo(DebugInfo, OdysseyBrushObj, OdysseyBrushInstance, Pin);
...
void WatchViewer::RemoveWatchesForOdysseyBrush(TWeakObjectPtr<UBlueprint> OdysseyBrushObj)
{
    if (!ensure(OdysseyBrushObj.IsValid()))
    {
        return;
    }
...
void WatchViewer::RemoveWatchesForAsset(const struct FAssetData& AssetData)
{
    ...
    {
        if (AssetData.PackageName == WatchRow->OdysseyBrushPackageName && FText::FromName(AssetData.AssetName).EqualTo(WatchRow->OdysseyBrushName))
        {
            RemoveWatchesForOdysseyBrush(WatchRow->BP);
            break;
        }
    }
}
...
void WatchViewer::OnRenameAsset(const struct FAssetData& AssetData, const FString& OldAssetName)
{
    ...
    if (OldAssetName.Split(".", &OldPackageName, &OldBPName))
    {
        ...
        for (TSharedRef<FWatchRow> WatchRow : Private_WatchSource)
        {
            if (OldPackageName == WatchRow->OdysseyBrushPackageName.ToString() && FText::FromString(OldBPName).EqualTo(WatchRow->OdysseyBrushName))
            {
                WatchRow->OdysseyBrushName = FText::FromName(AssetData.AssetName);
                bUpdated = true;
            }
        }
...
void WatchViewer::UpdateWatchListFromOdysseyBrush(TWeakObjectPtr<UBlueprint> OdysseyBrushObj)
{
    UpdateWatchListFromOdysseyBrushImpl(OdysseyBrushObj, true);
}

void WatchViewer::ClearWatchListFromOdysseyBrush(TWeakObjectPtr<UBlueprint> OdysseyBrushObj)
{
    UpdateWatchListFromOdysseyBrushImpl(OdysseyBrushObj, false);
}
