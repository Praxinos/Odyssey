...
FSearchResult FFiBSearchInstance::StartSearchQuery(const FString& InSearchString, FImaginaryFiBDataSharedPtr InImaginaryOdysseyBrushRoot)
{
    PendingSearchables.Add(InImaginaryOdysseyBrushRoot);
    DoSearchQuery(InSearchString);

    return GetSearchResults(InImaginaryOdysseyBrushRoot);
}

void FFiBSearchInstance::MakeSearchQuery(const FString& InSearchString, FImaginaryFiBDataSharedPtr InImaginaryOdysseyBrushRoot)
{
    PendingSearchables.Add(InImaginaryOdysseyBrushRoot);
    DoSearchQuery(InSearchString);
}

FSearchResult FFiBSearchInstance::GetSearchResults(FImaginaryFiBDataSharedPtr InImaginaryOdysseyBrushRoot)
{
    FSearchResult CachedSearchResult = FImaginaryFiBData::CreateSearchTree(nullptr, InImaginaryOdysseyBrushRoot, MatchesSearchQuery, MatchingSearchComponents);
...
{
    TSharedPtr< FFindInOdysseyBrushExpressionEvaluator> ExpressionEvaluator(new FFindInOdysseyBrushExpressionEvaluator(ETextFilterExpressionEvaluatorMode::Complex, this));
    ...
    ExpressionEvaluator->AddFunctionTokenCallback( TEXT("OdysseyBrush"), FTokenFunctionHandler::CreateRaw(this, &FFiBSearchInstance::OnFilterFunction, ESearchQueryFilter::OdysseyBrushFilter));
...
bool FFiBSearchInstance::OnFilterFunction(const FTextFilterString& A, ESearchQueryFilter InSearchQueryFilter)
{
    if (CurrentSearchable.IsValid())
    {
        ...
        else if (InSearchQueryFilter == ESearchQueryFilter::OdysseyBrushFilter && CurrentSearchablePinned->IsCompatibleWithFilter(ESearchQueryFilter::OdysseyBrushFilter))
        {
            // We are filtering by OdysseyBrush, since this is a OdysseyBrush just add the CurrentSearchable to the PendingSearchables and do the sub-search on it
...
////////////////////////////////////////
// FFindInOdysseyBrushExpressionEvaluator

bool FFindInOdysseyBrushExpressionEvaluator::EvaluateCompiledExpression(const ExpressionParser::CompileResultType& InCompiledResult, const ITextFilterExpressionContext& InContext, FText* OutErrorText) const
{
...
void FFindInOdysseyBrushExpressionEvaluator::MapBasicJumps()
{
    using namespace TextFilterExpressionParser;
    using TextFilterExpressionParser::FTextToken;
...
void FFindInOdysseyBrushExpressionEvaluator::MapOrBinaryJumps()
{
    using namespace TextFilterExpressionParser;
...
void FFindInOdysseyBrushExpressionEvaluator::MapAndBinaryJumps()
{
    using namespace TextFilterExpressionParser;
...
void FFindInOdysseyBrushExpressionEvaluator::ConstructExpressionParser()
{
    SetupGrammar();
    MapBasicJumps();
    MapOrBinaryJumps();
