...
#define LOCTEXT_NAMESPACE "OdysseyBrushEditorCommands"

/** UI_COMMAND takes long for the compile to optimize */
PRAGMA_DISABLE_OPTIMIZATION
void FOdysseyBrushEditorCommands::RegisterCommands()
{
    // Edit commands
    UI_COMMAND( FindInOdysseyBrush, "Find", "Finds references to functions, events, variables, and pins in the current OdysseyBrush (use Ctrl+Shift+F to search in all Blueprints)", EUserInterfaceActionType::Button, FInputChord(EModifierKey::Control, EKeys::F) );
    UI_COMMAND( ReparentOdysseyBrush, "Reparent OdysseyBrush", "Change the parent of this OdysseyBrush", EUserInterfaceActionType::Button, FInputChord() );
    ...
    UI_COMMAND( FindReferencesFromOdysseyBrush, "List referenced (from blueprint)", "Find all objects that the blueprint references", EUserInterfaceActionType::Button, FInputChord() );
    UI_COMMAND( RepairCorruptedOdysseyBrush, "Repair corrupted blueprint", "Attempts to repair a corrupted blueprint that cannot be saved", EUserInterfaceActionType::Button, FInputChord() );
    ...
    // SCC commands
    UI_COMMAND( BeginOdysseyBrushMerge, "Merge", "Shows the OdysseyBrush merge panel and toolbar, allowing the user to resolve conflicted blueprints", EUserInterfaceActionType::Button, FInputChord() );
}
...
{
    /**
     * Checks if the passed in function is available as an event for the OdysseyBrush
     *
     * @param InOdysseyBrush        The OdysseyBrush to check for validity with
     * @param InFunction        The Function to check for being available as an event
     *
     * @return                    Returns true if the function is available as an event in the OdysseyBrush
     */
    bool IsFunctionAvailableAsEvent(const UBlueprint* InOdysseyBrush, const UFunction* InFunction)
    {
        ...
        FBlueprintEditorUtils::FindImplementedInterfaces(InOdysseyBrush, true, InterfaceClasses);
        InterfaceClasses.Add(InOdysseyBrush->ParentClass);
        ...
        if( InOdysseyBrush->ParentClass->HasMetaData(*ExclusionListKeyName) )
        {
            const FString& ExcludedEventNameString = InOdysseyBrush->ParentClass->GetMetaData(*ExclusionListKeyName);
    ...
    /**
     * Finds the event version of a UFunction for a given OdysseyBrush
     *
     * @param InOdysseyBrush            The OdysseyBrush to find the function within
     * @param InFunction            The Function to find an event version of
     *
     * @return                        Event Function used by the given OdysseyBrush for the given Function
     */
    UFunction* FindEventFunctionForClass(const UBlueprint* InOdysseyBrush, const UFunction* InFunction)
    {
        // Look at all of the OdysseyBrush parent's functions for an event
        for (TFieldIterator<UFunction> FunctionIt(InOdysseyBrush->ParentClass, EFieldIteratorFlags::IncludeSuper); FunctionIt; ++FunctionIt)
        {
            UFunction* Function = *FunctionIt;
            if(Function->GetName() == InFunction->GetName())
            {
...
public:
    ...
    virtual void GetActions(UEdGraph* InDestGraph, FVector2D& InOutDestPosition, TArray<UEdGraphNode*>& OutNodes) override
    {
        ...
        InOutDestPosition.Y = FMath::Max(InOutDestPosition.Y, GraphNode->NodePosY + UEdGraphSchema_OdysseyBrush::EstimateNodeHeight(GraphNode));
...
public:
    ...
    virtual void GetActions(UEdGraph* InDestGraph, FVector2D& InOutDestPosition, TArray<UEdGraphNode*>& OutNodes) override
    {
        const UBlueprint* OdysseyBrush = FBlueprintEditorUtils::FindBlueprintForGraph(InDestGraph);

        const UEdGraphSchema_OdysseyBrush* K2Schema = GetDefault<UEdGraphSchema_OdysseyBrush>();
        if(NodeSpawnInfoHelpers::IsFunctionAvailableAsEvent(OdysseyBrush, FunctionPtr))
        {
            UFunction* FunctionEvent = NodeSpawnInfoHelpers::FindEventFunctionForClass(OdysseyBrush, FunctionPtr);
            if(FunctionEvent)
            {
                IBlueprintNodeBinder::FBindingSet Bindings;
        ...
        else
        {
            ...
            uint32 FunctionTypes = UEdGraphSchema_OdysseyBrush::EFunctionType::FT_Pure | UEdGraphSchema_OdysseyBrush::EFunctionType::FT_Const | UEdGraphSchema_OdysseyBrush::EFunctionType::FT_Protected;
            if(bAllowImpureFuncs)
            {
                FunctionTypes |= UEdGraphSchema_OdysseyBrush::EFunctionType::FT_Imperative;
            }

            if(K2Schema->CanFunctionBeUsedInGraph(OdysseyBrush->GeneratedClass, FunctionPtr, InDestGraph, FunctionTypes, false))
            {
                IBlueprintNodeBinder::FBindingSet Bindings;
...
public:
    ...
    virtual void GetActions(UEdGraph* InDestGraph, FVector2D& InOutDestPosition, TArray<UEdGraphNode*>& OutNodes) override
    {
        ...
        if ((OdysseyBrush != nullptr) && OdysseyBrush->ParentClass->IsChildOf<ALevelScriptActor>() && (SelectedLvlActors->Num() > 0))
        {
...
void FOdysseyBrushSpawnNodeCommands::RegisterCommands()
{
    const FString ConfigSection = TEXT("OdysseyBrushSpawnNodes");
    ...
    for(int32 x = 0; x < NodeSpawns.Num(); ++x)
    {
        ...
        else
        {
            ...
            for (TObjectIterator<UBlueprint> OdysseyBrushIt; OdysseyBrushIt; ++OdysseyBrushIt)
            {
                UBlueprint* MacroBP = *OdysseyBrushIt;
        ...
        if(InfoPtr.IsValid())
        {
            ...
            FText Description = FText::Format( NSLOCTEXT("OdysseyBrushEditor", "NodeSpawnDescription", "Hold down the bound keys and left click in the graph panel to spawn a {0} node."), CommandLabelText );
...
void FOdysseyBrushSpawnNodeCommands::GetGraphActionByChord(FInputChord& InChord, UEdGraph* InDestGraph, FVector2D& InOutDestPosition, TArray<UEdGraphNode*>& OutNodes) const
{
