...
#define LOCTEXT_NAMESPACE "OdysseyBrushPalette"
...
namespace OdysseyBrushPalette
{
    static FString const ConfigSection("OdysseyBrushEditor.Palette");
    ...
    if (VarScope)
    {
        if (UProperty* Property = FindField<UProperty>(VarScope, VarName))
        {
            ...
            if (VarDesc.Len() == 0)
            {
                ...
                {
                    VarDesc = UEdGraphSchema_OdysseyBrush::TypeToText(PinType).ToString();
                }
            }
        }
    }
 ...
 *
 * @param  InOdysseyBrush    The blueprint that the palette is associated.
 ...
 */
static FString GetVarTooltip(UBlueprint* InOdysseyBrush, UClass* VarClass, FName VarName)
{
    ...
    if (VarClass)
    {
        ...
        if (UProperty* Property = FindField<UProperty>(VarClass, VarName))
        {
            ...
            {
                const UBlueprint* SourceOdysseyBrush = SourceClass ? Cast<UBlueprint>(SourceClass->ClassGeneratedBy) : nullptr;
                FBlueprintEditorUtils::GetBlueprintVariableMetaData(SourceOdysseyBrush ? SourceOdysseyBrush : InOdysseyBrush, VarName, nullptr, TEXT("tooltip"), ResultTooltip);
            }
        }
    }
 ...
 * @param  OdysseyBrushIn    The blueprint currently being edited (that the action is for).
 ...
 */
static void GetSubGraphIcon(FEdGraphSchemaAction_K2Graph const* const ActionIn, UBlueprint const* OdysseyBrushIn, FSlateBrush const*& IconOut, FSlateColor& ColorOut, FText& ToolTipOut)
{
    check(OdysseyBrushIn);
    ...
    case EEdGraphSchemaAction_K2Graph::Graph:
        {
            if (ActionIn->EdGraph)
            {
                IconOut = FOdysseyBrushEditor::GetGlyphForGraph(ActionIn->EdGraph);
            }
            else
            {
    ...
    case EEdGraphSchemaAction_K2Graph::Macro:
        {
            ...
            else
            {
                ...
                ;
                if (UFunction* OverrideFunc = FindField<UFunction>(OdysseyBrushIn->ParentClass, ActionIn->FuncName))
                {
    ...
    case EEdGraphSchemaAction_K2Graph::Interface:
        {
            ...
            if (UFunction* OverrideFunc = FindField<UFunction>(OdysseyBrushIn->SkeletonGeneratedClass, ActionIn->FuncName))
            {
                if (UEdGraphSchema_OdysseyBrush::FunctionCanBePlacedAsEvent(OverrideFunc))
                {
    ...
    case EEdGraphSchemaAction_K2Graph::Function:
        {
            ...
            else
            {
                ...
                else if (UFunction* OverrideFunc = FindField<UFunction>(OdysseyBrushIn->ParentClass, ActionIn->FuncName))
                {
 ...
 */
static void GetPaletteItemIcon(TSharedPtr<FEdGraphSchemaAction> ActionIn, UBlueprint const* OdysseyBrushIn, FSlateBrush const*& BrushOut, FSlateColor& ColorOut, FText& ToolTipOut, FString& DocLinkOut, FString& DocExcerptOut, FSlateBrush const*& SecondaryBrushOut, FSlateColor& SecondaryColorOut)
{
    ...
    if (ActionIn->GetTypeId() == FOdysseyBrushActionMenuItem::StaticGetTypeId())
    {
        FOdysseyBrushActionMenuItem* NodeSpawnerAction = (FOdysseyBrushActionMenuItem*)ActionIn.Get();
        BrushOut = NodeSpawnerAction->GetMenuIcon(ColorOut);
    }
    else if (ActionIn->GetTypeId() == FOdysseyBrushDragDropMenuItem::StaticGetTypeId())
    {
        FOdysseyBrushDragDropMenuItem* DragDropAction = (FOdysseyBrushDragDropMenuItem*)ActionIn.Get();
        BrushOut = DragDropAction->GetMenuIcon(ColorOut);
    }
    // for backwards compatibility:
    else if (UK2Node const* const NodeTemplate = FOdysseyBrushActionMenuUtils::ExtractNodeTemplateFromAction(ActionIn))
    {
    ...
    // for MyOdysseyBrush tab specific actions:
    else if (ActionIn->GetTypeId() == FEdGraphSchemaAction_K2Graph::StaticGetTypeId())
    {
        ...
        GetSubGraphIcon(GraphAction, OdysseyBrushIn, BrushOut, ColorOut, ToolTipOut);
    }
    ...
    else if (ActionIn->GetTypeId() == FEdGraphSchemaAction_K2Var::StaticGetTypeId())
    {
        ...
        BrushOut = FOdysseyBrushEditor::GetVarIconAndColor(VarClass, VarAction->GetVariableName(), ColorOut, SecondaryBrushOut, SecondaryColorOut);
    ...
    else if (ActionIn->GetTypeId() == FEdGraphSchemaAction_K2LocalVar::StaticGetTypeId())
    {
        ...
        BrushOut = FOdysseyBrushEditor::GetVarIconAndColor(VarScope, LocalVarAction->GetVariableName(), ColorOut, SecondaryBrushOut, SecondaryColorOut);
...
/*******************************************************************************
* FOdysseyBrushPaletteItemRenameUtils
*******************************************************************************/

/** A set of utilities to aid SOdysseyBrushPaletteItem when the user attempts to rename one. */
struct FOdysseyBrushPaletteItemRenameUtils
{
private:
    ...
    static void CommitNewAssetName(UObject* Object, FOdysseyBrushEditor* OdysseyBrushEditor, const FText& NewText)
    {
        if (Object && OdysseyBrushEditor)
        {
            ...
            TSharedPtr<SMyOdysseyBrush> MyOdysseyBrush = OdysseyBrushEditor->GetMyOdysseyBrushWidget();
            if (MyOdysseyBrush.IsValid())
            {
                MyOdysseyBrush->SelectItemByName(FName(*Object->GetPathName()));
            }
        }
    }
...
public:
     ...
     * @param  ActionPtr            The selected action that the calling palette item represents.
     * @param  OdysseyBrushEditorPtr    A pointer to the blueprint editor that the palette belongs to.
     */
    static void CommitNewEnumName(const FText& NewText, ETextCommit::Type InTextCommit, TWeakPtr<FEdGraphSchemaAction> ActionPtr, TWeakPtr<FOdysseyBrushEditor> OdysseyBrushEditorPtr)
    {
        // Should never make it here with anything but an enum action
        check(ActionPtr.Pin()->GetTypeId() == FEdGraphSchemaAction_K2Enum::StaticGetTypeId());
        ...
        if(EnumAction->Enum->GetName() != NewText.ToString())
        {
            ...
            OdysseyBrushEditorPtr.Pin()->GetMyOdysseyBrushWidget()->SelectItemByName(FName("ConstructionScript"));

            AssetToolsModule.Get().RenameAssetsWithDialog(AssetsAndNames);
        }

        OdysseyBrushEditorPtr.Pin()->GetMyOdysseyBrushWidget()->SelectItemByName(FName(*EnumAction->Enum->GetPathName()));
    }
    ...
    */
     ...
     * @param  OdysseyBrushEditorPtr    A pointer to the blueprint editor that the palette belongs to.
     */
    static void CommitNewStructName(const FText& NewText, ETextCommit::Type InTextCommit, TWeakPtr<FEdGraphSchemaAction> ActionPtr, TWeakPtr<FOdysseyBrushEditor> OdysseyBrushEditorPtr)
    {
        // Should never make it here with anything but a struct action
        check(ActionPtr.Pin()->GetTypeId() == FEdGraphSchemaAction_K2Struct::StaticGetTypeId());
        ...
        CommitNewAssetName(Action ? Action->Struct : nullptr, OdysseyBrushEditorPtr.Pin().Get(), NewText);
    }
...
public:
     ...
     */
    void Construct(const FArguments& InArgs, UProperty* InVariableProperty, UBlueprint* InOdysseyBrush, TWeakPtr<FOdysseyBrushEditor> InOdysseyBrushEditor)
    {
        OdysseyBrushObj = InOdysseyBrush;
        ...
        if (FBlueprintEditorUtils::IsPinTypeValid(InNewPinType))
        {
            if (VariableProperty)
            {
                ...
                if (VarName != NAME_None)
                {
                    ...
                    OdysseyBrushEditorPtr.Pin()->GetMyOdysseyBrushWidget()->GetLastPinTypeUsed() = InNewPinType;

                    if (UFunction* LocalVariableScope = Cast<UFunction>(VariableProperty->GetOuter()))
                    {
                        FBlueprintEditorUtils::ChangeLocalVariableType(OdysseyBrushObj, LocalVariableScope, VarName, InNewPinType);
                    }
                    else
                    {
                        FBlueprintEditorUtils::ChangeMemberVariableType(OdysseyBrushObj, VarName, InNewPinType);
                    }
                }
            }
        }
...
public:
     ...
     */
    void Construct(const FArguments& InArgs, TWeakPtr<FEdGraphSchemaAction> ActionPtrIn, TWeakPtr<FOdysseyBrushEditor> InOdysseyBrushEditor, UBlueprint* InOdysseyBrush)
    {
        ...
        if (PaletteAction->GetTypeId() == FEdGraphSchemaAction_K2Var::StaticGetTypeId())
        {
            ...
            const bool bIsOdysseyBrushVariable = (VarSourceScope == OdysseyBrushObj->SkeletonGeneratedClass);
            ...
            bShouldHaveAVisibilityToggle = bIsOdysseyBrushVariable && (!bIsComponentVar || FBlueprintEditorUtils::IsVariableCreatedByBlueprint(OdysseyBrushObj, VariableObjProp));
        }
...
private:
    ...
    void OnVisibilityToggleFlipped(ECheckBoxState InNewState)
    {
        ...
        if ( PaletteAction->GetTypeId() == FEdGraphSchemaAction_K2Var::StaticGetTypeId() )
        {
            ...
            FBlueprintEditorUtils::SetBlueprintOnlyEditableFlag(OdysseyBrushObj, VarAction->GetVariableName(), !bVariableIsExposed);
        }
    }
    ...
    FLinearColor GetVisibilityToggleColor() const
    {
        ...
        else
        {
            ...
            FString Result;
            FBlueprintEditorUtils::GetBlueprintVariableMetaData(OdysseyBrushObj, VarAction->GetVariableName(), nullptr, TEXT("tooltip"), Result);
    ...
    FText GetVisibilityToggleToolTip() const
    {
        ...
        if ( GetVisibilityToggleState() != ECheckBoxState::Checked )
        {
            ToolTipText = LOCTEXT("VariablePrivacy_not_public_Tooltip", "Variable is not public and will not be editable on an instance of this OdysseyBrush.");
        }
        else
        {
            TSharedPtr<FEdGraphSchemaAction_K2Var> VarAction = StaticCastSharedPtr<FEdGraphSchemaAction_K2Var>(ActionPtr.Pin());
            ...
            if ( !Result.IsEmpty() )
            {
                ToolTipText = LOCTEXT("VariablePrivacy_is_public_Tooltip", "Variable is public and is editable on each instance of this OdysseyBrush.");
            }
            else
            {
...
/*******************************************************************************
* SOdysseyBrushPaletteItem Public Interface
*******************************************************************************/

//------------------------------------------------------------------------------
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void SOdysseyBrushPaletteItem::Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData, TWeakPtr<FOdysseyBrushEditor> InOdysseyBrushEditor)
{
    Construct(InArgs, InCreateData, InOdysseyBrushEditor.Pin()->GetBlueprintObj(), InOdysseyBrushEditor);
}

void SOdysseyBrushPaletteItem::Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData, UBlueprint* InOdysseyBrush)
{
    Construct( InArgs, InCreateData, InOdysseyBrush, TWeakPtr<FOdysseyBrushEditor>() );
}

void SOdysseyBrushPaletteItem::Construct(const FArguments& InArgs, FCreateWidgetForActionData* const InCreateData, UBlueprint* InOdysseyBrush, TWeakPtr<FOdysseyBrushEditor> InOdysseyBrushEditor)
{
    check(InCreateData->Action.IsValid());
    check(InOdysseyBrush);

    OdysseyBrush = InOdysseyBrush;
    ...
    const bool bIsFullyReadOnly = !InOdysseyBrushEditor.IsValid() || InCreateData->bIsReadOnly;

    TWeakPtr<FEdGraphSchemaAction> WeakGraphAction = GraphAction;
    auto IsReadOnlyLambda = [WeakGraphAction, InOdysseyBrushEditor, bIsFullyReadOnly]()
    {
        if(WeakGraphAction.IsValid() && InOdysseyBrushEditor.IsValid())
        {
            return bIsFullyReadOnly; /* || FBlueprintEditorUtils::IsPaletteActionReadOnly(WeakGraphAction.Pin(), InOdysseyBrushEditor.Pin()); */
        }
    ...
    auto IsEditingEnabledLambda = [InOdysseyBrushEditor]()
    {
        if(InOdysseyBrushEditor.IsValid())
        {
            return InOdysseyBrushEditor.Pin()->InEditingMode();
        }
    ...
    GetPaletteItemIcon(GraphAction, OdysseyBrush, IconBrush, IconColor, IconToolTip, IconDocLink, IconDocExcerpt, SecondaryBrush, SecondaryIconColor);
    ...
    if (GraphAction->GetTypeId() == FEdGraphSchemaAction_K2Var::StaticGetTypeId() || GraphAction->GetTypeId() == FEdGraphSchemaAction_K2LocalVar::StaticGetTypeId())
    {
        ...
        // If the variable is not a local variable or created by the current OdysseyBrush, do not use the PinTypeSelector
        if (VariableProp)
        {
            if (FBlueprintEditorUtils::IsVariableCreatedByBlueprint(OdysseyBrush, VariableProp) || Cast<UFunction>(VariableProp->GetOuter()))
            {
                const UEdGraphSchema_OdysseyBrush* Schema = GetDefault<UEdGraphSchema_OdysseyBrush>();
                IconWidget = SNew(SPinTypeSelectorHelper, VariableProp, OdysseyBrush, OdysseyBrushEditorPtr)
    ...
    [
        ...
        +SHorizontalBox::Slot()
            ...
            .VAlign(VAlign_Center)
        [
            SNew(SPaletteItemVisibilityToggle, ActionPtr, InOdysseyBrushEditor, InOdysseyBrush)
            .IsEnabled(bIsEditingEnabled)
        ]
    ];
}
...
void SOdysseyBrushPaletteItem::OnDragEnter(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
    if (OdysseyBrushEditorPtr.IsValid())
    {
...
/*******************************************************************************
* SOdysseyBrushPaletteItem Private Methods
*******************************************************************************/

//------------------------------------------------------------------------------
TSharedRef<SWidget> SOdysseyBrushPaletteItem::CreateTextSlotWidget(const FSlateFontInfo& NameFont, FCreateWidgetForActionData* const InCreateData, TAttribute<bool> bIsReadOnlyIn)
{
    ...
    {
        OnVerifyTextChanged.BindStatic(&FOdysseyBrushPaletteItemRenameUtils::VerifyNewEnumName, ActionPtr);
        OnTextCommitted.BindStatic(&FOdysseyBrushPaletteItemRenameUtils::CommitNewEnumName, ActionPtr, OdysseyBrushEditorPtr);
    }
    else if (ActionTypeId == FEdGraphSchemaAction_K2Struct::StaticGetTypeId())
    {
        OnVerifyTextChanged.BindStatic(&FOdysseyBrushPaletteItemRenameUtils::VerifyNewStructName, ActionPtr );
        OnTextCommitted.BindStatic(&FOdysseyBrushPaletteItemRenameUtils::CommitNewStructName, ActionPtr, OdysseyBrushEditorPtr);
    }
    else if (ActionTypeId == FEdGraphSchemaAction_K2Event::StaticGetTypeId())
    {
        OnVerifyTextChanged.BindStatic(&FOdysseyBrushPaletteItemRenameUtils::VerifyNewEventName, ActionPtr);
        OnTextCommitted.BindStatic(&FOdysseyBrushPaletteItemRenameUtils::CommitNewEventName, ActionPtr);
    }
    else if (ActionTypeId == FEdGraphSchemaAction_K2TargetNode::StaticGetTypeId())
    {
        OnVerifyTextChanged.BindStatic(&FOdysseyBrushPaletteItemRenameUtils::VerifyNewTargetNodeName, ActionPtr);
        OnTextCommitted.BindStatic(&FOdysseyBrushPaletteItemRenameUtils::CommitNewTargetNodeName, ActionPtr);
    }
    else
    {
        // default to our own rename methods
        OnVerifyTextChanged.BindSP(this, &SOdysseyBrushPaletteItem::OnNameTextVerifyChanged);
        OnTextCommitted.BindSP(this, &SOdysseyBrushPaletteItem::OnNameTextCommitted);
    }
    ...
    SAssignNew(DisplayWidget, SOverlay)
        ...
        [
            SAssignNew(EditableTextElement, SInlineEditableTextBlock)
                .Text(this, &SOdysseyBrushPaletteItem::GetDisplayText)
                .Font(NameFont)
...
//------------------------------------------------------------------------------
FText SOdysseyBrushPaletteItem::GetDisplayText() const
{
    const UEdGraphSchema_OdysseyBrush* K2Schema = GetDefault<UEdGraphSchema_OdysseyBrush>();
...
//------------------------------------------------------------------------------
bool SOdysseyBrushPaletteItem::OnNameTextVerifyChanged(const FText& InNewText, FText& OutErrorMessage)
{
    ...
    TSharedPtr<INameValidatorInterface> NameValidator = MakeShareable(new FKismetNameValidator(OdysseyBrushObj, OriginalName, ValidationScope));
...
//------------------------------------------------------------------------------
void SOdysseyBrushPaletteItem::OnNameTextCommitted(const FText& NewText, ETextCommit::Type InTextCommit)
{
    ...
    else if(ActionPtr.Pin()->GetTypeId() == FEdGraphSchemaAction_K2Delegate::StaticGetTypeId())
    {
        ...
        if (Graph && (Graph->bAllowDeletion || Graph->bAllowRenaming))
        {
            ...
            if (ExistingGraph == nullptr || ExistingGraph == Graph)
            {
                ...
                UBlueprint* OdysseyBrushObj = OdysseyBrushEditorPtr.Pin()->GetBlueprintObj();
                FBlueprintEditorUtils::RenameMemberVariable(OdysseyBrushObj, OldName, NewName);
            }
        }
    }
    else if (ActionPtr.Pin()->GetTypeId() == FEdGraphSchemaAction_K2Var::StaticGetTypeId())
    {
        ...
        OdysseyBrushEditorPtr.Pin()->GetBlueprintObj()->Modify();
        ...
        if (bIsTimeline)
        {
            FBlueprintEditorUtils::RenameTimeline(OdysseyBrushEditorPtr.Pin()->GetBlueprintObj(), VarAction->GetVariableName(), NewName);
        }
        else
        {
            FBlueprintEditorUtils::RenameMemberVariable(OdysseyBrushEditorPtr.Pin()->GetBlueprintObj(), VarAction->GetVariableName(), NewName);
        }
    }
    else if (ActionPtr.Pin()->GetTypeId() == FEdGraphSchemaAction_K2LocalVar::StaticGetTypeId())
    {
        ...
        FBlueprintEditorUtils::RenameLocalVariable(OdysseyBrushEditorPtr.Pin()->GetBlueprintObj(), LocalVarAction->GetVariableScope(), LocalVarAction->GetVariableName(), NewName);
    }
    OdysseyBrushEditorPtr.Pin()->GetMyOdysseyBrushWidget()->SelectItemByName(NewName, ESelectInfo::OnMouseClick);
}

//------------------------------------------------------------------------------
FText SOdysseyBrushPaletteItem::GetToolTipText() const
{
    TSharedPtr<FEdGraphSchemaAction> PaletteAction = ActionPtr.Pin();
    ...
    if (PaletteAction.IsValid())
    {
        ...
        else if (UK2Node const* const NodeTemplate = FOdysseyBrushActionMenuUtils::ExtractNodeTemplateFromAction(PaletteAction))
        {
            // If the node wants to create tooltip text, use that instead, because its probably more detailed
            FText NodeToolTipText = NodeTemplate->GetTooltipText();
            if (!NodeToolTipText.IsEmpty())
            ...
            if (UK2Node_CallFunction const* CallFuncNode = Cast<UK2Node_CallFunction const>(NodeTemplate))
            {
                ...
                {
                    UBlueprint* OdysseyBrushObj = UBlueprint::GetBlueprintFromClass(ParentClass);
                    if (OdysseyBrushObj == nullptr)
                    {
                        ClassDisplayName = ParentClass->GetDisplayNameText();
                    }
                    else if (!OdysseyBrushObj->HasAnyFlags(RF_Transient))
                    {
                        ClassDisplayName = FText::FromName(OdysseyBrushObj->GetFName());
                    }
                }
            }
        }
        ...
        else if (PaletteAction->GetTypeId() == FEdGraphSchemaAction_K2Var::StaticGetTypeId())
        {
            ...
            {
                UBlueprint* OdysseyBrushObj = UBlueprint::GetBlueprintFromClass(VarClass);
                ClassDisplayName = (OdysseyBrushObj ? FText::FromName(OdysseyBrushObj->GetFName()) : VarClass->GetDisplayNameText());
            }
            else
            {
                FString Result = GetVarTooltip(OdysseyBrush, VarClass, VarAction->GetVariableName());
        ...
        else if (PaletteAction->GetTypeId() == FEdGraphSchemaAction_K2LocalVar::StaticGetTypeId())
        {
            ...
            if(LocalVarAction->GetVariableScope())
            {
                ...
                {
                    FString Result;
                    FBlueprintEditorUtils::GetBlueprintVariableMetaData(OdysseyBrush, LocalVarAction->GetVariableName(), LocalVarAction->GetVariableScope(), TEXT("tooltip"), Result);
                    // Only use the variable tooltip if it has been filled out.
        ...
        else if (PaletteAction->GetTypeId() == FEdGraphSchemaAction_K2Delegate::StaticGetTypeId())
        {
            ...
            FString Result = GetVarTooltip(OdysseyBrush, DelegateAction->GetDelegateClass(), DelegateAction->GetDelegateName());
    ...
    {
        ToolTipText = FText::Format(LOCTEXT("OdysseyBrushItemClassTooltip", "{0}\nClass: {1}"), ToolTipText, ClassDisplayName);
    }

    return ToolTipText;
}

TSharedPtr<SToolTip> SOdysseyBrushPaletteItem::ConstructToolTipWidget() const
{
    TSharedPtr<FEdGraphSchemaAction> PaletteAction = ActionPtr.Pin();
    UEdGraphNode const* const NodeTemplate = FOdysseyBrushActionMenuUtils::ExtractNodeTemplateFromAction(PaletteAction);

    FOdysseyBrushActionMenuItem::FDocExcerptRef DocExcerptRef;
    ...
    if (PaletteAction.IsValid())
    {
        if (NodeTemplate != nullptr)
        {
            ...
            // sometimes, with FOdysseyBrushActionMenuItem's, the NodeTemplate
            ...
            // FOdysseyBrushActionMenuItem's may have a more specific documentation
            // link of its own (most of the time, it will reflect the NodeTemplate's)
            if ( !DocExcerptRef.IsValid() && (PaletteAction->GetTypeId() == FOdysseyBrushActionMenuItem::StaticGetTypeId()) )
            {
                FOdysseyBrushActionMenuItem* NodeSpawnerAction = (FOdysseyBrushActionMenuItem*)PaletteAction.Get();
        ...
        else if (PaletteAction->GetTypeId() == FEdGraphSchemaAction_K2Var::StaticGetTypeId())
        {
            ...
            if (!bShowClassInTooltip || VarClass == nullptr)
            {
                // Don't show big tooltip if we are showing class as well (means we are not in MyOdysseyBrush)
                DocExcerptRef.DocLink = TEXT("Shared/Editors/OdysseyBrushEditor/GraphTypes");
    ...
    TextAttribute.Bind(this, &SOdysseyBrushPaletteItem::GetToolTipText);
...
//------------------------------------------------------------------------------
void SOdysseyBrushPalette::Construct(const FArguments& InArgs, TWeakPtr<FOdysseyBrushEditor> InOdysseyBrushEditor)
{
    ...
    GConfig->GetFloat(*OdysseyBrushPalette::ConfigSection, *OdysseyBrushPalette::FavoritesHeightConfigKey, FavoritesHeightRatio, GEditorPerProjectIni);
    float LibraryHeightRatio = 1.f - FavoritesHeightRatio;
    GConfig->GetFloat(*OdysseyBrushPalette::ConfigSection, *OdysseyBrushPalette::LibraryHeightConfigKey, LibraryHeightRatio, GEditorPerProjectIni);

    bool bUseLegacyLayout = false;
    GConfig->GetBool(*OdysseyBrushPalette::ConfigSection, TEXT("bUseLegacyLayout"), bUseLegacyLayout, GEditorIni);

    if (bUseLegacyLayout)
    {
        this->ChildSlot
        [
            SAssignNew(LibraryWrapper, SOdysseyBrushLibraryPalette, InOdysseyBrushEditor)
    ...
    else
    {
        ...
        [
            SAssignNew(PaletteSplitter, SSplitter)
                ...
                .OnSplitterFinishedResizing(this, &SOdysseyBrushPalette::OnSplitterResized)
                .AddMetaData<FTagMetaData>(FTagMetaData(TEXT("FullOdysseyBrushPalette")))

            + SSplitter::Slot()
            .Value(FavoritesHeightRatio)
            [
                SNew(SOdysseyBrushFavoritesPalette, InOdysseyBrushEditor)
            ...
            [
                SNew(SOdysseyBrushLibraryPalette, InOdysseyBrushEditor)
                .AddMetaData<FTagMetaData>(FTagMetaData(TEXT("OdysseyBrushPaletteLibrary")))
            ]
        ];
    }
}
...
//------------------------------------------------------------------------------
void SOdysseyBrushPalette::OnSplitterResized() const
{
    ...
    for (int32 SlotIndex = 0; SlotIndex < SplitterChildren->Num(); ++SlotIndex)
    {
        ...
        {
            GConfig->SetFloat(*OdysseyBrushPalette::ConfigSection, *OdysseyBrushPalette::FavoritesHeightConfigKey, SplitterSlot.SizeValue.Get(), GEditorPerProjectIni);
        }
        else if (SplitterSlot.GetWidget() == LibraryWrapper)
        {
            GConfig->SetFloat(*OdysseyBrushPalette::ConfigSection, *OdysseyBrushPalette::LibraryHeightConfigKey, SplitterSlot.SizeValue.Get(), GEditorPerProjectIni);
        }
