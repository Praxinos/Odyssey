...
#define LOCTEXT_NAMESPACE "OdysseyBrushDragDropMenuItem"
DEFINE_LOG_CATEGORY_STATIC(LogBlueprintDragDropMenuItem, Log, All);

//------------------------------------------------------------------------------
FOdysseyBrushDragDropMenuItem::FOdysseyBrushDragDropMenuItem(FBlueprintActionContext const& Context, UBlueprintNodeSpawner const* SampleAction, int32 MenuGrouping, FText InNodeCategory, FText InMenuDesc, FText InToolTip)
...
//------------------------------------------------------------------------------
UEdGraphNode* FOdysseyBrushDragDropMenuItem::PerformAction(UEdGraph* ParentGraph, UEdGraphPin* FromPin, FVector2D const Location, bool bSelectNewNode/* = true*/)
{
    // we shouldn't get here (this should just be used for drag/drop ops), but just in case we do...
    FOdysseyBrushActionMenuItem OdysseyBrushActionItem(GetSampleAction());
    return OdysseyBrushActionItem.PerformAction(ParentGraph, FromPin, Location, bSelectNewNode);
}

//------------------------------------------------------------------------------
UEdGraphNode* FOdysseyBrushDragDropMenuItem::PerformAction(UEdGraph* ParentGraph, TArray<UEdGraphPin*>& FromPins, FVector2D const Location, bool bSelectNewNode/* = true*/)
{
...
//------------------------------------------------------------------------------
void FOdysseyBrushDragDropMenuItem::AddReferencedObjects(FReferenceCollector& Collector)
{
...
//------------------------------------------------------------------------------
void FOdysseyBrushDragDropMenuItem::AppendAction(UBlueprintNodeSpawner const* Action)
{
    ActionSet.Add(Action);
}

//------------------------------------------------------------------------------
FSlateBrush const* FOdysseyBrushDragDropMenuItem::GetMenuIcon(FSlateColor& ColorOut)
{
    ...
    else if (UBlueprintVariableNodeSpawner const* VariableSpawner = Cast<UBlueprintVariableNodeSpawner const>(SampleAction))
    {
        if (UProperty const* Property = VariableSpawner->GetVarProperty())
        {
            ...
            IconBrush = FOdysseyBrushEditor::GetVarIconAndColor(PropertyOwner, Property->GetFName(), ColorOut, Discarded, DiscardedColor);
        }
    }
    return IconBrush;
}
...
//------------------------------------------------------------------------------
UBlueprintNodeSpawner const* FOdysseyBrushDragDropMenuItem::GetSampleAction() const
{
...
//------------------------------------------------------------------------------
TSet<UBlueprintNodeSpawner const*> const& FOdysseyBrushDragDropMenuItem::GetActionSet() const
{
    return ActionSet;
}

//------------------------------------------------------------------------------
TSharedPtr<FDragDropOperation> FOdysseyBrushDragDropMenuItem::OnDragged(FNodeCreationAnalytic AnalyticsDelegate) const
{
