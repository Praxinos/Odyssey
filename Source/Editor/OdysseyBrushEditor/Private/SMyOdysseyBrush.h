...
class FMyOdysseyBrushCommands : public TCommands<FMyOdysseyBrushCommands>
{
public:
    /** Constructor */
    FMyOdysseyBrushCommands()
        : TCommands<FMyOdysseyBrushCommands>(TEXT("MyOdysseyBrush"), NSLOCTEXT("Contexts", "My OdysseyBrush", "My OdysseyBrush"), NAME_None, FEditorStyle::GetStyleSetName())
    {
    }
...
class SMyOdysseyBrush : public SCompoundWidget
{
public:
    SLATE_BEGIN_ARGS( SMyOdysseyBrush ) {}
    SLATE_END_ARGS()

    void Construct(const FArguments& InArgs, TWeakPtr<FOdysseyBrushEditor> InOdysseyBrushEditor, const UBlueprint* InOdysseyBrush = nullptr);
    ~SMyOdysseyBrush();
    ...
    UBlueprint* GetBlueprintObj() const {return OdysseyBrush;}
    ...
    TWeakPtr<FOdysseyBrushEditor> GetBlueprintEditor() {return OdysseyBrushEditorPtr;}
...
private:
    ...
    static void OnActionSelectedHelper(TSharedPtr<FEdGraphSchemaAction> InAction, TWeakPtr< FOdysseyBrushEditor > InOdysseyBrushEditor, UBlueprint* OdysseyBrush, TSharedRef<SKismetInspector> Inspector);
    ...
    bool IsActionReadOnly(TSharedPtr<FEdGraphSchemaAction> ActionIn ) const;
    ...
    /** Helper function to delete a graph in the MyOdysseyBrush window */
    void OnDeleteGraph(UEdGraph* InGraph, EEdGraphSchemaAction_K2Graph::Type);

    /** Helper function to delete a delegate in the MyOdysseyBrush window */
    void OnDeleteDelegate(FEdGraphSchemaAction_K2Delegate* InDelegateAction);

    UEdGraph* GetFocusedGraph() const;

    /** Delegate to hook us into non-structural OdysseyBrush object post-change events */
...
private:
    ...
    TArray<TWeakObjectPtr<UUserDefinedEnum>> EnumsAddedToOdysseyBrush;

    /** The filter box that handles filtering for both graph action menus. */
    TSharedPtr< SSearchBox > FilterBox;

    /** Enums created from 'blueprint' level */
    TArray<TWeakObjectPtr<UUserDefinedStruct>> StructsAddedToOdysseyBrush;
