...
#define LOCTEXT_NAMESPACE "OdysseyReplaceNodeReferences"
...
void SReplaceNodeReferences::Construct(const FArguments& InArgs, TSharedPtr<class FOdysseyBrushEditor> InOdysseyBrushEditor)
{
            ...
            +SVerticalBox::Slot()
                [
                    SNew(SBox)
                        ...
                        [
                            SAssignNew(FindInBlueprints, SFindInBlueprints, InOdysseyBrushEditor)
...
void SReplaceNodeReferences::Refresh()
{
    ...
    OdysseyBrushVariableList.Empty();
    TargetClass = OdysseyBrushEditor.Pin()->GetBlueprintObj()->SkeletonGeneratedClass;
    GatherAllAvailableOdysseyBrushVariables(TargetClass);
}
...
TSharedRef<SWidget>    SReplaceNodeReferences::GetMenuContent()
{
    return SAssignNew(AvailableTargetReferencesTreeView, SReplaceReferencesTreeViewType)
        .ItemHeight(24)
        .TreeItemsSource( &OdysseyBrushVariableList )
...
void SReplaceNodeReferences::GatherAllAvailableOdysseyBrushVariables(UClass* InTargetClass)
{
    if (InTargetClass == nullptr)
    {
        return;
    }
    GatherAllAvailableOdysseyBrushVariables(InTargetClass->GetSuperClass());
    ...
    TSharedPtr< FTargetCategoryReplaceReferences > OdysseyBrushCategory = MakeShareable(new FTargetCategoryReplaceReferences(FText::FromString(PathObject->GetPathName())));
    for (TFieldIterator<UProperty> PropertyIt(InTargetClass, EFieldIteratorFlags::ExcludeSuper); PropertyIt; ++PropertyIt)
    {
        ...
        UEdGraphSchema_OdysseyBrush const* K2Schema = GetDefault<UEdGraphSchema_OdysseyBrush>();
        if (CategoryName.EqualTo(FText::FromString(PathObject->GetName())) || CategoryName.EqualTo(UEdGraphSchema_OdysseyBrush::VR_DefaultCategory))
        {
        ...
        if (bShouldShowAsVar)
        {
            ...
            TSharedPtr< FTargetCategoryReplaceReferences > CategoryReference = OdysseyBrushCategory;
            ...
            if (VariableItem->PinType == SourcePinType)
            {
                ...
                // If this is the first Child, add the category to the main OdysseyBrush category
                if (CategoryReference != OdysseyBrushCategory && CategoryReference->Children.Num() == 1)
                {
                    OdysseyBrushCategory->Children.Add(CategoryReference);
                }
            }
        }
    }

    if (OdysseyBrushCategory->Children.Num())
    {
        OdysseyBrushVariableList.Add(OdysseyBrushCategory);
        // Sort markers
        ...
        OdysseyBrushCategory->Children.Sort(FCompareCategoryTitles());
    }
}
...
void SReplaceNodeReferences::FindAllReplacementsComplete(TArray<FImaginaryFiBDataSharedPtr>& InRawDataList)
{
    if (SelectedTargetReferenceItem.IsValid())
    {
        ...
        if (SelectedTargetReferenceItem->GetMemberReference(VariableReference))
        {
            ...
            OdysseyBrushEditor.Pin()->GetBlueprintObj()->Modify();
            ...
            for (FImaginaryFiBDataSharedPtr ImaginaryData : InRawDataList)
            {
                ...
                if (ensure(VariableNode))
                {
                    ...
                    {
                        UBlueprint* OdysseyBrush = OdysseyBrushEditor.Pin()->GetBlueprintObj();
                        VariableNode->VariableReference.SetFromField<UProperty>(VariableReference.ResolveMember<UProperty>(OdysseyBrush), OdysseyBrush->GeneratedClass);
                    }
                    VariableNode->ReconstructNode();
                }
            }

            for (UBlueprint* OdysseyBrush : BlueprintsModified)
            {
                FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(OdysseyBrush);
                FFindInOdysseyBrushSearchManager::Get().AddOrUpdateOdysseyBrushSearchMetadata(OdysseyBrush);
            }
