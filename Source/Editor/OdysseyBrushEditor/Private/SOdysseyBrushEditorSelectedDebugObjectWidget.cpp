...
#define LOCTEXT_NAMESPACE "OdysseyBrushEditorSelectedDebugObjectWidget"
...
//////////////////////////////////////////////////////////////////////////
// SOdysseyBrushEditorSelectedDebugObjectWidget

void SOdysseyBrushEditorSelectedDebugObjectWidget::Construct(const FArguments& InArgs, TSharedPtr<FOdysseyBrushEditor> InOdysseyBrushEditor)
{
    OdysseyBrushEditor = InOdysseyBrushEditor;

    const TSharedRef<SWidget> BrowseButton = PropertyCustomizationHelpers::MakeBrowseButton(FSimpleDelegate::CreateSP(this, &SOdysseyBrushEditorSelectedDebugObjectWidget::SelectedDebugObject_OnClicked));
    BrowseButton->SetVisibility(TAttribute<EVisibility>::Create(TAttribute<EVisibility>::FGetter::CreateSP(this, &SOdysseyBrushEditorSelectedDebugObjectWidget::IsSelectDebugObjectButtonVisible)));
    ...
    DebugWorldsComboBox = SNew(STextComboBox)
        ...
        LOCTEXT("OdysseyBrushDebugWorldTooltip", "Select a world to debug"),
        nullptr,
        TEXT("Shared/Editors/OdysseyBrushEditor/OdysseyBrushDebugger"),
        TEXT("DebugWorld")))
        .OptionsSource(&DebugWorldNames)
        .InitiallySelectedItem(GetDebugWorldName())
        .Visibility(this, &SOdysseyBrushEditorSelectedDebugObjectWidget::IsDebugWorldComboVisible)
        .OnComboBoxOpening(this, &SOdysseyBrushEditorSelectedDebugObjectWidget::GenerateDebugWorldNames, true)
        .OnSelectionChanged(this, &SOdysseyBrushEditorSelectedDebugObjectWidget::DebugWorldSelectionChanged);

    DebugObjectsComboBox = SNew(STextComboBox)
        .ButtonStyle(FEditorStyle::Get(), "FlatButton.Light")
        .ToolTip(IDocumentation::Get()->CreateToolTip(
        LOCTEXT("OdysseyBrushDebugObjectTooltip", "Select an object to debug"),
        nullptr,
        TEXT("Shared/Editors/OdysseyBrushEditor/OdysseyBrushDebugger"),
        TEXT("DebugObject")))
        .OptionsSource(&DebugObjectNames)
        .InitiallySelectedItem(GetDebugObjectName())
        .OnComboBoxOpening(this, &SOdysseyBrushEditorSelectedDebugObjectWidget::GenerateDebugObjectNames, true)
        .OnSelectionChanged(this, &SOdysseyBrushEditorSelectedDebugObjectWidget::DebugObjectSelectionChanged)
...
void SOdysseyBrushEditorSelectedDebugObjectWidget::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
    if (GetBlueprintObj())
    {
...
const FString& SOdysseyBrushEditorSelectedDebugObjectWidget::GetNoDebugString() const
{
    return NSLOCTEXT("OdysseyBrushEditor", "DebugObjectNothingSelected", "No debug object selected").ToString();
}

const FString& SOdysseyBrushEditorSelectedDebugObjectWidget::GetDebugAllWorldsString() const
{
    return NSLOCTEXT("OdysseyBrushEditor", "DebugWorldNothingSelected", "All Worlds").ToString();
}

void SOdysseyBrushEditorSelectedDebugObjectWidget::OnRefresh()
{
    if (GetBlueprintObj())
    {
        GenerateDebugWorldNames(false);
...
void SOdysseyBrushEditorSelectedDebugObjectWidget::GenerateDebugWorldNames(bool bRestoreSelection)
{
    ...
    UWorld* PreviewWorld = OdysseyBrushEditor.Pin()->GetPreviewScene()->GetWorld();
    ...
    for (TObjectIterator<UWorld> It; It; ++It)
    {
        ...
        {
        case NM_Standalone:
            WorldName = NSLOCTEXT("OdysseyBrushEditor", "DebugWorldStandalone", "Standalone").ToString();
            break;

        case NM_ListenServer:
            WorldName = NSLOCTEXT("OdysseyBrushEditor", "DebugWorldListenServer", "Listen Server").ToString();
            break;

        case NM_DedicatedServer:
            WorldName = NSLOCTEXT("OdysseyBrushEditor", "DebugWorldDedicatedServer", "Dedicated Server").ToString();
            break;

        case NM_Client:
            if (FWorldContext* PieContext = GEngine->GetWorldContextFromWorld(TestWorld))
            {
                WorldName = FString::Printf(TEXT("%s %d"), *NSLOCTEXT("OdysseyBrushEditor", "DebugWorldClient", "Client").ToString(), PieContext->PIEInstance - 1);
            }
            break;
        };
...
void SOdysseyBrushEditorSelectedDebugObjectWidget::GenerateDebugObjectNames(bool bRestoreSelection)
{
    TSharedPtr<FString> OldSelection;
    ...
    TArray<FOdysseyCustomDebugObject> CustomDebugObjects;
    OdysseyBrushEditor.Pin()->GetCustomDebugObjects(/*inout*/ CustomDebugObjects);

    for (const FOdysseyCustomDebugObject& Entry : CustomDebugObjects)
    {
    ...
    UClass* OdysseyBrushClass = GetBlueprintObj()->GeneratedClass;

    if (bModifiedIterator && OdysseyBrushClass)
    {
        // Experimental new path for debug object discovery
        TArray<UObject*> OdysseyBrushInstances;
        GetObjectsOfClass(OdysseyBrushClass, OdysseyBrushInstances, true);

        for (auto It = OdysseyBrushInstances.CreateIterator(); It; ++It)
        {
            UObject* TestObject = *It;
            // Skip OdysseyBrush preview objects (don't allow them to be selected for debugging)
    ...
    for (TObjectIterator<UObject> It; It; ++It)
    {
        ...
        const bool bGeneratedByAnyOdysseyBrush = TestObject->GetClass()->ClassGeneratedBy != nullptr;
        const bool bGeneratedByThisOdysseyBrush = bGeneratedByAnyOdysseyBrush && GetBlueprintObj()->GeneratedClass && TestObject->IsA(GetBlueprintObj()->GeneratedClass);

        if (bPassesFlags && bGeneratedByThisOdysseyBrush)
        {
            UObject *ObjOuter = TestObject;
            UWorld *ObjWorld = nullptr;
            do        // Run through at least once in case the TestObject is a UGameInstance
...
TSharedPtr<FString> SOdysseyBrushEditorSelectedDebugObjectWidget::GetDebugObjectName() const
{
...
TSharedPtr<FString> SOdysseyBrushEditorSelectedDebugObjectWidget::GetDebugWorldName() const
{
    check(GetBlueprintObj());
...
void SOdysseyBrushEditorSelectedDebugObjectWidget::DebugWorldSelectionChanged(TSharedPtr<FString> NewSelection, ESelectInfo::Type SelectInfo)
{
    if (!NewSelection.IsValid())
    {
        return;
...
void SOdysseyBrushEditorSelectedDebugObjectWidget::DebugObjectSelectionChanged(TSharedPtr<FString> NewSelection, ESelectInfo::Type SelectInfo)
{
...
EVisibility SOdysseyBrushEditorSelectedDebugObjectWidget::IsSelectDebugObjectButtonVisible() const
{
    check(GetBlueprintObj());
    if (UObject* DebugObj = GetBlueprintObj()->GetObjectBeingDebugged())
    {
...
void SOdysseyBrushEditorSelectedDebugObjectWidget::SelectedDebugObject_OnClicked()
{
    if (UObject* DebugObj = GetBlueprintObj()->GetObjectBeingDebugged())
    {
        if (AActor* Actor = Cast<AActor>(DebugObj))
...
EVisibility SOdysseyBrushEditorSelectedDebugObjectWidget::IsDebugWorldComboVisible() const
{
...
void SOdysseyBrushEditorSelectedDebugObjectWidget::AddDebugObject(UObject* TestObject)
{
    FString Label;
...
void SOdysseyBrushEditorSelectedDebugObjectWidget::AddDebugObjectWithName(UObject* TestObject, const FString& TestObjectName)
{
