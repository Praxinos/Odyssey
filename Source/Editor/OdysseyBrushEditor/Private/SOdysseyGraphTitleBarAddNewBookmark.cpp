...
#define LOCTEXT_NAMESPACE "OdysseyGraphTitleBarAddNewBookmark"
...
FText SGraphTitleBarAddNewBookmark::GetDefaultNameText() const
{
    ...
    TSharedPtr<FOdysseyBrushEditor> EditorContext = EditorContextPtr.Pin();
    if (EditorContext.IsValid())
    {
        UBlueprint* OdysseyBrush = EditorContext->GetBlueprintObj();
        check(OdysseyBrush);
        ...
        if (CurrentViewBookmarkId.IsValid())
        {
            ...
            const FBPEditorBookmarkNode* BookmarkNode = FindCurrentBookmarkNodeLambda(OdysseyBrush->BookmarkNodes);
        ...
        else
        {
            ...
            do
            {
                ...
                for (const FBPEditorBookmarkNode& BookmarkNode : OdysseyBrush->BookmarkNodes)
                {
