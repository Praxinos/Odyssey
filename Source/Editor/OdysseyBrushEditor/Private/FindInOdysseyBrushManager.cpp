...
#include "FindInOdysseyBrushManager.h"
...
#define LOCTEXT_NAMESPACE "FindInOdysseyBrushManager"

FFindInOdysseyBrushSearchManager* FFindInOdysseyBrushSearchManager::Instance = NULL;

const FText FFindInOdysseyBrushSearchTags::FiB_Properties = LOCTEXT("Properties", "Properties");

const FText FFindInOdysseyBrushSearchTags::FiB_Components = LOCTEXT("Components", "Components");
const FText FFindInOdysseyBrushSearchTags::FiB_IsSCSComponent = LOCTEXT("IsSCSComponent", "IsSCSComponent");

const FText FFindInOdysseyBrushSearchTags::FiB_Nodes = LOCTEXT("Nodes", "Nodes");

const FText FFindInOdysseyBrushSearchTags::FiB_SchemaName = LOCTEXT("SchemaName", "SchemaName");

const FText FFindInOdysseyBrushSearchTags::FiB_UberGraphs = LOCTEXT("Uber", "Uber");
const FText FFindInOdysseyBrushSearchTags::FiB_Functions = LOCTEXT("Functions", "Functions");
const FText FFindInOdysseyBrushSearchTags::FiB_Macros = LOCTEXT("Macros", "Macros");
const FText FFindInOdysseyBrushSearchTags::FiB_SubGraphs = LOCTEXT("Sub", "Sub");

const FText FFindInOdysseyBrushSearchTags::FiB_Name = LOCTEXT("Name", "Name");
const FText FFindInOdysseyBrushSearchTags::FiB_NativeName = LOCTEXT("NativeName", "Native Name");
const FText FFindInOdysseyBrushSearchTags::FiB_ClassName = LOCTEXT("ClassName", "ClassName");
const FText FFindInOdysseyBrushSearchTags::FiB_NodeGuid = LOCTEXT("NodeGuid", "NodeGuid");
const FText FFindInOdysseyBrushSearchTags::FiB_Tooltip = LOCTEXT("Tooltip", "Tooltip");
const FText FFindInOdysseyBrushSearchTags::FiB_DefaultValue = LOCTEXT("DefaultValue", "DefaultValue");
const FText FFindInOdysseyBrushSearchTags::FiB_Description = LOCTEXT("Description", "Description");
const FText FFindInOdysseyBrushSearchTags::FiB_Comment = LOCTEXT("Comment", "Comment");
const FText FFindInOdysseyBrushSearchTags::FiB_Path = LOCTEXT("Path", "Path");
const FText FFindInOdysseyBrushSearchTags::FiB_ParentClass = LOCTEXT("ParentClass", "ParentClass");
const FText FFindInOdysseyBrushSearchTags::FiB_Interfaces = LOCTEXT("Interfaces", "Interfaces");

const FText FFindInOdysseyBrushSearchTags::FiB_Pins = LOCTEXT("Pins", "Pins");
const FText FFindInOdysseyBrushSearchTags::FiB_PinCategory = LOCTEXT("PinCategory", "PinCategory");
const FText FFindInOdysseyBrushSearchTags::FiB_PinSubCategory = LOCTEXT("SubCategory", "SubCategory");
const FText FFindInOdysseyBrushSearchTags::FiB_ObjectClass = LOCTEXT("ObjectClass", "ObjectClass");
const FText FFindInOdysseyBrushSearchTags::FiB_IsArray = LOCTEXT("IsArray", "IsArray");
const FText FFindInOdysseyBrushSearchTags::FiB_IsReference = LOCTEXT("IsReference", "IsReference");
const FText FFindInOdysseyBrushSearchTags::FiB_Glyph = LOCTEXT("Glyph", "Glyph");
const FText FFindInOdysseyBrushSearchTags::FiB_GlyphStyleSet = LOCTEXT("GlyphStyleSet", "GlyphStyleSet");
const FText FFindInOdysseyBrushSearchTags::FiB_GlyphColor = LOCTEXT("GlyphColor", "GlyphColor");

const FText FFindInOdysseyBrushSearchTags::FiBMetaDataTag = LOCTEXT("FiBMetaDataTag", "!!FiBMD");
...
FStreamSearch::FStreamSearch(const FString& InSearchValue)
    ...
    , OdysseyBrushCountBelowVersion(0)
...
{
    FFindInOdysseyBrushSearchManager::Get().BeginSearchQuery(this);
    ...
    while (FFindInOdysseyBrushSearchManager::Get().ContinueSearchQuery(this, QueryResult))
    {
        if (QueryResult.ImaginaryOdysseyBrush.IsValid())
        {
            // If the OdysseyBrush is below the version, add it to a list. The search will still proceed on this OdysseyBrush
            if (QueryResult.Version < MinimiumVersionRequirement)
            {
                ++OdysseyBrushCountBelowVersion;
            }
            ...
            {
                SearchInstance->MakeSearchQuery(*SearchValue, QueryResult.ImaginaryOdysseyBrush);
                SearchInstance->CreateFilteredResultsListFromTree(ImaginaryDataFilter, FilteredImaginaryResults);
                SearchResult = SearchInstance->GetSearchResults(QueryResult.ImaginaryOdysseyBrush);
            }
            else
            {
                SearchResult = SearchInstance->StartSearchQuery(*SearchValue, QueryResult.ImaginaryOdysseyBrush);
            }
        ...
        if (StopTaskCounter.GetValue())
        {
            ...
            FFindInOdysseyBrushSearchManager::Get().EnsureSearchQueryEnds(this);
        }
    }
...
{
    return FFindInOdysseyBrushSearchManager::Get().GetPercentComplete(this);
}
...
namespace OdysseyBrushSearchMetaDataHelpers
{
    ...
    class TJsonFindInOdysseyBrushStringWriter : public TJsonStringWriter<PrintPolicy>
    {
    public:
        static TSharedRef< TJsonFindInOdysseyBrushStringWriter > Create( FString* const InStream )
        {
            return MakeShareable( new TJsonFindInOdysseyBrushStringWriter( InStream ) );
        }
    ...
    protected:
        TJsonFindInOdysseyBrushStringWriter( FString* const InOutString )
    ...
    public:
        /** Cached mapping of all searchable properties that have been discovered while gathering searchable data for the current OdysseyBrush */
    ...
    typedef TJsonFindInOdysseyBrushStringWriter<TCondensedJsonPrintPolicy<TCHAR>> SearchMetaDataWriter;

    /** Json Writer used for serializing FText's in the correct format for Find-in-Blueprints */
    template <class CharType = TCHAR>
    class TJsonFindInOdysseyBrushStringReader : public TJsonReader<CharType>
    {
    public:
        static TSharedRef< TJsonFindInOdysseyBrushStringReader< TCHAR > > Create( FArchive* const Stream, TMap< int32, FText >& InLookupTable  )
        {
            return MakeShareable( new TJsonFindInOdysseyBrushStringReader( Stream, InLookupTable ) );
        }

        TJsonFindInOdysseyBrushStringReader( FArchive* InStream,  TMap< int32, FText >& InLookupTable )
    ...
    typedef TJsonFindInOdysseyBrushStringReader<TCHAR> SearchMetaDataReader;
    ...
    void SavePinTypeToJson(TSharedRef< SearchMetaDataWriter>& InWriter, const FEdGraphPinType& InPinType)
    {
        ...
        {
            InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_PinCategory, InPinType.PinCategory.ToString());
        }

        if(!InPinType.PinSubCategory.IsNone())
        {
            InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_PinSubCategory, InPinType.PinSubCategory.ToString());
        }

        if(InPinType.PinSubCategoryObject.IsValid())
        {
            InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_ObjectClass, FText::FromString(InPinType.PinSubCategoryObject->GetName()));
        }
        InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_IsArray, InPinType.IsArray());
        InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_IsReference, InPinType.bIsReference);
    }
     ...
     * @param InOdysseyBrush                OdysseyBrush the property for the variable can be found in, if any
     * @param InVariableDescription        The variable description being serialized to Json
     */
    void SaveVariableDescriptionToJson(TSharedRef< SearchMetaDataWriter>& InWriter, const UBlueprint* InOdysseyBrush, const FBPVariableDescription& InVariableDescription)
    {
        FEdGraphPinType VariableType = InVariableDescription.VarType;

        InWriter->WriteObjectStart();

        InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_Name, InVariableDescription.FriendlyName);
        ...
        InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_Tooltip, TooltipResult);

        // Save the variable's pin type
        SavePinTypeToJson(InWriter, VariableType);

        // Find the UProperty and convert it into a Json value.
        UProperty* VariableProperty = FindField<UProperty>(InOdysseyBrush->GeneratedClass, InVariableDescription.VarName);
        if(VariableProperty)
        {
            const uint8* PropData = VariableProperty->ContainerPtrToValuePtr<uint8>(InOdysseyBrush->GeneratedClass->GetDefaultObject());
            ...
            if(OdysseyBrushSearchMetaDataHelpers::CheckIfJsonValueIsSearchable(JsonValue))
            {
                TSharedRef< FJsonValue > JsonValueAsSharedRef = JsonValue.ToSharedRef();
                FJsonSerializer::Serialize(JsonValue, FFindInOdysseyBrushSearchTags::FiB_DefaultValue.ToString(), StaticCastSharedRef<SearchMetaDataWriterParentClass>(InWriter), false );
            }
        }
        ...
        if (InValue)
        {
            ...
            for (FSearchableProperty& SearchableProperty : *SearchablePropertyData)
            {
                    ...
                    || bIsShallowSearchableMD || bIsSearchableMD)
                {
                    ...
                    {
                        InWriter->WriteObjectStart(FFindInOdysseyBrushSearchTags::FiBMetaDataTag);
                        {
    ...
    void GatherNodesFromGraph(TSharedRef< SearchMetaDataWriter>& InWriter, const UEdGraph* InGraph)
    {
        ...
        InWriter->WriteArrayStart(FFindInOdysseyBrushSearchTags::FiB_Nodes);
        {
            for(auto* Node : InGraph->Nodes)
            {
                if(Node)
                {
...

                    {
                        ...
                        InWriter->WriteArrayStart(FFindInOdysseyBrushSearchTags::FiB_Pins);
                        for (UEdGraphPin* Pin : Node->Pins)
                        {
                            ...
                            if (Pin->bHidden == false)
                            {
                                InWriter->WriteObjectStart();
                                {
                                    InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_Name, Pin->GetSchema()->GetPinDisplayName(Pin));
                                    InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_DefaultValue, Pin->GetDefaultAsText());
                                }
     ...
     */
    void GatherGraphSearchData(TSharedRef< SearchMetaDataWriter>& InWriter, const UBlueprint* InOdysseyBrush, const TArray< UEdGraph* >& InGraphArray, FText InTitle, TArray< UEdGraph* >* InOutSubGraphs)
    {
        if(InGraphArray.Num() > 0)
        {
            ...
            InWriter->WriteArrayStart(InTitle);
            {
                for(const UEdGraph* Graph : InGraphArray)
                {
                    ...
                    InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_Name, DisplayInfo.PlainName);

                    FText GraphDescription = FBlueprintEditorUtils::GetGraphDescription(Graph);
                    if(!GraphDescription.IsEmpty())
                    {
                        InWriter->WriteValue(FFindInOdysseyBrushSearchTags::FiB_Description, GraphDescription);
                    }
                    ...
                    InWriter->WriteArrayStart(FFindInOdysseyBrushSearchTags::FiB_Properties);
                    {
                        ...
                        for (UK2Node_FunctionEntry* const FunctionEntry : FunctionEntryNodes)
                        {
                            ...
                            {
                                SaveVariableDescriptionToJson(InWriter, InOdysseyBrush, Variable);
                            }
                        }
                    }
...
public:
    ...
    {
        // Start the OdysseyBrush indexing 'progress' notification
        FNotificationInfo Info( LOCTEXT("OdysseyBrushIndexMessage", "Indexing Blueprints...") );
        Info.bFireAndForget = false;
        Info.ButtonDetails.Add(FNotificationButtonInfo(
            LOCTEXT("OdysseyBrushIndexCancel","Cancel"),
            LOCTEXT("OdysseyBrushIndexCancelToolTip","Cancels indexing Blueprints."), FSimpleDelegate::CreateRaw(this, &FCacheAllBlueprintsTickableObject::OnCancelCaching, false)));
    ...
    /** Returns the name of the current OdysseyBrush being cached */
    FName GetCurrentCacheOdysseyBrushName() const
    {
    ...
    void OnCancelCaching(bool bIsImmediate)
    {
        if (!bIsCancelled)
        {
            ProgressNotification.Pin()->SetText(LOCTEXT("OdysseyBrushIndexCancelled", "Cancelled Indexing Blueprints!"));
            ...
            if (bIsImmediate)
            {
                FFindInOdysseyBrushSearchManager::Get().FinishedCachingBlueprints(TickCacheIndex, FailedToCacheList);
            }
            else
            {
                bIsCancelled = true;
    ...
    EActiveTimerReturnType Tick(double InCurrentTime, float InDeltaTime)
    {
        ...
        else
        {
            ...
            if (!bIsAssetReadOnlyOnDisk || !bCheckOutAndSave)
            {
                ...
                {
                    if (UBlueprint* OdysseyBrushAsset = Cast<UBlueprint>(Asset))
                    {
                        if (OdysseyBrushAsset->SkeletonGeneratedClass == nullptr)
                        {
                            // There is no skeleton class, something was wrong with the OdysseyBrush during compile on load. This asset will be marked as failing to cache.
            ...
            if(TickCacheIndex == UncachedAssets.Num())
            {
                ...
                ProgressNotification.Pin()->SetText(LOCTEXT("OdysseyBrushIndexComplete", "Finished indexing Blueprints!"));
            ...
            else
            {
                ...
                ProgressNotification.Pin()->SetText(FText::Format(LOCTEXT("OdysseyBrushIndexProgress", "Indexing Blueprints... ({Percent})"), Args));
            }
        }
        return EActiveTimerReturnType::Continue;
    }
...
FFindInOdysseyBrushSearchManager& FFindInOdysseyBrushSearchManager::Get()
{
    if (Instance == NULL)
    {
        Instance = new FFindInOdysseyBrushSearchManager();
        Instance->Initialize();
    }
...
FFindInOdysseyBrushSearchManager::FFindInOdysseyBrushSearchManager()
...
void FFindInOdysseyBrushSearchManager::Initialize()
{
    ...
    if (!GIsSavingPackage || (GIsSavingPackage && FModuleManager::Get().IsModuleLoaded(TEXT("AssetRegistry"))))
    {
        ...
        AssetRegistryModule->Get().OnAssetAdded().AddRaw(this, &FFindInOdysseyBrushSearchManager::OnAssetAdded);
        AssetRegistryModule->Get().OnAssetRemoved().AddRaw(this, &FFindInOdysseyBrushSearchManager::OnAssetRemoved);
        AssetRegistryModule->Get().OnAssetRenamed().AddRaw(this, &FFindInOdysseyBrushSearchManager::OnAssetRenamed);
    }
    else
    {
    ...
    FKismetEditorUtilities::OnBlueprintUnloaded.AddRaw(this, &FFindInOdysseyBrushSearchManager::OnBlueprintUnloaded);

    FCoreUObjectDelegates::GetPreGarbageCollectDelegate().AddRaw(this, &FFindInOdysseyBrushSearchManager::PauseFindInOdysseyBrushSearch);
    FCoreUObjectDelegates::GetPostGarbageCollect().AddRaw(this, &FFindInOdysseyBrushSearchManager::UnpauseFindInOdysseyBrushSearch);
    FCoreUObjectDelegates::OnAssetLoaded.AddRaw(this, &FFindInOdysseyBrushSearchManager::OnAssetLoaded);

    // Register to be notified of hot reloads
    IHotReloadInterface& HotReloadSupport = FModuleManager::LoadModuleChecked<IHotReloadInterface>("HotReload");
    HotReloadSupport.OnHotReload().AddRaw(this, &FFindInOdysseyBrushSearchManager::OnHotReload);
...
void FFindInOdysseyBrushSearchManager::OnAssetAdded(const FAssetData& InAssetData)
{
    ...
    const IOdysseyBrushAssetHandler* Handler = AssetClass ? FOdysseyBrushAssetHandler::Get().FindHandler(AssetClass) : nullptr;
    ...
    if (InAssetData.IsAssetLoaded())
    {
        ...
        UBlueprint* OdysseyBrush   = Handler->RetrieveOdysseyBrush(AssetObject);
        if (OdysseyBrush)
        {
            AddOrUpdateOdysseyBrushSearchMetadata(OdysseyBrush);
        }
    }
...
void FFindInOdysseyBrushSearchManager::ExtractUnloadedFiBData(const FAssetData& InAssetData, const FString& InFiBData, bool bIsVersioned)
{
    ...
    if(!ImplementedInterfaces.IsEmpty())
    {
        ...
        //  Entry := (Interface=Type'Package.Class',Graphs=(Type'Package.OdysseyBrush:Graph1',Type'Package.OdysseyBrush:Graph2',...Type'Package.OdysseyBrush:GraphN'))
        //
...
int32 FFindInOdysseyBrushSearchManager::AddSearchDataToDatabase(FSearchData InSearchData)
{
...
void FFindInOdysseyBrushSearchManager::RemoveOdysseyBrushByPath(FName InPath)
{
...
void FFindInOdysseyBrushSearchManager::OnAssetRemoved(const struct FAssetData& InAssetData)
{
    if(InAssetData.IsAssetLoaded())
    {
        RemoveOdysseyBrushByPath(InAssetData.ObjectPath);
    }
}

void FFindInOdysseyBrushSearchManager::OnAssetRenamed(const struct FAssetData& InAssetData, const FString& InOldName)
{
    // Renaming removes the item from the manager, it will be re-added in the OnAssetAdded event under the new name.
    if(InAssetData.IsAssetLoaded())
    {
        RemoveOdysseyBrushByPath(FName(*InOldName));
    }
}

void FFindInOdysseyBrushSearchManager::OnAssetLoaded(UObject* InAsset)
{
    const IOdysseyBrushAssetHandler* Handler = FOdysseyBrushAssetHandler::Get().FindHandler(InAsset->GetClass());
    UBlueprint* OdysseyBrushObject = Handler ? Handler->RetrieveOdysseyBrush(InAsset) : nullptr;

    if (OdysseyBrushObject)
    {
        ...
        // Confirm that the OdysseyBrush has not been added already, this can occur during duplication of Blueprints.
        int32* IndexPtr = SearchMap.Find(AssetPath);

        // The asset registry might not have informed us of this asset yet.
        if(IndexPtr)
        {
            // That index should never have a OdysseyBrush already, but if it does, it should be the same OdysseyBrush!
            ensureMsgf(!SearchArray[*IndexPtr].OdysseyBrush.IsValid() || SearchArray[*IndexPtr].OdysseyBrush == OdysseyBrushObject, TEXT("OdysseyBrush in database has path %s and is being stomped by %s"), *(SearchArray[*IndexPtr].AssetPath.ToString()), *AssetPath.ToString());
            ensureMsgf(!SearchArray[*IndexPtr].OdysseyBrush.IsValid() || SearchArray[*IndexPtr].AssetPath == AssetPath, TEXT("OdysseyBrush in database has path %s and is being stomped by %s"), *(SearchArray[*IndexPtr].AssetPath.ToString()), *AssetPath.ToString());
            SearchArray[*IndexPtr].OdysseyBrush = OdysseyBrushObject;
        }
...
void FFindInOdysseyBrushSearchManager::OnBlueprintUnloaded(UBlueprint* InOdysseyBrush)
{
    RemoveOdysseyBrushByPath(*InOdysseyBrush->GetPathName());
}

void FFindInOdysseyBrushSearchManager::OnHotReload(bool bWasTriggeredAutomatically)
{
    CachedAssetClasses.Reset();
}

FString FFindInOdysseyBrushSearchManager::GatherOdysseyBrushSearchMetadata(const UBlueprint* OdysseyBrush)
{
    FTemporarilyUseFriendlyNodeTitles TemporarilyUseFriendlyNodeTitles;

    FString SearchMetaData;

    // The search registry tags for a OdysseyBrush are all in Json
    TSharedRef< OdysseyBrushSearchMetaDataHelpers::TJsonFindInOdysseyBrushStringWriter<TCondensedJsonPrintPolicy<TCHAR>> > Writer = OdysseyBrushSearchMetaDataHelpers::TJsonFindInOdysseyBrushStringWriter<TCondensedJsonPrintPolicy<TCHAR>>::Create( &SearchMetaData );

    TMap<FString, TMap<FString,int>> AllPaths;
    Writer->WriteObjectStart();

    // Only pull properties if the OdysseyBrush has been compiled
    if(OdysseyBrush->SkeletonGeneratedClass)
    {
        Writer->WriteArrayStart(FFindInOdysseyBrushSearchTags::FiB_Properties);
        {
            for (const FBPVariableDescription& Variable : OdysseyBrush->NewVariables)
            {
                OdysseyBrushSearchMetaDataHelpers::SaveVariableDescriptionToJson(Writer, OdysseyBrush, Variable);
            }
        }
    ...
    OdysseyBrushSearchMetaDataHelpers::GatherGraphSearchData(Writer, OdysseyBrush, OdysseyBrush->UbergraphPages, FFindInOdysseyBrushSearchTags::FiB_UberGraphs, &SubGraphs);
    ...
    // We have interface graphs and function graphs to put into the Functions category. We cannot do them separately, so we must compile the full list
    {
        ...
        CompleteGraphList.Append(OdysseyBrush->FunctionGraphs);
        // Gather all interface graphs as functions
        for (const FBPInterfaceDescription& InterfaceDesc : OdysseyBrush->ImplementedInterfaces)
        {
            CompleteGraphList.Append(InterfaceDesc.Graphs);
        }
        OdysseyBrushSearchMetaDataHelpers::GatherGraphSearchData(Writer, OdysseyBrush, CompleteGraphList, FFindInOdysseyBrushSearchTags::FiB_Functions, &SubGraphs);
    }

    // Gather Macros
    OdysseyBrushSearchMetaDataHelpers::GatherGraphSearchData(Writer, OdysseyBrush, OdysseyBrush->MacroGraphs, FFindInOdysseyBrushSearchTags::FiB_Macros, &SubGraphs);
    ...
    OdysseyBrushSearchMetaDataHelpers::GatherGraphSearchData(Writer, OdysseyBrush, SubGraphs, FFindInOdysseyBrushSearchTags::FiB_SubGraphs, NULL);

    // Gather all SCS components
    // If we have an SCS but don't support it, then we remove it
    if(OdysseyBrush->SimpleConstructionScript)
    {
        // Remove any SCS variable nodes
        const TArray<USCS_Node*>& AllSCSNodes = OdysseyBrush->SimpleConstructionScript->GetAllNodes();
        Writer->WriteArrayStart(FFindInOdysseyBrushSearchTags::FiB_Components);
        for (TFieldIterator<UProperty> PropertyIt(OdysseyBrush->SkeletonGeneratedClass, EFieldIteratorFlags::ExcludeSuper); PropertyIt; ++PropertyIt)
        {
            ...
            if(bComponentProperty && FBlueprintEditorUtils::FindSCS_Node(OdysseyBrush, PropName) != INDEX_NONE)
            {
                FEdGraphPinType PropertyPinType;
                if(UEdGraphSchema_OdysseyBrush::StaticClass()->GetDefaultObject<UEdGraphSchema_OdysseyBrush>()->ConvertPropertyToPinType(Property, PropertyPinType))
                {
                    Writer->WriteObjectStart();
                    {
                        Writer->WriteValue(FFindInOdysseyBrushSearchTags::FiB_Name, FText::FromName(PropName));
                        Writer->WriteValue(FFindInOdysseyBrushSearchTags::FiB_IsSCSComponent, true);
...
void FFindInOdysseyBrushSearchManager::AddOrUpdateOdysseyBrushSearchMetadata(UBlueprint* InOdysseyBrush, bool bInForceReCache/* = false*/)
{
    ...
    if (InOdysseyBrush->GetOutermost()->HasAnyPackageFlags(PKG_ForDiffing))
    {
        return;
    }
    ...
    UObject* AssetObject = GetAssetObject(InOdysseyBrush);

    check(InOdysseyBrush && AssetObject);
    ...
    if(!IndexPtr)
    {
        ...
        SearchData.OdysseyBrush = InOdysseyBrush;
    ...
    {
        Index = *IndexPtr;
        SearchArray[Index].OdysseyBrush = InOdysseyBrush; // OdysseyBrush instance may change due to reloading
    }

    // Build the search data
    if (UProperty* ParentClassProp = InOdysseyBrush->GetClass()->FindPropertyByName(GET_MEMBER_NAME_CHECKED(UBlueprint, ParentClass)))
    {
        ParentClassProp->ExportTextItem(SearchArray[Index].ParentClass, ParentClassProp->ContainerPtrToValuePtr<uint8>(InOdysseyBrush), nullptr, InOdysseyBrush, 0);
    }
    ...
    if(InOdysseyBrush->SkeletonGeneratedClass != nullptr)
    {
        SearchArray[Index].Value = GatherOdysseyBrushSearchMetadata(InOdysseyBrush);
        SearchArray[Index].Version = EFiBVersion::FIB_VER_LATEST;
    }
    SearchArray[Index].bMarkedForDeletion = false;
}

void FFindInOdysseyBrushSearchManager::BeginSearchQuery(const FStreamSearch* InSearchOriginator)
{
...
bool FFindInOdysseyBrushSearchManager::ContinueSearchQuery(const FStreamSearch* InSearchOriginator, FSearchData& OutSearchData)
{
    ...
    if(int32* SearchIdxPtr = ActiveSearchQueries.Find(InSearchOriginator))
    {
        ...
        {
            // If the OdysseyBrush is not marked for deletion, and the asset is valid, we will check to see if we want to refresh the searchable data.
            if( SearchArray[SearchIdx].bMarkedForDeletion || (SearchArray[SearchIdx].OdysseyBrush.IsValid() && SearchArray[SearchIdx].OdysseyBrush->IsPendingKill()) )
            {
...
void FFindInOdysseyBrushSearchManager::EnsureSearchQueryEnds(const class FStreamSearch* InSearchOriginator)
{
    // Must lock this behind a critical section to ensure that no other thread is accessing it at the same time
    FScopeLock ScopeLock(&SafeQueryModifyCriticalSection);
...
float FFindInOdysseyBrushSearchManager::GetPercentComplete(const FStreamSearch* InSearchOriginator) const
{
    FScopeLock ScopeLock(&SafeQueryModifyCriticalSection);
...
FString FFindInOdysseyBrushSearchManager::QuerySingleOdysseyBrush(UBlueprint* InOdysseyBrush, bool bInRebuildSearchData/* = true*/)
{
    // AddOrUpdateOdysseyBrushSearchMetadata would fail to cache any data for a OdysseyBrush loaded specifically for diffing, but the bigger question
    // here in this function is how you are doing a search specifically for data within this OdysseyBrush. This function is limited to be called
    // only when querying within the specific OdysseyBrush (somehow opened a diff OdysseyBrush) and when gathering the OdysseyBrush's tags (usually for saving)
    const bool bIsDiffingOdysseyBrush = InOdysseyBrush->GetOutermost()->HasAnyPackageFlags(PKG_ForDiffing);
    if (!bIsDiffingOdysseyBrush)
    {
        if (bInRebuildSearchData)
        {
            // Update the OdysseyBrush, make sure it is fully up-to-date
            AddOrUpdateOdysseyBrushSearchMetadata(InOdysseyBrush, true);
        }
        ...
        // This should always be true since we make sure to refresh the search data for this OdysseyBrush when doing the search, unless we do not rebuild the searchable data
    ...
    {
        UE_LOG(LogBlueprint, Warning, TEXT("Attempted to query an old OdysseyBrush package opened for diffing!"));
    }
    return FString();
}

void FFindInOdysseyBrushSearchManager::PauseFindInOdysseyBrushSearch()
{
...
void FFindInOdysseyBrushSearchManager::UnpauseFindInOdysseyBrushSearch()
{
...
void FFindInOdysseyBrushSearchManager::CleanCache()
{
    ...
    for(auto& SearchValuePair : SearchMap)
    {
        ...
        if( !SearchArray[SearchValuePair.Value].bMarkedForDeletion && !(SearchArray[SearchValuePair.Value].OdysseyBrush.IsValid() && SearchArray[SearchValuePair.Value].OdysseyBrush->IsPendingKill()) )
        {
        ...
        else
        {
            ...
            {
                SearchArray[SearchValuePair.Value].OdysseyBrush = nullptr;
...
void FFindInOdysseyBrushSearchManager::BuildCache()
{
    AssetRegistryModule = &FModuleManager::LoadModuleChecked<FAssetRegistryModule>(TEXT("AssetRegistry"));

    TArray< FAssetData > OdysseyBrushAssets;
    FARFilter ClassFilter;
    ClassFilter.bRecursiveClasses = true;

    for (FName ClassName : FOdysseyBrushAssetHandler::Get().GetRegisteredClassNames())
    {
        ClassFilter.ClassNames.Add(ClassName);
    }

    AssetRegistryModule->Get().GetAssets(ClassFilter, OdysseyBrushAssets);

    for( FAssetData& Asset : OdysseyBrushAssets )
    {
        OnAssetAdded(Asset);
    }
}

FText FFindInOdysseyBrushSearchManager::ConvertHexStringToFText(FString InHexString)
{
    TArray<uint8> SerializedData;
...
FString FFindInOdysseyBrushSearchManager::ConvertFTextToHexString(FText InValue)
{
    TArray<uint8> SerializedData;
    FMemoryWriter Ar(SerializedData);
...
void FFindInOdysseyBrushSearchManager::OnCacheAllUncachedAssets(bool bInSourceControlActive, bool bInCheckoutAndSave)
{
...
void FFindInOdysseyBrushSearchManager::CacheAllUncachedAssets(TWeakPtr< SFindInBlueprints > InSourceWidget, FWidgetActiveTimerDelegate& InOutActiveTimerDelegate, FSimpleDelegate InOnFinished/* = FSimpleDelegate()*/, EFiBVersion InMinimiumVersionRequirement/* = EFiBVersion::FIB_VER_LATEST*/)
{
    ...
    if(!IsCacheInProgress())
    {
        ...
        {
            if ((SearchData.Value.Len() != 0 || SearchData.ImaginaryOdysseyBrush.IsValid()) && SearchData.Version < InMinimiumVersionRequirement)
            {
        ...
        if (ReturnValue != EAppReturnType::Cancel)
        {
            ...
            if(!bIsSourceControlEnabled && bCheckoutAndSave)
            {
                ...
                ISourceControlModule::Get().ShowLoginDialog(FSourceControlLoginClosed::CreateRaw(this, &FFindInOdysseyBrushSearchManager::OnCacheAllUncachedAssets, bCheckoutAndSave), ELoginWindowMode::Modeless, EOnLoginWindowStartup::PreserveProvider);
            }
            else
            {
...
void FFindInOdysseyBrushSearchManager::CancelCacheAll(SFindInBlueprints* InFindInOdysseyBrushWidget)
{
    if(IsCacheInProgress() && ((SourceCachingWidget.IsValid() && SourceCachingWidget.Pin().Get() == InFindInOdysseyBrushWidget) || !SourceCachingWidget.IsValid()))
    {
...
int32 FFindInOdysseyBrushSearchManager::GetCurrentCacheIndex() const
{
...
FName FFindInOdysseyBrushSearchManager::GetCurrentCacheOdysseyBrushName() const
{
    FName CachingBPName;
    if(CachingObject)
    {
        CachingBPName = CachingObject->GetCurrentCacheOdysseyBrushName();
    }

    return CachingBPName;
}

float FFindInOdysseyBrushSearchManager::GetCacheProgress() const
{
...
int32 FFindInOdysseyBrushSearchManager::GetNumberUncachedAssets() const
{
...
void FFindInOdysseyBrushSearchManager::FinishedCachingBlueprints(int32 InNumberCached, TSet<FName>& InFailedToCacheList)
{
    ...
    // Signal the callback, so the source FindInOdysseyBrush can resubmit their search queries
...
bool FFindInOdysseyBrushSearchManager::IsCacheInProgress() const
{
    return CachingObject != nullptr;
}

TSharedPtr< FJsonObject > FFindInOdysseyBrushSearchManager::ConvertJsonStringToObject(bool bInIsVersioned, FString InJsonString, TMap<int32, FText>& OutFTextLookupTable)
{
    ...
    TSharedRef< TJsonReader<> > Reader = OdysseyBrushSearchMetaDataHelpers::SearchMetaDataReader::Create( &ReaderStream, LookupTable );
    FJsonSerializer::Deserialize( Reader, JsonObject );

    return JsonObject;
}

void FFindInOdysseyBrushSearchManager::GlobalFindResultsClosed(const TSharedRef<SFindInBlueprints>& FindResults)
{
...
FText FFindInOdysseyBrushSearchManager::GetGlobalFindResultsTabLabel(int32 TabIdx)
{
...
TSharedRef<SDockTab> FFindInOdysseyBrushSearchManager::SpawnGlobalFindResultsTab(const FSpawnTabArgs& SpawnTabArgs, int32 TabIdx)
{
    TAttribute<FText> Label = TAttribute<FText>::Create(TAttribute<FText>::FGetter::CreateRaw(this, &FFindInOdysseyBrushSearchManager::GetGlobalFindResultsTabLabel, TabIdx));

    TSharedRef<SDockTab> NewTab = SNew(SDockTab)
        .TabRole(ETabRole::NomadTab)
        .Label(Label)
        .ToolTipText(LOCTEXT("GlobalFindResultsTabTooltip", "Search for a string in all OdysseyBrush assets."));
...
TSharedPtr<SFindInBlueprints> FFindInOdysseyBrushSearchManager::OpenGlobalFindResultsTab()
{
...
TSharedPtr<SFindInBlueprints> FFindInOdysseyBrushSearchManager::GetGlobalFindResults()
{
...
void FFindInOdysseyBrushSearchManager::EnableGlobalFindResults(bool bEnable)
{
    ...
    if (bEnable)
    {
        ...
        for (int32 TabIdx = 0; TabIdx < ARRAY_COUNT(GlobalFindResultsTabIDs); TabIdx++)
        {
            ...
            if (!GlobalTabManager->HasTabSpawner(TabID))
            {
                ...
                GlobalTabManager->RegisterNomadTabSpawner(TabID, FOnSpawnTab::CreateRaw(this, &FFindInOdysseyBrushSearchManager::SpawnGlobalFindResultsTab, TabIdx))
    ...
    {
        // Close all Global Find Results tabs when turning the feature off, since these may not get closed along with the OdysseyBrush Editor contexts above.
...
void FFindInOdysseyBrushSearchManager::CloseOrphanedGlobalFindResultsTabs(TSharedPtr<class FTabManager> TabManager)
{
