...
FSCSDiff::FSCSDiff(const UBlueprint* InOdysseyBrush)
{
    if (!FBlueprintEditorUtils::SupportsConstructionScript(InOdysseyBrush) || InOdysseyBrush->SimpleConstructionScript == NULL)
    {
    ...
    ContainerWidget = SNew(SSplitter)
        ...
        [
            SAssignNew(SCSEditor, SSCSEditor)
                .ActorContext(InOdysseyBrush->GeneratedClass->GetDefaultObject<AActor>())
                .AllowEditing(false)
