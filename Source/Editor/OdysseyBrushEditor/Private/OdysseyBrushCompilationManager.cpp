...
#include "OdysseyBrushCompilationManager.h"
...
#define LOCTEXT_NAMESPACE "OdysseyBrushCompilationManager"
...
struct FOdysseyBrushCompilationManagerImpl : public FGCObject
{
    FOdysseyBrushCompilationManagerImpl();
    virtual ~FOdysseyBrushCompilationManagerImpl();
...
FOdysseyBrushCompilationManagerImpl::FOdysseyBrushCompilationManagerImpl()
{
...
void FOdysseyBrushCompilationManagerImpl::AddReferencedObjects(FReferenceCollector& Collector)
{
...
void FOdysseyBrushCompilationManagerImpl::QueueForCompilation(const FBPCompileRequest& CompileJob)
{
...
void FOdysseyBrushCompilationManagerImpl::CompileSynchronouslyImpl(const FBPCompileRequest& Request)
{
    ...
    // We do not want to regenerate a search Guid during loads, nothing has changed in the OdysseyBrush
    ...
    {
        FFindInOdysseyBrushSearchManager::Get().AddOrUpdateOdysseyBrushSearchMetadata(Request.BPToCompile);
    }
    ...
    {
        // When the OdysseyBrush is recompiled, then update the bound events for level scripting
...
struct FCompilerData
{
    explicit FCompilerData(UBlueprint* InBP, ECompilationManagerJobType InJobType, FCompilerResultsLog* InResultsLogOverride, EBlueprintCompileOptions UserOptions, bool bBytecodeOnly)
    {
        ...
        if(InResultsLogOverride == nullptr)
        {
            ...
            ResultsLog->BeginEvent(TEXT("OdysseyBrushCompilationManager Compile"));
    ...
    bool ShouldSetTemporaryOdysseyBrushFlags() const { return JobType != ECompilationManagerJobType::RelinkOnly; }
    ...
    bool ShouldUpdateOdysseyBrushSearchMetadata() const { return JobType == ECompilationManagerJobType::Normal && !InternalOptions.bSkipFiBSearchMetaUpdate; }
...
void FOdysseyBrushCompilationManagerImpl::FlushCompilationQueueImpl(TArray<UObject*>* ObjLoaded, bool bSuppressBroadcastCompiled, TArray<UBlueprint*>* BlueprintsCompiled, TArray<UBlueprint*>* BlueprintsCompiledOrSkeletonCompiled)
{
    TGuardValue<bool> GuardTemplateNameFlag(GCompilingBlueprint, true);
    ...
    { // begin GTimeCompiling scope
        ...
        for(const FBPCompileRequest& CompileJob : QueuedRequests)
        {
            ...
            if(CompileJob.BPToCompile->BlueprintType == BPTYPE_MacroLibrary)
            {
                ...
                for(UBlueprint* DependentOdysseyBrush : DependentBlueprints)
                {
                    if(!IsQueuedForCompilation(DependentOdysseyBrush))
                    {
                        DependentOdysseyBrush->bQueuedForCompilation = true;
                        CurrentlyCompilingBPs.Add(
                            FCompilerData(
                                DependentOdysseyBrush,
                        ...
                        BlueprintsToRecompile.Add(DependentOdysseyBrush);
                    }
                }
            }
        }
        ...
        for(UBlueprint* BP : BlueprintsToRecompile)
        {
            ...
            if(UClass* OldSkeletonClass = BP->SkeletonGeneratedClass)
            {
                ...
                {
                    if(UBlueprint* ChildOdysseyBrush = UBlueprint::GetBlueprintFromClass(ChildClass))
                    {
                        if(!IsQueuedForCompilation(ChildOdysseyBrush))
                        {
                            ChildOdysseyBrush->bQueuedForCompilation = true;
                            ensure(ChildOdysseyBrush->bHasBeenRegenerated);
                            CurrentlyCompilingBPs.Add(FCompilerData(ChildOdysseyBrush, ECompilationManagerJobType::RelinkOnly, nullptr, EBlueprintCompileOptions::None, false));
                        }
                    }
                }
            }
        ...
        {
            if (!CompilerData.ShouldSetTemporaryOdysseyBrushFlags())
            {
                continue;
            }
        ...
        for (FCompilerData& CompilerData : CurrentlyCompilingBPs)
        {
            ...
            if (CompilerData.ShouldUpdateOdysseyBrushSearchMetadata())
            {
                // Do not want to run this code without the editor present nor when running commandlets.
                if (GEditor && GIsEditor)
                {
                    // We do not want to regenerate a search Guid during loads, nothing has changed in the OdysseyBrush and it is cached elsewhere
                    if (!BP->bIsRegeneratingOnLoad)
                    {
                        FFindInOdysseyBrushSearchManager::Get().AddOrUpdateOdysseyBrushSearchMetadata(BP);
                    }
                }
            }
        ...
        for (FCompilerData& CompilerData : CurrentlyCompilingBPs)
        {
            ...
            if(CompilerData.ShouldCompileClassLayout())
            {
                ...
                {
                    CompilerData.ActiveResultsLog->Error(*LOCTEXT("KismetCompileError_MalformedParentClasss", "OdysseyBrush @@ has missing or NULL parent class.").ToString(), BP);
                }
            }
        ...
        const bool bSaveOdysseyBrushAfterCompileSucceeded = Settings->SaveOnCompile == SoC_SuccessOnly;
        ...
        for (FCompilerData& CompilerData : CurrentlyCompilingBPs)
        {
            ...
            else
            {
                ...
                {
                    // OdysseyBrush is error free.  Go ahead and fix up debug info
                ...
                if(!BP->bIsRegeneratingOnLoad)
                {
                    if(bSaveBlueprintsAfterCompile || (bSaveOdysseyBrushAfterCompileSucceeded && BP->Status == BS_UpToDate))
                    {
            ...
            if (!CompilerData.IsSkeletonOnly())
            {
                ...
                if (!BP->bIsRegeneratingOnLoad && BP->GeneratedClass)
                {
                    ...
                    if (BP->Status != BS_Error)
                    {
                        ...
                        {
                            // Route through the compiler context in order to perform type-specific OdysseyBrush class validation.
                            ...
                            if (CompilerData.ShouldValidateClassDefaultObject())
                            {
                                ...
                                if (ensureAlways(ClassDefaultObject))
                                {
                                    ...
                                    // Adjust OdysseyBrush status to match anything new that was found during validation.
                                    if (CompilerData.ActiveResultsLog->NumErrors > 0)
                                    {
                                        BP->Status = BS_Error;
                                    }
...
void FOdysseyBrushCompilationManagerImpl::FlushReinstancingQueueImpl()
{
...

    {
        ...
        FBatchReplaceInstancesOfClassParameters params;
        params.bArchetypesAreUpToDate = true;
        FBlueprintCompileReinstancer::BatchReplaceInstancesOfClass(ClassesToReinstance, params);
...
bool FOdysseyBrushCompilationManagerImpl::HasBlueprintsToCompile() const
{
    return QueuedRequests.Num() != 0;
}

bool FOdysseyBrushCompilationManagerImpl::IsGeneratedClassLayoutReady() const
{
    return bGeneratedClassLayoutReady;
}

void FOdysseyBrushCompilationManagerImpl::GetDefaultValue(const UClass* ForClass, const UProperty* Property, FString& OutDefaultValueAsString) const
{
...
void FOdysseyBrushCompilationManagerImpl::BuildDSOMap(UObject* OldObject, UObject* NewObject, TMap<UObject*, UObject*>& OutOldToNewDSO)
{
...
void FOdysseyBrushCompilationManagerImpl::ReinstanceBatch(TArray<FReinstancingJob>& Reinstancers, TMap< UClass*, UClass* >& InOutOldToNewClassMap, TArray<UObject*>* ObjLoaded)
{
    ...
    for (const FReinstancingJob& ReinstancingJob : Reinstancers)
    {
        ...
        UClass* OldClass = NULL; /* CurrentReinstancer->DuplicatedClass; */
        if(!OldClass)
        {
            continue;
        }
        ...
        bool bParentLayoutChanged = false; /* !FStructUtils::TheSameLayout(OldClass, CurrentReinstancer->ClassToReinstance); */
    ...
    for(UClass* Class : ClassesToReinstance)
    {
        ...
        */
        ...
        UClass* SuperClass = NULL; /* NewestReinstancer->ClassToReinstance->GetSuperClass(); */
    ...
    {
        const TSharedPtr<FBlueprintCompileReinstancer>& CurrentReinstancer = ReinstancingJob.Reinstancer;
        if ( true /* CurrentReinstancer->DuplicatedClass */ )
        {
            //InOutOldToNewClassMap.Add(CurrentReinstancer->DuplicatedClass, CurrentReinstancer->ClassToReinstance);
        }

        /*UBlueprint* CompiledOdysseyBrush = UBlueprint::GetBlueprintFromClass(CurrentReinstancer->ClassToReinstance);
    ...
    Reinstancers.Sort(
        [](const FReinstancingJob& ReinstancingDataA, const FReinstancingJob& ReinstancingDataB)
        {
            ...
            UClass* A = NULL; /*CompilerDataA->ClassToReinstance;*/
            UClass* B = NULL; /*CompilerDataB->ClassToReinstance;*/
            int32 DepthA = 0;
            int32 DepthB = 0;
    ...
    for (const FReinstancingJob& ReinstancingJob : Reinstancers)
    {
        ...
        if ( true /* CurrentReinstancer->DuplicatedClass */)
        {
            ...
            if (OldCDO)
            {
                UObject* NewCDO = NULL; /*CurrentReinstancer->ClassToReinstance->GetDefaultObject(true);*/
        ...
        if ( true /*UBlueprintGeneratedClass* BPGClass = CastChecked<UBlueprintGeneratedClass>(CurrentReinstancer->ClassToReinstance) */)
        {
            ...
            if(OldCDO && ObjLoaded)
            {
                UBlueprint* CurrentBP = NULL; /* CastChecked<UBlueprint>(BPGClass->ClassGeneratedBy); */
    ...
    for (const FReinstancingJob& ReinstancingJob : Reinstancers)
    {
        ...
        if(OldClass)
        {
            ...
            for(UObject* Archetype : ArchetypeObjects )
            {
                ...
                UObject* NewArchetype = NULL; /*NewObject<UObject>(OriginalOuter, CurrentReinstancer->ClassToReinstance, OriginalName, OriginalFlags & FlagMask);*/
    ...
    for (const FReinstancingJob& ReinstancingJob : Reinstancers)
    {
        ...
        UBlueprint* BP  = NULL;
        if( true /* UBlueprint* BP = Cast<UBlueprint>(CurrentReinstancer->ClassToReinstance->ClassGeneratedBy) */ )
        {
...
*/
UClass* FOdysseyBrushCompilationManagerImpl::FastGenerateSkeletonClass(UBlueprint* BP, FKismetCompilerContext& CompilerContext, bool bIsSkeletonOnly)
{
            ...
            for(UEdGraphPin* Pin : InputPins)
            {
                if(Pin->Direction == EEdGraphPinDirection::EGPD_Output && !Schema->IsExecPin(*Pin) && Pin->ParentPin == nullptr && Pin->GetFName() != UK2Node_Event::DelegateOutputName)
                {
                    ...
                    if(Param)
                    {
                        ...
                        if(Pin->PinType.bIsConst || (bForceArrayStructRefsConst && (Pin->PinType.IsArray() || Pin->PinType.PinCategory == UEdGraphSchema_OdysseyBrush::PC_Struct) && Pin->PinType.bIsReference))
                        {
            ...
            if(EntryNodes.Num() > 0)
            {
                ...
                if(NewFunction)
                {
                    ...
                    if(bIsStaticFunction)
                    {
                        ...
                        {
                            FEdGraphPinType WorldContextPinType(UEdGraphSchema_OdysseyBrush::PC_Object, NAME_None, UObject::StaticClass(), EPinContainerType::None, false, FEdGraphTerminalType());
    ...
    for ( const UTimelineTemplate* Timeline : BP->Timelines )
    {
        for(int32 EventTrackIdx=0; EventTrackIdx<Timeline->EventTracks.Num(); EventTrackIdx++)
        {
            ...
            MakeEventFunction(Timeline->EventTracks[EventTrackIdx].GetFunctionName(), EFunctionFlags::FUNC_None, TArray<UEdGraphPin*>(), TArray< TSharedPtr<FUserPinInfo> >(), nullptr, false);
        }
    ...
    if (CompilerContext.UsePersistentUberGraphFrame() && AllEventGraphs.Num() != 0)
    {
        ...
        CompilerContext.NewClass = Ret;
        UProperty* Property = NULL; /* CompilerContext.CreateVariable(UBlueprintGeneratedClass::GetUberGraphFrameName(), Type); */
        CompilerContext.NewClass = OriginalNewClass;
...
bool FOdysseyBrushCompilationManagerImpl::IsQueuedForCompilation(UBlueprint* BP)
{
    return BP->bQueuedForCompilation;
}

// FFixupBytecodeReferences Implementation:
FOdysseyBrushCompilationManagerImpl::FFixupBytecodeReferences::FFixupBytecodeReferences(UObject* InObject)
{
...
FArchive& FOdysseyBrushCompilationManagerImpl::FFixupBytecodeReferences::operator<<( UObject*& Obj )
{
    if (Obj != NULL)
    {
...
FOdysseyBrushCompilationManagerImpl* BPCMImpl = nullptr;
...
void FOdysseyBrushCompilationManager::Initialize()
{
    if(!BPCMImpl)
    {
        BPCMImpl = new FOdysseyBrushCompilationManagerImpl();
    }
}

void FOdysseyBrushCompilationManager::Shutdown()
{
    delete BPCMImpl;
    BPCMImpl = nullptr;
}
...
// Forward to impl:
void FOdysseyBrushCompilationManager::FlushCompilationQueue(TArray<UObject*>* ObjLoaded)
{
    if(BPCMImpl)
    {
...
void FOdysseyBrushCompilationManager::FlushCompilationQueueAndReinstance()
{
    if(BPCMImpl)
    {
...
void FOdysseyBrushCompilationManager::CompileSynchronously(const FBPCompileRequest& Request)
{
    if(BPCMImpl)
    {
...
void FOdysseyBrushCompilationManager::NotifyOdysseyBrushLoaded(UBlueprint* BPLoaded)
{
    // Blueprints can be loaded before editor modules are on line:
    if(!BPCMImpl)
    {
        FOdysseyBrushCompilationManager::Initialize();
    }
...
void FOdysseyBrushCompilationManager::QueueForCompilation(UBlueprint* BPLoaded)
{
    BPCMImpl->QueueForCompilation(FBPCompileRequest(BPLoaded, EBlueprintCompileOptions::None, nullptr));
}

bool FOdysseyBrushCompilationManager::IsGeneratedClassLayoutReady()
{
    if(!BPCMImpl)
    {
...
bool FOdysseyBrushCompilationManager::GetDefaultValue(const UClass* ForClass, const UProperty* Property, FString& OutDefaultValueAsString)
{
    if(!BPCMImpl)
    {
