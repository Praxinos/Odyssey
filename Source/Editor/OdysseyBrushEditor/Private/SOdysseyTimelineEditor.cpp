...
#define LOCTEXT_NAMESPACE "OdysseyTimelineEditor"
...
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void STimelineEditor::Construct(const FArguments& InArgs, TSharedPtr<FOdysseyBrushEditor> InKismet2, UTimelineTemplate* InTimelineObj)
{
...
FText STimelineEditor::GetTimelineName() const
{
    if(TimelineObj != NULL)
    {
        ...
        return FText::FromName(TimelineObj->GetVariableName());
    }
    else
    {
...
FReply STimelineEditor::CreateNewTrack(FTimelineEdTrack::ETrackType Type)
{
    ...
    TSharedPtr<FOdysseyBrushEditor> Kismet2 = Kismet2Ptr.Pin();
    UBlueprint* OdysseyBrush = Kismet2->GetBlueprintObj();
    UK2Node_Timeline* TimelineNode = FBlueprintEditorUtils::FindNodeForTimeline(OdysseyBrush, TimelineObj);
    UClass* OwnerClass = OdysseyBrush->GeneratedClass;
    check(OwnerClass);
    ...
    if( SelectedObj && TimelineNode )
    {
        ...
        if(SelectedObj->IsA( UCurveFloat::StaticClass() ) )
        {
            ...
            else
            {
                ...
                NewFloatTrack.SetTrackName( FName( *TrackName ), TimelineObj );
        ...
        else if(SelectedObj->IsA( UCurveVector::StaticClass() ))
        {
            ...
            NewTrack.SetTrackName( FName( *TrackName ), TimelineObj );
