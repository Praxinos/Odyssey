...
#include "OdysseyBitmaskLiteralDetails.h"
#include "OdysseyFormatTextDetails.h"

#define LOCTEXT_NAMESPACE "OdysseyInspector"
        ...
        [
            ...
            [
                ...
                [
                    ...
                    .Visibility(this, &SKismetInspector::GetInheritedOdysseyBrushComponentWarningVisibility)
                    .WarningText(NSLOCTEXT("SKismetInspector", "OdysseyBrushUneditableInheritedComponentWarning", "Components flagged as not editable when inherited must be edited in the <a id=\"HyperlinkDecorator\" style=\"DetailsView.BPMessageHyperlinkStyle\">Parent OdysseyBrush</>"))
                    .OnHyperlinkClicked(this, &SKismetInspector::OnInheritedOdysseyBrushComponentWarningHyperlinkClicked)
                ]
                +SVerticalBox::Slot()
                [
...
void SKismetInspector::Construct(const FArguments& InArgs)
{
    ...
    OdysseyBrushEditorPtr = InArgs._Kismet2;
    bShowPublicView = InArgs._ShowPublicViewControl;
    bShowTitleArea = InArgs._ShowTitleArea;
    TSharedPtr<FOdysseyBrushEditor> Kismet2 = OdysseyBrushEditorPtr.Pin();
    ...
    TWeakPtr<SMyOdysseyBrush> MyOdysseyBrush = Kismet2.IsValid() ? Kismet2->GetMyOdysseyBrushWidget() : InArgs._MyOdysseyBrushWidget;

    if( MyOdysseyBrush.IsValid() )
    {
        FOnGetDetailCustomizationInstance LayoutDelegateDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FOdysseyBrushDelegateActionDetails::MakeInstance, MyOdysseyBrush);
        ...
        FOnGetDetailCustomizationInstance LayoutGraphDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FOdysseyBrushGraphActionDetails::MakeInstance, MyOdysseyBrush);
        ...
        FOnGetDetailCustomizationInstance LayoutVariableDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FOdysseyBrushVarActionDetails::MakeInstance, MyOdysseyBrush);
    ...
    if (Kismet2.IsValid() && Kismet2->IsEditingSingleOdysseyBrush())
    {
        FOnGetDetailCustomizationInstance LayoutOptionDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FOdysseyBrushGlobalOptionsDetails::MakeInstance, OdysseyBrushEditorPtr);
        ...
        FOnGetDetailCustomizationInstance LayoutDocumentationDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FOdysseyBrushDocumentationDetails::MakeInstance, OdysseyBrushEditorPtr);
        PropertyView->RegisterInstancedCustomPropertyLayout(UEdGraphNode_Documentation::StaticClass(), LayoutDocumentationDetails);

        FOnGetDetailCustomizationInstance GraphNodeDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FOdysseyBrushGraphNodeDetails::MakeInstance, OdysseyBrushEditorPtr);
        ...
        FOnGetDetailCustomizationInstance::CreateStatic(&FChildActorComponentDetails::MakeInstance, OdysseyBrushEditorPtr));
    }
    ...
    [
        SNew(SVerticalBox)
        .AddMetaData<FTagMetaData>(FTagMetaData(TEXT("OdysseyBrushInspector")))
        + SVerticalBox::Slot()
        .FillHeight(1.0f)
        [
...
void SKismetInspector::EnableComponentDetailsCustomization(bool bEnable)
{
    ...
    if (bEnable)
    {
        ...
        FOnGetDetailCustomizationInstance LayoutComponentDetails = FOnGetDetailCustomizationInstance::CreateStatic(&FOdysseyBrushComponentDetails::MakeInstance, OdysseyBrushEditorPtr);
...
void SKismetInspector::UpdateFromObjects(const TArray<UObject*>& PropertyObjects, struct FKismetSelectionInfo& SelectionInfo, const FShowDetailsOptions& Options)
{
    ...
    {
        if (OdysseyBrushEditor->CanAccessComponentsMode())
        {
...
bool SKismetInspector::IsPropertyVisible( const FPropertyAndParent& PropertyAndParent ) const
{
    ...
    {
        const UBlueprint* BP = OdysseyBrushEditorPtr.IsValid() ? OdysseyBrushEditorPtr.Pin()->GetBlueprintObj() : NULL;
        const bool VariableAddedInCurentOdysseyBrush = (OwningClass->ClassGeneratedBy == BP);

        // If we did not add this var, hide it!
        if(!VariableAddedInCurentOdysseyBrush)
        {
    ...
    // figure out if this OdysseyBrush variable is an Actor variable
    ...
    {
        // Actor variables can't have default values (because OdysseyBrush templates are library elements that can
...
bool SKismetInspector::IsPropertyEditingEnabled() const
{
    ...
    if (OdysseyBrushEditorPtr.IsValid())
    {
        ...
        {
            bIsEditable = OdysseyBrushEditorPtr.Pin()->InEditingMode();
        }
        else
        {
...
EVisibility SKismetInspector::GetInheritedOdysseyBrushComponentWarningVisibility() const
{
    bool bIsUneditableOdysseyBrushComponent = false;

    // Check to see if any selected components are inherited from blueprint
    for (const TWeakObjectPtr<UObject>& SelectedObject : SelectedObjects)
    {
        UActorComponent* Component = Cast<UActorComponent>(SelectedObject.Get());
        bIsUneditableOdysseyBrushComponent = Component ? !CastChecked<UActorComponent>(Component->GetArchetype())->IsEditableWhenInherited() : false;
        if (bIsUneditableOdysseyBrushComponent)
        {
            break;
        }
    }

    return bIsUneditableOdysseyBrushComponent ? EVisibility::Visible : EVisibility::Collapsed;
}

void SKismetInspector::OnInheritedOdysseyBrushComponentWarningHyperlinkClicked(const FSlateHyperlinkRun::FMetadata& Metadata)
{
    if (OdysseyBrushEditorPtr.IsValid())
    {
        UBlueprint* OdysseyBrush = OdysseyBrushEditorPtr.Pin()->GetBlueprintObj();
        if (OdysseyBrush && OdysseyBrush->ParentClass->HasAllClassFlags(CLASS_CompiledFromBlueprint))
        {
            // Open the blueprint
            GEditor->EditObject(CastChecked<UBlueprint>(OdysseyBrush->ParentClass->ClassGeneratedBy));
        }
    }
}
...
void SKismetInspector::SetPublicViewCheckboxState( ECheckBoxState InIsChecked )
{
    ...
    OdysseyBrushEditorPtr.Pin()->StartEditingDefaults();
}
