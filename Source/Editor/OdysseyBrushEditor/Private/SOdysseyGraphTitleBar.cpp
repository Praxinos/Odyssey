...
#include "SOdysseyGraphTitleBarAddNewBookmark.h"
...
#define LOCTEXT_NAMESPACE "OdysseyGraphTitleBar"
...
const FSlateBrush* SGraphTitleBar::GetTypeGlyph() const
{
    ...
    return FOdysseyBrushEditor::GetGlyphForGraph(EdGraphObj, true);
}
...
EVisibility SGraphTitleBar::IsGraphOdysseyBrushNameVisible() const
{
    return bShowOdysseyBrushTitle ? EVisibility::Visible : EVisibility::Collapsed;
}
...
void SGraphTitleBar::Construct( const FArguments& InArgs )
{
            ...
            [
                ...
                [
                    ...
                    [
                        ...
                        .Visibility( this, &SGraphTitleBar::IsGraphOdysseyBrushNameVisible )
                    ]
                    +SHorizontalBox::Slot()
                    .AutoWidth()
                    .VAlign(VAlign_Center)
    ...
    UBlueprint* OdysseyBrushObj = FBlueprintEditorUtils::FindBlueprintForGraph(this->EdGraphObj);
    if (OdysseyBrushObj)
    {
        bShowOdysseyBrushTitle = true;
        OdysseyBrushTitle = FText::FromString(OdysseyBrushObj->GetFriendlyName());
...
void SGraphTitleBar::Refresh()
{
    ...
    if (EdGraphObj)
    {
        if (UBlueprint* OdysseyBrushObj = FBlueprintEditorUtils::FindBlueprintForGraph(this->EdGraphObj))
        {
            OdysseyBrushTitle = FText::FromString(OdysseyBrushObj->GetFriendlyName());
            RebuildBreadcrumbTrail();
        }
