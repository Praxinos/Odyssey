...
#include "SOdysseyCSEditorViewportClient.h"
...
FSCSEditorViewportClient::FSCSEditorViewportClient(TWeakPtr<FOdysseyBrushEditor>& InOdysseyBrushEditorPtr, FPreviewScene* InPreviewScene, const TSharedRef<SSCSEditorViewport>& InSCSEditorViewport)
...
void FSCSEditorViewportClient::Tick(float DeltaSeconds)
{
    ...
    TSharedPtr<SSCSEditor> SCSEditor = OdysseyBrushEditorPtr.Pin()->GetSCSEditor();
    ...
    {
        // Ensure that the preview actor instance is up-to-date for component editing (e.g. after compiling the OdysseyBrush, the actor may be reinstanced outside of this class)
        if(PreviewActor != OdysseyBrushEditorPtr.Pin()->GetBlueprintObj()->SimpleConstructionScript->GetComponentEditorActorInstance())
        {
            OdysseyBrushEditorPtr.Pin()->GetBlueprintObj()->SimpleConstructionScript->SetComponentEditorActorInstance(PreviewActor);
        }
...
void FSCSEditorViewportClient::Draw(const FSceneView* View, FPrimitiveDrawInterface* PDI)
{
    ...
    if(PreviewActor)
    {
        if(GUnrealEd != NULL)
        {
            TArray<FSCSEditorTreeNodePtrType> SelectedNodes = OdysseyBrushEditorPtr.Pin()->GetSelectedSCSEditorTreeNodes();
...
void FSCSEditorViewportClient::DrawCanvas( FViewport& InViewport, FSceneView& View, FCanvas& Canvas )
{
    ...
    if(PreviewActor)
    {
        ...
        auto SelectedNodes = OdysseyBrushEditorPtr.Pin()->GetSelectedSCSEditorTreeNodes();
...
void FSCSEditorViewportClient::ProcessClick(class FSceneView& View, class HHitProxy* HitProxy, FKey Key, EInputEvent Event, uint32 HitX, uint32 HitY)
{
    ...
    if (HitProxy)
    {
        if (HitProxy->IsA(HInstancedStaticMeshInstance::StaticGetType()))
        {
            ...
            TSharedPtr<ISCSEditorCustomization> Customization = OdysseyBrushEditorPtr.Pin()->CustomizeSCSEditor(InstancedStaticMeshInstanceProxy->Component);
        ...
        else if (HitProxy->IsA(HActor::StaticGetType()))
        {
            ...
            if (ActorProxy && ActorProxy->Actor && ActorProxy->PrimComponent)
            {
                ...
                {
                    TSharedPtr<ISCSEditorCustomization> Customization = OdysseyBrushEditorPtr.Pin()->CustomizeSCSEditor(SelectedCompInstance);
                    if (!(Customization.IsValid() && Customization->HandleViewportClick(AsShared(), View, HitProxy, Key, Event, HitX, HitY)))
                    {
                        ...
                        if (OdysseyBrushEditorPtr.IsValid())
                        {
                            ...
                            OdysseyBrushEditorPtr.Pin()->FindAndSelectSCSEditorTreeNode(SelectedCompInstance, bIsCtrlKeyDown);
                        }
                    }
                }
            }
...
bool FSCSEditorViewportClient::InputWidgetDelta( FViewport* InViewport, EAxisList::Type CurrentAxis, FVector& Drag, FRotator& Rot, FVector& Scale )
{
    ...
    if(bIsManipulating && CurrentAxis != EAxisList::None)
    {
        bHandled = true;
        AActor* PreviewActor = GetPreviewActor();
        auto OdysseyBrushEditor = OdysseyBrushEditorPtr.Pin();
        if (PreviewActor && OdysseyBrushEditor.IsValid())
        {
            TArray<FSCSEditorTreeNodePtrType> SelectedNodes = OdysseyBrushEditor->GetSelectedSCSEditorTreeNodes();
            if(SelectedNodes.Num() > 0)
            {
                ...
                for(auto It(SelectedNodes.CreateIterator());It;++It)
                {
                    ...
                    if(bCanEdit)
                    {
                        ...
                        USceneComponent* SelectedTemplate = Cast<USceneComponent>(SelectedNodePtr->GetEditableComponentTemplate(OdysseyBrushEditor->GetBlueprintObj()));
                        if(SceneComp != NULL && SelectedTemplate != NULL)
                        {
                            ...
                            TSharedPtr<ISCSEditorCustomization> Customization = OdysseyBrushEditor->CustomizeSCSEditor(SceneComp);
                            ...
                            UBlueprint* PreviewOdysseyBrush = UBlueprint::GetBlueprintFromClass(PreviewActor->GetClass());
                            if(PreviewOdysseyBrush != NULL)
                            {
                                // Like PostEditMove(), but we only need to re-run construction scripts
                                if(PreviewOdysseyBrush && PreviewOdysseyBrush->bRunConstructionScriptOnDrag)
                                {
...
void FSCSEditorViewportClient::TrackingStopped()
{
    if( bIsManipulating )
    {
        ...
        {
            UBlueprint* PreviewOdysseyBrush = UBlueprint::GetBlueprintFromClass(PreviewActor->GetClass());
            if(PreviewOdysseyBrush != nullptr && !PreviewOdysseyBrush->bRunConstructionScriptOnDrag)
            {
                PreviewActor->RerunConstructionScripts();
            }
        }
...
FWidget::EWidgetMode FSCSEditorViewportClient::GetWidgetMode() const
{
    ...
    {
        const TSharedPtr<FOdysseyBrushEditor> BluePrintEditor = OdysseyBrushEditorPtr.Pin();
...
FVector FSCSEditorViewportClient::GetWidgetLocation() const
{
    ...
    if(PreviewActor)
    {
        ...
        if(SelectedNodes.Num() > 0)
        {
            ...
            if( SceneComp )
            {
                TSharedPtr<ISCSEditorCustomization> Customization = OdysseyBrushEditorPtr.Pin()->CustomizeSCSEditor(SceneComp);
...
void FSCSEditorViewportClient::InvalidatePreview(bool bResetCamera)
{
    ...
    OdysseyBrushEditorPtr.Pin()->UpdatePreviewActor(OdysseyBrush, !bIsPreviewActorValid);
...
void FSCSEditorViewportClient::ResetCamera()
{
    ...
    USceneThumbnailInfo* ThumbnailInfo = Cast<USceneThumbnailInfo>(OdysseyBrush->ThumbnailInfo);
    if(ThumbnailInfo)
    {
...
{
    return OdysseyBrushEditorPtr.Pin()->GetPreviewActor();
}
...
void FSCSEditorViewportClient::ToggleIsSimulateEnabled()
{
    ...
    OdysseyBrushEditorPtr.Pin()->DestroyPreview();
    ...
    TSharedPtr<SWidget> SCSEditor = OdysseyBrushEditorPtr.Pin()->GetSCSEditor();
    TSharedRef<SWidget> Inspector = OdysseyBrushEditorPtr.Pin()->GetInspector();

    // When simulate is enabled, we don't want to allow the user to modify the components
    OdysseyBrushEditorPtr.Pin()->UpdatePreviewActor(OdysseyBrushEditorPtr.Pin()->GetBlueprintObj(), true);
...
void FSCSEditorViewportClient::BeginTransaction(const FText& Description)
{
    ...
    if(!ScopedTransaction)
    {
        ...
        {
            UBlueprint* PreviewOdysseyBrush = OdysseyBrushEditor->GetBlueprintObj();
            if (PreviewOdysseyBrush != nullptr)
            {
                FBlueprintEditorUtils::MarkBlueprintAsModified(PreviewOdysseyBrush);
            }
            ...
            for(auto SelectedSCSNodeIter(SelectedNodes.CreateIterator()); SelectedSCSNodeIter; ++SelectedSCSNodeIter)
            {
                ...
                if(Node.IsValid())
                {
                    if(USCS_Node* SCS_Node = Node->GetSCSNode())
                    {
                        ...
                        UBlueprint* OdysseyBrush = SCS ? SCS->GetBlueprint() : nullptr;
                        if (OdysseyBrush == PreviewOdysseyBrush)
                        {
                            SCS_Node->Modify();
                        }
                    }
                    ...
                    UActorComponent* ComponentTemplate = Node->GetEditableComponentTemplate(PreviewOdysseyBrush);
