...
class ODYSSEYBRUSHEDITOR_API SKismetInspector : public SCompoundWidget
{
public:
    SLATE_BEGIN_ARGS( SKismetInspector )
        ...
        SLATE_ARGUMENT(TWeakPtr<FOdysseyBrushEditor>, Kismet2)
        SLATE_ARGUMENT(TWeakPtr<SMyOdysseyBrush>, MyOdysseyBrushWidget)
...
protected:
    ...
    EVisibility GetInheritedOdysseyBrushComponentWarningVisibility() const;

    /** Opens the parent blueprint when the hyperlink in the warning is clicked */
    void OnInheritedOdysseyBrushComponentWarningHyperlinkClicked(const FSlateHyperlinkRun::FMetadata& Metadata);
