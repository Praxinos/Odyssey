 ...
 */
class ODYSSEYBRUSHEDITOR_API FSCSEditorTreeNode : public TSharedFromThis<FSCSEditorTreeNode>
{
public:
    ...
    */
    ...
    /**
     * @param ActualEditedOdysseyBrush currently edited blueprint
     *
     * @return The component template that can be editable for actual class.
     */
    virtual UActorComponent* GetEditableComponentTemplate(UBlueprint* ActualEditedOdysseyBrush);
    /**
...
public:
    ...
    /**
    * @return The OdysseyBrush to which this node belongs.
    */
    ...
    /**
     * @return Whether or not this object represents an inherited SCS node (one from a SCS node in a parent OdysseyBrush).
     */
    ...
    /**
     * @return Whether or not this object represents a component instance that was created by the user and not by a native or OdysseyBrush-generated class.
     */
...
class ODYSSEYBRUSHEDITOR_API FSCSEditorTreeNodeComponentBase : public FSCSEditorTreeNode
{
protected:
...
class ODYSSEYBRUSHEDITOR_API FSCSEditorTreeNodeInstancedInheritedComponent : public FSCSEditorTreeNodeComponentBase
{
public:
    /**
    ...
    virtual UActorComponent* GetEditableComponentTemplate(UBlueprint* ActualEditedOdysseyBrush) override;
...
class ODYSSEYBRUSHEDITOR_API FSCSEditorTreeNodeInstanceAddedComponent : public FSCSEditorTreeNodeComponentBase
{
public:
    /**
    * Constructs a wrapper around a component template not contained within an SCS tree node (e.g. "native" components).
...
class ODYSSEYBRUSHEDITOR_API FSCSEditorTreeNodeComponent : public FSCSEditorTreeNodeComponentBase
{
public:
    /**
     ...
     * @param bInIsInherited Whether or not the SCS tree node is inherited from a parent OdysseyBrush class.
     */
...
protected:
    ...
    UActorComponent* INTERNAL_GetOverridenComponentTemplate(UBlueprint* OdysseyBrush, bool bCreateIfNecessary) const;
...
class ODYSSEYBRUSHEDITOR_API FSCSEditorTreeNodeRootActor : public FSCSEditorTreeNode
{
public:
...
class ODYSSEYBRUSHEDITOR_API FSCSEditorTreeNodeSeparator : public FSCSEditorTreeNode
{
public:
...
public:
    ...
    ODYSSEYBRUSHEDITOR_API static FSlateColor GetColorTintForIcon(FSCSEditorTreeNodePtrType InNode);
...
class ODYSSEYBRUSHEDITOR_API SSCSEditor : public SCompoundWidget
{
public:
    ...
    SLATE_BEGIN_ARGS( SSCSEditor )
        :_EditorMode(EComponentEditorMode::OdysseyBrushSCS)
        ,_IsDiffing(false)
    ...
    /** Adds a component to the SCS Table
       ...
       @param bSkipMarkOdysseyBrushModified     (In) Optionally skip marking this blueprint as modified (e.g. if we're handling that externally)
       @return The reference of the newly created ActorComponent */
    UActorComponent* AddNewComponent(UClass* NewComponentClass, UObject* Asset, const bool bSkipMarkOdysseyBrushModified = false, bool bSetFocusToNewItem = true);

    /** Adds a new SCS Node to the component Table
       @param NewNode    (In) The SCS node to add
       @param Asset        (In) Optional asset to assign to the component
       @param bMarkOdysseyBrushModified (In) Whether or not to mark the OdysseyBrush as structurally modified
       @param bSetFocusToNewItem (In) Select the new item and activate the inline rename widget (default is true)
       @return The reference of the newly created ActorComponent */
    UActorComponent* AddNewNode(USCS_Node* NewNode, UObject* Asset, bool bMarkOdysseyBrushModified, bool bSetFocusToNewItem = true);
     ...
     * @param OdysseyBrush                            the active blueprint context being edited
     ...
     */
    static void BuildMenuEventsSection( FMenuBuilder& Menu, UBlueprint* OdysseyBrush, UClass* SelectedClass, FCanExecuteAction CanExecuteActionDelegate, FOdysseyGetSelectedObjectsDelegate GetSelectedObjectsDelegate );
     ...
     */
    void GetSelectedItemsForContextMenu(TArray<FOdysseyComponentEventConstructionData>& OutSelectedItems) const;

    /** Provides access to the OdysseyBrush context that's being edited */
...
protected:
     ...
     * @param OdysseyBrush                        the active blueprint context
     ...
     */
    static void CreateEventsForSelection(UBlueprint* OdysseyBrush, FName EventName, FOdysseyGetSelectedObjectsDelegate GetSelectedObjectsDelegate);

    /**
     * Function to construct an event for a node
     * @param OdysseyBrush                        the nodes blueprint
     * @param EventName                        the event to add
     * @param EventData                        the event data structure describing the node
     */
    static void ConstructEvent(UBlueprint* OdysseyBrush, const FName EventName, const FOdysseyComponentEventConstructionData EventData);
     ...
     * @param EventData                        the event data structure describing the node
     */
    static void ViewEvent(UBlueprint* OdysseyBrush, const FName EventName, const FOdysseyComponentEventConstructionData EventData);
    ...
    EVisibility GetPromoteToOdysseyBrushButtonVisibility() const;

    /** @return The visibility of the Edit OdysseyBrush button (only visible with an actor instance that is created from a blueprint)*/
    EVisibility GetEditOdysseyBrushButtonVisibility() const;

    /** @return the tooltip describing how many properties will be applied to the blueprint */
    FText OnGetApplyChangesToOdysseyBrushTooltip() const;

    /** @return the tooltip describing how many properties will be reset to the blueprint default*/
    FText OnGetResetToOdysseyBrushDefaultsTooltip() const;

    /** Opens the blueprint editor for the blueprint being viewed by the scseditor */
    void OnOpenOdysseyBrushEditor(bool bForceCodeEditing) const;

    /** Propagates instance changes to the blueprint */
    void OnApplyChangesToOdysseyBrush() const;

    /** Resets instance changes to the blueprint default */
    void OnResetToOdysseyBrushDefaults() const;

    /** Converts the current actor instance to a blueprint */
    void PromoteToOdysseyBrush() const;

    /** Called when the promote to blueprint button is clicked */
    FReply OnPromoteToOdysseyBrushClicked();
    ...
    /**
     * Creates a new OdysseyBrush component from the specified class type
