 ...
 */
struct ODYSSEYBRUSHEDITOR_API FFindInOdysseyBrushSearchTags
{
    /** Properties tag, for OdysseyBrush variables */
    ...
    /** Sub graph tag, for any sub-graphs in a OdysseyBrush */
    ...
    /** End const values for Find-in-OdysseyBrush */
};

/** Tracks data relevant to a OdysseyBrush for searches */
struct FSearchData
{
    /** The OdysseyBrush this search data points to, if available */
    ...
    /** Search data block for the OdysseyBrush */
    FString Value;

    /** Parent Class */
    FString ParentClass;

    /** Interfaces implemented by the OdysseyBrush */
    TArray<FString> Interfaces;

    /** Cached to determine if the OdysseyBrush is seen as no longer valid, allows it to be cleared out next save to disk */
    bool bMarkedForDeletion;

    /** Cached ImaginaryOdysseyBrush data for the searchable content, prevents having to re-parse every search */
    FImaginaryFiBDataSharedPtr ImaginaryOdysseyBrush;

    /** Version of the data */
    int32 Version;

    FSearchData()
        : OdysseyBrush(nullptr)
...
struct ODYSSEYBRUSHEDITOR_API FFiBMD
{
...
public:
    ...
    UBlueprint* GetParentOdysseyBrush() const;
    ...
    */
    ...
    /** Returns the Object represented by this search information give the OdysseyBrush it can be found in */
    virtual UObject* GetObject(UBlueprint* InOdysseyBrush) const;
...
public:
    ...
    /** Returns the Out-of-Date OdysseyBrush count */
    int32 GetOutOfDateCount() const
    {
        return OdysseyBrushCountBelowVersion;
    }
...
public:
    ...
    /** When searching, any OdysseyBrush below this version will be considered out-of-date */
    EFiBVersion MinimiumVersionRequirement;

    /** A going count of all Blueprints below the MinimiumVersionRequirement */
    int32 OdysseyBrushCountBelowVersion;
...
////////////////////////////////////
// FFindInOdysseyBrushSearchManager

/** Singleton manager for handling all OdysseyBrush searches, helps to manage the going progress of Blueprints, and is thread-safe. */
class ODYSSEYBRUSHEDITOR_API FFindInOdysseyBrushSearchManager
{
public:
    static FFindInOdysseyBrushSearchManager* Instance;
    static FFindInOdysseyBrushSearchManager& Get();

    FFindInOdysseyBrushSearchManager();
    ~FFindInOdysseyBrushSearchManager();

    /**
     * Gathers the OdysseyBrush's search metadata and adds or updates it in the cache
     *
     * @param InOdysseyBrush        OdysseyBrush to cache the searchable data for
     * @param bInForceReCache    Forces the OdysseyBrush to be recache'd, regardless of what data it believes exists
     */
    void AddOrUpdateOdysseyBrushSearchMetadata(UBlueprint* InOdysseyBrush, bool bInForceReCache = false);
     ...
     *
     * @param InSearchOriginator        Pointer to the thread object that the query originates from
     * @param OutSearchData                Result of the search, if there is any OdysseyBrush search data still available to query
    ...
    /**
     * Query for a single, specific OdysseyBrush's search block
     *
     * @param InOdysseyBrush                The OdysseyBrush to search for
     * @param bInRebuildSearchData        When TRUE the search data will be freshly collected
     * @return                            The search block
     */
    FString QuerySingleOdysseyBrush(UBlueprint* InOdysseyBrush, bool bInRebuildSearchData = true);
    ...
    void CancelCacheAll(SFindInBlueprints* InFindInOdysseyBrushWidget);
    ...
    /** Returns the list of OdysseyBrush paths that failed to cache */
    ...
    /**
     * Callback to note that OdysseyBrush caching is complete
     *
    ...
    /** Given a fully constructed Find-in-OdysseyBrush FString of searchable data, will parse and construct a JsonObject */
...
private:
    ...
    /** Callback from Kismet when a OdysseyBrush is unloaded */
    void OnBlueprintUnloaded(class UBlueprint* InOdysseyBrush);

    /** Callback hook from the Hot Reload manager that indicates that a module has been hot-reloaded */
    void OnHotReload(bool bWasTriggeredAutomatically);

    /** Helper to gathers the OdysseyBrush's search metadata */
    FString GatherOdysseyBrushSearchMetadata(const UBlueprint* OdysseyBrush);
    ...
    /** Builds the cache from all available OdysseyBrush assets that the asset registry has discovered at the time of this function. Occurs on startup */
    void BuildCache();

    /**
     * Helper to properly add a OdysseyBrush's SearchData to the database
     *
    ...
    /** Removes a OdysseyBrush from being managed by the FiB system by passing in the UBlueprint's path */
    void RemoveOdysseyBrushByPath(FName InPath);
...
protected:
    ...
    /** Maps the OdysseyBrush paths to their index in the SearchArray */
    TMap<FName, int32> SearchMap;

    /** Stores the OdysseyBrush search data and is used to iterate over in small chunks */
    ...
    /** Mapping between a class name and its UClass instance - used for faster look up in FFindInOdysseyBrushSearchManager::OnAssetAdded */
...
struct ODYSSEYBRUSHEDITOR_API FDisableGatheringDataOnScope
{
    bool bOriginallyEnabled;
    FDisableGatheringDataOnScope() : bOriginallyEnabled(FFindInOdysseyBrushSearchManager::Get().IsGatheringDataEnabled())
    {
        FFindInOdysseyBrushSearchManager::Get().EnableGatheringData(false);
    }
    ~FDisableGatheringDataOnScope()
    {
        FFindInOdysseyBrushSearchManager::Get().EnableGatheringData(bOriginallyEnabled);
    }
};
