...
class FOdysseyBrushEditorToolbar;
class FSCSEditorTreeNode;
class IMessageLogListing;
class INameValidatorInterface;
class ISCSEditorCustomization;
class SOdysseyBrushPalette;
class SOdysseyBrushBookmarks;
...
class SOdysseyBrushPreferencesOverrides;
...
/////////////////////////////////////////////////////
// FOdysseyCustomDebugObjectEntry - Used to pass a custom debug object override around

struct FOdysseyCustomDebugObject
{
public:
...
public:
    FOdysseyCustomDebugObject()
        : Object(NULL)
    {
    }

    FOdysseyCustomDebugObject(UObject* InObject, const FString& InLabel)
        : Object(InObject)
...
struct ODYSSEYBRUSHEDITOR_API FOdysseySelectionDetailsSummoner : public FWorkflowTabFactory
{
public:
    FOdysseySelectionDetailsSummoner(TSharedPtr<class FAssetEditorToolkit> InHostingApp);
...
/////////////////////////////////////////////////////
// FOdysseyComponentEventConstructionData

/** The structure used to construct the "Add Event" menu entries */
struct FOdysseyComponentEventConstructionData
{
...
DECLARE_DELEGATE_OneParam(FOdysseyGetSelectedObjectsDelegate, TArray<FOdysseyComponentEventConstructionData>&);

/** Delegate for Node Creation Analytics */
DECLARE_DELEGATE(FNodeCreationAnalytic);

/** Describes user actions that created new node */
namespace EOdysseyNodeCreateAction
{
    enum Type
    {
        MyOdysseyBrushDragPlacement,
...
class ODYSSEYBRUSHEDITOR_API FOdysseyBrushEditor : public IOdysseyBrushEditor, public FGCObject, public FNotifyHook, public FTickableEditorObject, public FEditorUndoClient
{
...
public:
     ...
     */
    void InitOdysseyBrushEditor(const EToolkitMode::Type Mode, const TSharedPtr< class IToolkitHost >& InitToolkitHost, const TArray<class UBlueprint*>& InBlueprints, bool bShouldOpenInDefaultsMode);
...
public:
    ...
    //~ Begin IOdysseyBrushEditor Interface
    ...
    virtual void SummonSearchUI(bool bSetFindWithinOdysseyBrush, FString NewSearchTerms = FString(), bool bSelectFirstResult = false) override;
    ...
    virtual void AnalyticsTrackNodeEvent(UBlueprint* OdysseyBrush, UEdGraphNode *GraphNode, bool bNodeDelete = false) const override;
    void AnalyticsTrackCompileEvent(UBlueprint* OdysseyBrush, int32 NumErrors, int32 NumWarnings) const;
    //~ End IOdysseyBrushEditor Interface
...
public:
    ...
    /** Returns a pointer to the OdysseyBrush object we are currently editing, as long as we are editing exactly one */
    virtual UBlueprint* GetBlueprintObj() const;

    /**    Returns whether the editor is currently editing a single blueprint object */
    bool IsEditingSingleOdysseyBrush() const;
    ...
    TSharedRef<class SOdysseyBrushPalette> GetPalette() const { return Palette.ToSharedRef(); }
    TSharedRef<class SOdysseyBrushBookmarks> GetBookmarksWidget() const { return BookmarksWidget.ToSharedRef(); }
    ...
    TSharedRef<class SOdysseyBrushPreview> GetOdysseyBrushPreview() const { return OdysseyBrushPreview.ToSharedRef(); }
    TSharedRef<class SOdysseyBrushPreferencesOverrides> GetOdysseyBrushPreferencesOverrides() const { return OdysseyBrushPreferencesOverridesWidget.ToSharedRef(); }
    TSharedRef<class SOdysseyBrushTestArea> GetOdysseyBrushTestArea() const { return OdysseyBrushTestArea.ToSharedRef(); }
    TSharedRef<class SOdysseyBrushInternalExposed> GetOdysseyBrushInternalExposed() const { return OdysseyBrushInternalExposed.ToSharedRef(); }
    ...
    TSharedPtr<class SMyOdysseyBrush> GetMyOdysseyBrushWidget() const { return MyOdysseyBrushWidget; }
    ...
    *
    * @param InOdysseyBrush            The OdysseyBrush to create or update the preview for.
    * @param bInForceFullUpdate    Force a full update to respawn actors.
    */
    void UpdatePreviewActor(UBlueprint* InOdysseyBrush, bool bInForceFullUpdate = false);

    /**
    * Destroy the OdysseyBrush preview.
    */
    void DestroyPreview();

    TSharedPtr<class FOdysseyBrushEditorToolbar> GetToolbarBuilder() { return Toolbar; }
    ...
    /** Returns true if the parent class is also a OdysseyBrush */
    bool IsParentClassOfObjectAOdysseyBrush(const UBlueprint* OdysseyBrush) const;

    /** Returns true if the parent class of the OdysseyBrush being edited is also a OdysseyBrush */
    bool IsParentClassAOdysseyBrush() const;

    /** Returns true if the parent class of the OdysseyBrush being edited is native */
    ...
    void SetupViewForOdysseyBrushEditingMode();

    // Ensures the blueprint is up to date
    void EnsureOdysseyBrushIsUpToDate(UBlueprint* OdysseyBrushObj);
    ...
    void PostLayoutOdysseyBrushEditorInitialization();
    ...
    void ReparentOdysseyBrush_Clicked();
    bool ReparentOdysseyBrush_IsVisible() const;
    void ReparentOdysseyBrush_NewParentChosen(UClass* ChosenClass);
    ...
    static FName SelectionState_MyOdysseyBrush;
     ...
     *
     * @param    InOdysseyBrush                The blueprint in which to create the variable.
     ...
     */
    void DoPromoteToVariable(UBlueprint* InOdysseyBrush, UEdGraphPin* InTargetPin, bool bInToMemberVariable);

    /** Called when node is spawned by keymap */
    void OnNodeSpawnedByKeymap();

    /** Update Node Creation mechanisms for analytics */
    void UpdateNodeCreationStats(const EOdysseyNodeCreateAction::Type CreateAction);
    ...
    void CreateMergeToolTab(const UBlueprint* BaseOdysseyBrush, const UBlueprint* RemoteOdysseyBrush, const FOnMergeResolved& ResolutionCallback);
    ...
    /**
     * Returns the currently focused graph in the OdysseyBrush editor
     */
    ...
    virtual void GetCustomDebugObjects(TArray<FOdysseyCustomDebugObject>& DebugList) const { }
    ...
    bool IsGraphInCurrentOdysseyBrush(UEdGraph* InGraph) const;

    /** Get the context to use from the OdysseyBrush type */
    static FName GetContextFromBlueprintType(EBlueprintType InType);

    /* Selects an item in "My OdysseyBrush" by name. */
    ...
    /** Adds a new bookmark node to the OdysseyBrush that's being edited. */
...
protected:
    ...
    /** Returns the name of the OdysseyBrush's parent class */
    ...
    void FindInOdysseyBrush_Clicked();
    ...
    /** Edit the class settings aka OdysseyBrush global options */
    ...
    /** Called when the OdysseyBrush we are editing has changed */
    virtual void OnOdysseyBrushChangedImpl(UBlueprint* InOdysseyBrush, bool bIsJustBeingCompiled = false);

    /** Called when the OdysseyBrush we are editing has changed, forwards to impl */
    void OnOdysseyBrushChanged(UBlueprint* InOdysseyBrush) { return OnOdysseyBrushChangedImpl(InOdysseyBrush); }

    void OnOdysseyBrushCompiled(UBlueprint* InOdysseyBrush);

    /** Handles the unloading of Blueprints (by closing the editor, if it operating on the OdysseyBrush being unloaded)*/
    void OnBlueprintUnloaded(UBlueprint* InOdysseyBrush);
    ...
    void OnListObjectsReferencedByOdysseyBrush();
    void OnRepairCorruptedOdysseyBrush();
...
protected:
    ...
    TSharedPtr<class SOdysseyBrushPalette> Palette;

    /** OdysseyBrush bookmark editing widget */
    TSharedPtr<class SOdysseyBrushBookmarks> BookmarksWidget;

    /** All of this blueprints' functions and variables */
    TSharedPtr<class SMyOdysseyBrush> MyOdysseyBrushWidget;
    ...
    TSharedPtr<class SOdysseyBrushPreferencesOverrides> OdysseyBrushPreferencesOverridesWidget;

    /** Odyssey Brush preview */
    TSharedPtr<class SOdysseyBrushPreview> OdysseyBrushPreview;

    /** Odyssey Brush test area */
    TSharedPtr<class SOdysseyBrushTestArea> OdysseyBrushTestArea;

    /** Odyssey Brush internal exposed */
    TSharedPtr<class SOdysseyBrushInternalExposed> OdysseyBrushInternalExposed;
    ...
    TSharedPtr<class FOdysseyBrushEditorToolbar> Toolbar;
    ...
    bool bOdysseyBrushModifiedOnOpen;
...
public:
    ...
    DECLARE_EVENT ( FOdysseyBrushEditor, FOnRefreshEvent );
    FOnRefreshEvent& OnRefresh() { return RefreshEvent; }

private:
    /** Purge Blueprint for hacking a custom ed graph schema */
    void  PurgeHackBP();
...
private:
    ...
    struct FAnalyticsStatistics
    {
        ...
        int32 MyOdysseyBrushNodeDragPlacementCount;
