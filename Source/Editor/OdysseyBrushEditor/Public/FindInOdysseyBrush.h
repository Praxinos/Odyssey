...
/** Some utility functions to help with Find-in-OdysseyBrush functionality */
...
public:
    ...
    virtual UObject* GetObject(UBlueprint* InOdysseyBrush) const override;
...
public:
    ...
    void Construct(const FArguments& InArgs, TSharedPtr<class FOdysseyBrushEditor> InOdysseyBrushEditor = nullptr);
    ~SFindInBlueprints();

    /** Focuses this widget's search box, and changes the mode as well, and optionally the search terms */
    void FocusForUse(bool bSetFindWithinOdysseyBrush, FString NewSearchTerms = FString(), bool bSelectFirstResult = false);

    /**
     * Submits a search query
     *
     * @param InSearchString                        String to search using
     * @param bInIsFindWithinOdysseyBrush                TRUE if searching within the current OdysseyBrush only
     ...
     */
    void MakeSearchQuery(FString InSearchString, bool bInIsFindWithinOdysseyBrush, enum ESearchQueryFilter InSearchFilterForImaginaryDataReturn = ESearchQueryFilter::AllFilter, EFiBVersion InMinimiumVersionRequirement = EFiBVersion::FIB_VER_LATEST, FOnSearchComplete InOnSearchComplete = FOnSearchComplete());
...
private:
    ...
    /** Register any Find-in-OdysseyBrush commands */
    ...
    /** Callback to return the cache bar's current indexing OdysseyBrush name */
    FText GetCurrentCacheOdysseyBrushName() const;
    ...
    /** Retrieves the current index of the OdysseyBrush caching process */
    ...
    /** Returns the visibility of the caching OdysseyBrush name, visible when in progress, collapsed when not */
    EVisibility GetCachingOdysseyBrushNameVisiblity() const;
    ...
    /** Returns TRUE if OdysseyBrush caching is in progress */
...
private:
    ...
    /** In Find Within OdysseyBrush mode, we need to keep a handle on the root result, because it won't show up in the tree */
    ...
    bool bIsInFindWithinOdysseyBrushMode;

    /** Thread object that searches through OdysseyBrush data on a separate thread */
