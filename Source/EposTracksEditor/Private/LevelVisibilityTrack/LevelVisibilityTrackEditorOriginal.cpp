...
FLevelVisibilityTrackEditorOriginal::FLevelVisibilityTrackEditorOriginal( TSharedRef<ISequencer> InSequencer )
	: FMovieSceneTrackEditor( InSequencer ) 
{ }


TSharedRef<ISequencerTrackEditor> FLevelVisibilityTrackEditorOriginal::CreateTrackEditor( TSharedRef<ISequencer> InSequencer )
{
	return MakeShareable( new FLevelVisibilityTrackEditorOriginal( InSequencer ) );
}

bool FLevelVisibilityTrackEditorOriginal::SupportsSequence(UMovieSceneSequence* InSequence) const
{
	return (InSequence != nullptr) && (InSequence->GetClass()->GetName() == TEXT("LevelSequence"));
}

bool FLevelVisibilityTrackEditorOriginal::SupportsType( TSubclassOf<UMovieSceneTrack> Type ) const
{
	return Type == UMovieSceneLevelVisibilityTrack::StaticClass();
}

const FSlateBrush* FLevelVisibilityTrackEditorOriginal::GetIconBrush() const
{
	return FEditorStyle::GetBrush("Sequencer.Tracks.LevelVisibility");
}

TSharedRef<ISequencerSection> FLevelVisibilityTrackEditorOriginal::MakeSectionInterface( UMovieSceneSection& SectionObject, UMovieSceneTrack& Track, FGuid ObjectBinding )
{
	...
	return MakeShareable( new FLevelVisibilitySectionOriginal( *LevelVisibilitySection ) );
}

void FLevelVisibilityTrackEditorOriginal::BuildAddTrackMenu( FMenuBuilder& MenuBuilder )
{
	MenuBuilder.AddMenuEntry(
		...
		FUIAction( FExecuteAction::CreateRaw( this, &FLevelVisibilityTrackEditorOriginal::OnAddTrack ) ) );
}


TSharedPtr<SWidget> FLevelVisibilityTrackEditorOriginal::BuildOutlinerEditWidget( const FGuid& ObjectBinding, UMovieSceneTrack* Track, const FBuildEditWidgetParams& Params )
{
	// Create a container edit box
	return FSequencerUtilities::MakeAddButton( 
		LOCTEXT( "AddVisibilityTrigger", "Visibility Trigger" ),
		FOnGetContent::CreateSP( this, &FLevelVisibilityTrackEditorOriginal::BuildAddVisibilityTriggerMenu, Track ),
		Params.NodeIsHovered, GetSequencer() );
}


UMovieSceneLevelVisibilitySection* FLevelVisibilityTrackEditorOriginal::AddNewSection( UMovieScene* MovieScene, UMovieSceneTrack* LevelVisibilityTrack, ELevelVisibility Visibility )
{
...
void FLevelVisibilityTrackEditorOriginal::OnAddTrack()
{
...
TSharedRef<SWidget> FLevelVisibilityTrackEditorOriginal::BuildAddVisibilityTriggerMenu( UMovieSceneTrack* LevelVisibilityTrack )
{
	...
	MenuBuilder.AddMenuEntry(
		...
		FUIAction( FExecuteAction::CreateSP(
			this, &FLevelVisibilityTrackEditorOriginal::OnAddNewSection, LevelVisibilityTrack, ELevelVisibility::Visible ) ) );
	...
	MenuBuilder.AddMenuEntry(
		...
		FSlateIcon(),
		FUIAction( FExecuteAction::CreateSP(
		this, &FLevelVisibilityTrackEditorOriginal::OnAddNewSection, LevelVisibilityTrack, ELevelVisibility::Hidden ) ) );

	return MenuBuilder.MakeWidget();
}


void FLevelVisibilityTrackEditorOriginal::OnAddNewSection( UMovieSceneTrack* LevelVisibilityTrack, ELevelVisibility Visibility )
{
	UMovieScene* FocusedMovieScene = GetFocusedMovieScene();
