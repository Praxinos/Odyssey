...
#define LOCTEXT_NAMESPACE "AudioTrackEditor"
...
FAudioSectionOriginal::FAudioSectionOriginal( UMovieSceneSection& InSection, TWeakPtr<ISequencer> InSequencer )
	: Section( InSection )
...
FAudioSectionOriginal::~FAudioSectionOriginal()
{
}

UMovieSceneSection* FAudioSectionOriginal::GetSectionObject()
{ 
	return &Section;
}


FText FAudioSectionOriginal::GetSectionTitle() const
{
	UMovieSceneAudioSection* AudioSection = Cast<UMovieSceneAudioSection>(&Section);
	...
	return NSLOCTEXT("FAudioSectionOriginal", "NoAudioTitleName", "No Audio");
}


float FAudioSectionOriginal::GetSectionHeight() const
{
	return Section.GetTypedOuter<UMovieSceneAudioTrack>()->GetRowHeight();
}

int32 FAudioSectionOriginal::OnPaintSection( FSequencerSectionPainter& Painter ) const
{
...
void FAudioSectionOriginal::Tick( const FGeometry& AllottedGeometry, const FGeometry& ParentGeometry, const double InCurrentTime, const float InDeltaTime )
{
	UMovieSceneAudioSection* AudioSection = Cast<UMovieSceneAudioSection>(&Section);
...
void FAudioSectionOriginal::BeginResizeSection()
{
	UMovieSceneAudioSection* AudioSection = Cast<UMovieSceneAudioSection>(&Section);
...
void FAudioSectionOriginal::ResizeSection(ESequencerSectionResizeMode ResizeMode, FFrameNumber ResizeTime)
{
	UMovieSceneAudioSection* AudioSection = Cast<UMovieSceneAudioSection>(&Section);
...
void FAudioSectionOriginal::BeginSlipSection()
{
	UMovieSceneAudioSection* AudioSection = Cast<UMovieSceneAudioSection>(&Section);
	InitialStartOffsetDuringResize = AudioSection->GetStartOffset();
	InitialStartTimeDuringResize = AudioSection->HasStartFrame() ? AudioSection->GetInclusiveStartFrame() : 0;
...
void FAudioSectionOriginal::SlipSection(FFrameNumber SlipTime)
{
	UMovieSceneAudioSection* AudioSection = Cast<UMovieSceneAudioSection>(&Section);
...
void FAudioSectionOriginal::RegenerateWaveforms(TRange<float> DrawRange, int32 XOffset, int32 XSize, const FColor& ColorTint, float DisplayScale)
{
	UMovieSceneAudioSection* AudioSection = Cast<UMovieSceneAudioSection>(&Section);
...
FAudioTrackEditorOriginal::FAudioTrackEditorOriginal( TSharedRef<ISequencer> InSequencer )
	: FMovieSceneTrackEditor( InSequencer ) 
{
}

FAudioTrackEditorOriginal::~FAudioTrackEditorOriginal()
{
}


TSharedRef<ISequencerTrackEditor> FAudioTrackEditorOriginal::CreateTrackEditor( TSharedRef<ISequencer> InSequencer )
{
	return MakeShareable( new FAudioTrackEditorOriginal( InSequencer ) );
}


void FAudioTrackEditorOriginal::BuildAddTrackMenu(FMenuBuilder& MenuBuilder)
{
	MenuBuilder.AddMenuEntry(
		...
		FUIAction(
			FExecuteAction::CreateRaw(this, &FAudioTrackEditorOriginal::HandleAddAudioTrackMenuEntryExecute)
		)
	);
}

void FAudioTrackEditorOriginal::BuildObjectBindingTrackMenu(FMenuBuilder& MenuBuilder, const TArray<FGuid>& ObjectBindings, const UClass* ObjectClass)
{
	if (ObjectClass != nullptr && ObjectClass->IsChildOf(AActor::StaticClass()))
	{
		MenuBuilder.AddSubMenu(
			...
			FNewMenuDelegate::CreateSP(this, &FAudioTrackEditorOriginal::HandleAddAttachedAudioTrackMenuEntryExecute, ObjectBindings));
	}
}


bool FAudioTrackEditorOriginal::SupportsType( TSubclassOf<UMovieSceneTrack> Type ) const
{
	return Type == UMovieSceneAudioTrack::StaticClass();
}


bool FAudioTrackEditorOriginal::SupportsSequence(UMovieSceneSequence* InSequence) const
{
...
void FAudioTrackEditorOriginal::BuildTrackContextMenu( FMenuBuilder& MenuBuilder, UMovieSceneTrack* Track )
{
...
const FSlateBrush* FAudioTrackEditorOriginal::GetIconBrush() const
{
	return FEditorStyle::GetBrush("Sequencer.Tracks.Audio");
}

bool FAudioTrackEditorOriginal::IsResizable(UMovieSceneTrack* InTrack) const
{
	return true;
}

void FAudioTrackEditorOriginal::Resize(float NewSize, UMovieSceneTrack* InTrack)
{
...
bool FAudioTrackEditorOriginal::OnAllowDrop(const FDragDropEvent& DragDropEvent, UMovieSceneTrack* Track, int32 RowIndex, const FGuid& TargetObjectGuid)
{
...
FReply FAudioTrackEditorOriginal::OnDrop(const FDragDropEvent& DragDropEvent, UMovieSceneTrack* Track, int32 RowIndex, const FGuid& TargetObjectGuid)
{
	if (!Track->IsA(UMovieSceneAudioTrack::StaticClass()))
	{
	...
	for (const FAssetData& AssetData : DragDropOp->GetAssets())
	{
		...
		if (Sound)
		{
			if (TargetObjectGuid.IsValid())
			{
				...
				AnimatablePropertyChanged(FOnKeyProperty::CreateRaw(this, &FAudioTrackEditorOriginal::AddNewAttachedSound, Sound, OutObjects));
			}
			else
			{
				AnimatablePropertyChanged(FOnKeyProperty::CreateRaw(this, &FAudioTrackEditorOriginal::AddNewMasterSound, Sound, RowIndex));
			}
...
TSharedRef<ISequencerSection> FAudioTrackEditorOriginal::MakeSectionInterface( UMovieSceneSection& SectionObject, UMovieSceneTrack& Track, FGuid ObjectBinding )
{
	check( SupportsType( SectionObject.GetOuter()->GetClass() ) );
	return MakeShareable( new FAudioSectionOriginal(SectionObject, GetSequencer()) );
}

TSharedPtr<SWidget> FAudioTrackEditorOriginal::BuildOutlinerEditWidget(const FGuid& ObjectBinding, UMovieSceneTrack* Track, const FBuildEditWidgetParams& Params)
{
	...
	[
		FSequencerUtilities::MakeAddButton(LOCTEXT("AudioText", "Audio"), FOnGetContent::CreateSP(this, &FAudioTrackEditorOriginal::BuildAudioSubMenu, FOnAssetSelected::CreateRaw(this, &FAudioTrackEditorOriginal::OnAudioAssetSelected, Track), FOnAssetEnterPressed::CreateRaw(this, &FAudioTrackEditorOriginal::OnAudioAssetEnterPressed, Track)), Params.NodeIsHovered, GetSequencer())
	];
}

bool FAudioTrackEditorOriginal::HandleAssetAdded(UObject* Asset, const FGuid& TargetObjectGuid)
{
...
FKeyPropertyResult FAudioTrackEditorOriginal::AddNewMasterSound( FFrameNumber KeyTime, USoundBase* Sound, int32 RowIndex )
{
...
FKeyPropertyResult FAudioTrackEditorOriginal::AddNewAttachedSound( FFrameNumber KeyTime, USoundBase* Sound, TArray<TWeakObjectPtr<UObject>> ObjectsToAttachTo )
{
	FKeyPropertyResult KeyPropertyResult;
...
/* FAudioTrackEditorOriginal callbacks
 *****************************************************************************/

void FAudioTrackEditorOriginal::HandleAddAudioTrackMenuEntryExecute()
{
	UMovieScene* FocusedMovieScene = GetFocusedMovieScene();
...
void FAudioTrackEditorOriginal::HandleAddAttachedAudioTrackMenuEntryExecute(FMenuBuilder& MenuBuilder, TArray<FGuid> ObjectBindings)
{
	MenuBuilder.AddWidget(BuildAudioSubMenu(FOnAssetSelected::CreateRaw(this, &FAudioTrackEditorOriginal::OnAttachedAudioAssetSelected, ObjectBindings), FOnAssetEnterPressed::CreateRaw(this, &FAudioTrackEditorOriginal::OnAttachedAudioEnterPressed, ObjectBindings)), FText::GetEmpty(), true);
}


TSharedRef<SWidget> FAudioTrackEditorOriginal::BuildAudioSubMenu(FOnAssetSelected OnAssetSelected, FOnAssetEnterPressed OnAssetEnterPressed)
{
...
void FAudioTrackEditorOriginal::OnAudioAssetSelected(const FAssetData& AssetData, UMovieSceneTrack* Track)
{
...
void FAudioTrackEditorOriginal::OnAudioAssetEnterPressed(const TArray<FAssetData>& AssetData, UMovieSceneTrack* Track)
{
	if (AssetData.Num() > 0)
	{
...
void FAudioTrackEditorOriginal::OnAttachedAudioAssetSelected(const FAssetData& AssetData, TArray<FGuid> ObjectBindings)
{
	FSlateApplication::Get().DismissAllMenus();
...
void FAudioTrackEditorOriginal::OnAttachedAudioEnterPressed(const TArray<FAssetData>& AssetData, TArray<FGuid> ObjectBindings)
{
	if (AssetData.Num() > 0)
	{
