...
namespace
{
    const FName ToolBlendModeShortcuts = "Tool's Blend Mode Shortcuts";
    const FName ToolColorBlendModeShortcuts = "Tool's Color Blend Mode Shortcuts";
    const FName ToolAlphaBlendModeShortcuts = "Tool's Alpha Blend Mode Shortcuts";
...
{
    AddBundle(ToolBlendModeShortcuts, LOCTEXT("editor-commands.category.tool-blend-mode-shortcuts", "Tool Blend Mode Shortcuts"));
    AddBundle(ToolColorBlendModeShortcuts, LOCTEXT("editor-commands.category.tool-color-blend-mode-shortcuts", "Tool Color Blend Mode Shortcuts"));
    AddBundle(ToolAlphaBlendModeShortcuts, LOCTEXT("editor-commands.category.tool-alpha-blend-mode-shortcuts", "Tool Alpha Blend Mode Shortcuts"));
...
// Brushes Shortcuts Category
    UI_BUNDLE_COMMAND( RecreateBrush, BrushStrokesShortcuts, "Refresh Brush", "Refresh Brush",                                                               EUserInterfaceActionType::Button, FInputChord( EKeys::F5 ) );
...
// Brush Blend Mode Shortcuts Category
    ...
    for (EOdysseyBlendMode Mode : TEnumRange<EOdysseyBlendMode>())
    {
        ...
        TSharedPtr<FUICommandInfo> commandInfo = FUICommandInfoDecl(
              this->AsShared()
            , FName( TEXT( "Set Tool's Blend Mode : ") + BlendModeName.ToString() )
            , FText::Format( LOCTEXT( "commands.set-tool-blend-mode.label", "Set Tool's Blend Mode to {0}%"), BlendModeName)
            , FText::Format( LOCTEXT( "commands.set-tool-blend-mode.tooltip", "Set Tool's Blend Mode to {0}%"), BlendModeName)
            , ToolBlendModeShortcuts
        )
        .UserInterfaceType( EUserInterfaceActionType::Button )
        .DefaultChord( FInputChord() );

        SetToolBlendMode.Add({ commandInfo, Mode });
    }

// Brush Color Blend Mode Shortcuts Category
    ...
    for (EOdysseyColorBlendMode Mode : TEnumRange<EOdysseyColorBlendMode>())
    {
        ...
        TSharedPtr<FUICommandInfo> commandInfo = FUICommandInfoDecl(
              this->AsShared()
            , FName( TEXT( "Set Tool's Color Blend Mode : ") + ColorBlendModeName.ToString())
            , FText::Format( LOCTEXT( "commands.set-tool-color-blend-mode.label", "Set Tool's Color Blend Mode to {0}%"), ColorBlendModeName)
            , FText::Format( LOCTEXT( "commands.set-tool-color-blend-mode.tooltip", "Set Tool's Color Blend Mode to {0}% (forces the blend mode to be composite)"), ColorBlendModeName)
            , ToolColorBlendModeShortcuts
        )
        .UserInterfaceType( EUserInterfaceActionType::Button )
        .DefaultChord( FInputChord() );

        SetToolColorBlendMode.Add({ commandInfo, Mode });
    }

// Brush Alpha Blend Mode Shortcuts Category
    ...
    for (EOdysseyAlphaBlendMode Mode : TEnumRange<EOdysseyAlphaBlendMode>())
    {
        ...
        TSharedPtr<FUICommandInfo> commandInfo = FUICommandInfoDecl(
              this->AsShared()
            , FName( TEXT( "Set Tool's Alpha Blend Mode : ") + AlphaBlendModeName.ToString())
            , FText::Format( LOCTEXT( "commands.set-tool-alpha-blend-mode.label", "Set Tool's Alpha Blend Mode to {0}%"), AlphaBlendModeName)
            , FText::Format( LOCTEXT( "commands.set-tool-alpha-blend-mode.tooltip", "Set Tool's Alpha Blend Mode to {0}% (forces the blend mode to be composite)"), AlphaBlendModeName)
            , ToolAlphaBlendModeShortcuts
        )
        .UserInterfaceType( EUserInterfaceActionType::Button )
        .DefaultChord( FInputChord() );

        SetToolAlphaBlendMode.Add({ commandInfo, Mode });
    }
