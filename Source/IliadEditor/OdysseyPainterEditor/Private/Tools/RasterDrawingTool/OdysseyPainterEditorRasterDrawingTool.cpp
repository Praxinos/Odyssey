...
void UOdysseyPainterEditorRasterDrawingTool::BindShortcuts(TSharedPtr<FUICommandList> iCommandList)
{
    ...
    for(TPair<TSharedPtr<FUICommandInfo>, EOdysseyBlendMode> Pair : painterEditorToolCommands.SetToolBlendMode)
    {
        MAP_ACTION(Pair.Key, SetBlendMode, Pair.Value )
    }

    for(TPair<TSharedPtr<FUICommandInfo>, EOdysseyColorBlendMode> Pair : painterEditorToolCommands.SetToolColorBlendMode)
    {
        MAP_ACTION(Pair.Key, SetColorBlendMode, Pair.Value )
    }

    for(TPair<TSharedPtr<FUICommandInfo>, EOdysseyAlphaBlendMode> Pair : painterEditorToolCommands.SetToolAlphaBlendMode)
    {
        MAP_ACTION(Pair.Key, SetAlphaBlendMode, Pair.Value )
    }
...
void
UOdysseyPainterEditorRasterDrawingTool::SetAlphaBlendMode(EOdysseyAlphaBlendMode InAlphaBlendMode)
{
    FOdysseyBlendParameters Value = BlendParameters;
    Value.SetCompositeColorBlendMode(BlendParameters.GetColorBlendMode());
    Value.SetCompositeAlphaBlendMode(InAlphaBlendMode);
    Value.SetIsComposite(true);
    FOdysseyObjectEditorUtils::SetPropertyValue(this, GET_MEMBER_NAME_CHECKED(UOdysseyPainterEditorRasterDrawingTool, BlendParameters), Value);

}

void
UOdysseyPainterEditorRasterDrawingTool::SetColorBlendMode(EOdysseyColorBlendMode InColorBlendMode)
{
    FOdysseyBlendParameters Value = BlendParameters;
    Value.SetCompositeColorBlendMode(InColorBlendMode);
    Value.SetCompositeAlphaBlendMode(BlendParameters.GetAlphaBlendMode());
    Value.SetIsComposite(true);
    FOdysseyObjectEditorUtils::SetPropertyValue(this, GET_MEMBER_NAME_CHECKED(UOdysseyPainterEditorRasterDrawingTool, BlendParameters), Value);
}

void
UOdysseyPainterEditorRasterDrawingTool::SetBlendMode(EOdysseyBlendMode InBlendMode)
{
    FOdysseyBlendParameters Value = BlendParameters;
    Value.SetBlendMode(InBlendMode);
    Value.SetIsComposite(false);
    FOdysseyObjectEditorUtils::SetPropertyValue(this, GET_MEMBER_NAME_CHECKED(UOdysseyPainterEditorRasterDrawingTool, BlendParameters), Value);
}
...
UOdysseyPainterEditorRasterDrawingTool::ApplyOverrides(UOdysseyBrushAssetBase* iBrushInstance)
{
    ...
    if (blendParametersOverrides)
    {
        ...
        if (blendParametersOverrides->bOverride_CompositeColorBlendMode)
            BlendParameters.SetCompositeColorBlendMode(blendParametersOverrides->CompositeColorBlendMode);

        if (blendParametersOverrides->bOverride_CompositeAlphaBlendMode)
            BlendParameters.SetCompositeAlphaBlendMode(blendParametersOverrides->CompositeAlphaBlendMode);
    }
