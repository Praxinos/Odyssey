...
UTexture2D*
ExportAsTexture(FOdysseyImageRenderingAbility* iObject, int iFrame, const FIntRect& iRect, ETextureSourceFormat iFormat, FString iAssetName, FString iPath )
{
    ...
    //Render frame block
    TSharedPtr<::ULIS::FBlock, ESPMode::ThreadSafe> block = MakeShared<::ULIS::FBlock>(iRect.Width(), iRect.Height(), blockFormat);

    TSharedPtr<IOdysseyImageRenderer> renderer = iObject->BuildImageRenderer(EOdysseyRenderingType::Render, iFrame);
    renderer->Init();

    FOdysseyImageRendererCopyParams params(block, { ::ULISUtils::ToIntRect(block->Rect()) }, ::ULISUtils::ToULISRectI(iRect).Position());
...
FString
ExportAsImage(FOdysseyImageRenderingAbility* iObject, ::ULIS::eFormat iULISFormat, int iFrame, EOdysseyExportImageFormat iFormat, const FIntRect& iRect, FString iFilename, FString iPath)
{
    ...
    ::ULIS::eFileFormat fileFormat = GetFileExportImageFormat(iFormat);
...
UPaperFlipbook*
ExportAsFlipbook(FOdysseyImageRenderingAbility* iObject, ::ULIS::eFormat iULISFormat, const FInt32Range& iRange, const FIntRect& iRect, float iFramesPerSecond, FString AssetName, FString Path)
{
    if ( Path.IsEmpty())
        return nullptr;
    ...
    for (int i = startFrame; i <= endFrame; i++)
    {
        ...
        UTexture2D* texture = ExportAsTexture(iObject, i, iRect, textureSourceFormat, textureName, flipbookPackagePath);
...
TArray<UTexture2D*>
ExportAsTextureSequence(FOdysseyImageRenderingAbility* iObject, ::ULIS::eFormat iULISFormat, const FInt32Range& iRange, const FIntRect& iRect, FString AssetName, FString Path)
{
    if ( Path.IsEmpty())
        return {};
    ...
    for (int i = startFrame; i <= endFrame; i++)
    {
        ...
        UTexture2D* texture = ExportAsTexture(iObject, i, iRect, textureSourceFormat, textureName, Path);
...
ExportAsImageSequence(
    FOdysseyImageRenderingAbility* iObject,
...
)
{
    ...
    for (int i = startFrame; i <= endFrame; i++)
    {
        ...
        FString fullpath = ExportAsImage(iObject, iULISFormat, i, Format, iRect, filename, Path );
