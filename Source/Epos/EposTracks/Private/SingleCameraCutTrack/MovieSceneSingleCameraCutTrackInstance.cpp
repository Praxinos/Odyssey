...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MovieSceneSingleCameraCutTrackInstance)

DECLARE_CYCLE_STAT(TEXT("Single Camera Cut Track Token Execute"), MovieSceneEval_SingleCameraCutTrack_TokenExecute, STATGROUP_MovieSceneEval);

namespace UE
{
namespace EposMovieScene
{
...
{
    using FCameraCutCache = UMovieSceneSingleCameraCutTrackInstance::FCameraCutCache;
...
void UMovieSceneSingleCameraCutTrackInstance::ToggleCameraCutLock(UMovieSceneEntitySystemLinker* Linker, bool bEnableCameraCuts, bool bRestoreViewports)
{
    using namespace UE::EposMovieScene;

    auto ForceEditorPreAnimatedStorageOperation = [](UMovieSceneSingleCameraCutTrackInstance* This, EForcedCameraCutPreAnimatedStorageOperation Operation)
    {
        using namespace UE::EposMovieScene;
        using namespace UE::MovieScene;
    ...
    auto ForceGamePreAnimatedStorageRestore = [](UMovieSceneSingleCameraCutTrackInstance* This)
    {
        using namespace UE::EposMovieScene;
        using namespace UE::MovieScene;
    ...
    {
        UMovieSceneSingleCameraCutTrackInstance* CameraCutTrackInstance = nullptr;
        for (auto It = TrackInstanceSystem->GetTrackInstances().CreateConstIterator(); It; ++It)
        {
            CameraCutTrackInstance = Cast<UMovieSceneSingleCameraCutTrackInstance>(It->TrackInstance.Get());
...
void UMovieSceneSingleCameraCutTrackInstance::OnInitialize()
{
#if WITH_EDITOR
    ViewportPreviewer = MakeUnique<UE::EposMovieScene::FCameraCutViewportPreviewer>();
#endif
}

void UMovieSceneSingleCameraCutTrackInstance::OnAnimate()
{
    using namespace UE::EposMovieScene;
    using namespace UE::MovieScene;
    ...
    for (const FCameraCutInputInfo& InputInfo : SortedInputInfos)
    {
        ...
        const UMovieSceneSingleCameraCutSection* Section = Cast<const UMovieSceneSingleCameraCutSection>(Input.Section);
        ...
        {
            const UMovieSceneSingleCameraCutTrack* Track = Section->GetTypedOuter<UMovieSceneSingleCameraCutTrack>();
            ...
            Params.bLockPreviousCamera = false; // Section->bLockPreviousCamera; //????????????????????????????
...
void UMovieSceneSingleCameraCutTrackInstance::OnEndUpdateInputs()
{
    using namespace UE::EposMovieScene;
    using namespace UE::MovieScene;
...
void UMovieSceneSingleCameraCutTrackInstance::OnDestroyed()
{
#if WITH_EDITOR
